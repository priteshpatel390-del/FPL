...
async function fetchHandler(request,env){
  ...
  if(request.method==='GET'&&url.pathname==='/v1/shadow/observations'){
    ...
    try{const sql=`SELECT * FROM shadow_observations WHERE admission_state='accepted' AND fetched_at <= ? ${cursor?'AND (fetched_at > ? OR (fetched_at = ? AND observation_id > ?))':''} ORDER BY fetched_at,observation_id LIMIT ?`;const args=cursor?[asOf,cursor.fetchedAt,cursor.fetchedAt,cursor.observationId,limit+1]:[asOf,limit+1];const rows=(await db.prepare(sql).bind(...args).all()).results||[];const more=rows.length>limit;const observations=rows.slice(0,limit);const last=observations.at(-1);return json({as_of:asOf,observations,next_cursor:more?encodeCursor({asOf,fetchedAt:last.fetched_at,observationId:last.observation_id}):null});}catch{return json({error:'storage_read_failed'},503);}
  }
  if(request.method==='POST'&&url.pathname==='/v1/shadow/observations'){
    let input;try{input=await body(request);}catch(error){return json({error:error.message==='request_too_large'?'request_too_large':'invalid_json'},error.message==='request_too_large'?413:400);}let revision,run;try{revision=await db.prepare('SELECT r.*,s.source_key,s.source_kind FROM data_source_revisions r JOIN data_sources s ON s.source_id=r.source_id WHERE r.source_revision_id=?').bind(input?.source_revision_id).first();run=await db.prepare('SELECT * FROM ingestion_runs WHERE run_id=? AND source_revision_id=?').bind(input?.ingestion_run_id,input?.source_revision_id).first();}catch{return json({error:'storage_read_failed'},503);}
    const verdict=validateObservation(input,revision);if(!verdict.ok){await safeRejection(db,input,verdict.reason,env.crypto||globalThis.crypto);return json({error:verdict.reason},422);}const row=verdict.observation;if(!run){await safeRejection(db,row,'schema_invalid',env.crypto||globalThis.crypto);return json({error:'run_source_mismatch'},422);}let provenance;try{provenance=await provenanceError(db,row,revision);}catch{return json({error:'storage_read_failed'},503);}if(provenance){await safeRejection(db,row,provenance,env.crypto||globalThis.crypto);return json({error:provenance},422);}
    row.admission_state=row.admission_state||'accepted';row.quality_state=row.quality_state||'fresh';const identity=await observationIdentity(row,env.crypto||globalThis.crypto);if(input.observation_id&&input.observation_id!==identity)return json({error:'observation_identity_mismatch'},422);row.observation_id=identity;
    try{const existing=await db.prepare('SELECT observation_id FROM shadow_observations WHERE source_revision_id=? AND logical_key=? AND input_revision=?').bind(row.source_revision_id,row.logical_key,row.input_revision).first();if(existing)return existing.observation_id===identity?json({accepted:true,result:'existing',observation_id:identity},200):json({error:'idempotency_conflict'},409);
      const fields=[row.observation_id,row.logical_key,row.ingestion_run_id,row.source_revision_id,row.category,row.subject_type,row.subject_entity_id,row.fixture_entity_id??null,row.competition_entity_id??null,row.subject_mapping_id??null,row.fixture_mapping_id??null,row.competition_mapping_id??null,row.provenance_kind,row.metric,row.value_type,row.value_number??null,row.value_text??null,row.value_boolean===undefined?null:Number(row.value_boolean),row.unit??null,row.observed_at,row.effective_at,row.fetched_at,row.expires_at,row.source_timestamp,row.provider_record_id??null,row.transform_version,row.validation_version,row.input_revision,row.admission_state,row.quality_state,row.conflict_group_id??null,'shadow_only',new Date().toISOString()];let result;try{result=await db.prepare(insertStatement()).bind(...fields).run();}catch{return reconcileInsertFailure(db,row,identity);}if(result?.meta?.changes!==1)return json({error:'storage_write_unconfirmed'},503);const stored=await db.prepare('SELECT observation_id FROM shadow_observations WHERE observation_id=?').bind(identity).first();if(stored?.observation_id!==identity)return json({error:'storage_write_unconfirmed'},503);return json({accepted:true,result:'inserted',observation_id:identity},201);}catch{return json({error:'storage_write_failed'},503);}
