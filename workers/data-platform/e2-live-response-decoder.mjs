...
export async function decodeE2QueryResponse(response,plan){
  ...
  let rowsRead=null,rowsWritten=null,changes=null,providerTimingMs=null,totalAttempts=null;const rowsByStatement=[];
  const add=(current,value)=>current===null?value:current+value;
  for(const item of payload.result){if(item.results!==undefined&&!Array.isArray(item.results))return malformed();rowsByStatement.push(item.results||[]);const meta=item.meta;if(meta!==undefined){if(!meta||typeof meta!=='object'||Array.isArray(meta))return malformed();for(const [key,target] of [['rows_read','rowsRead'],['rows_written','rowsWritten'],['changes','changes'],['total_attempts','totalAttempts']]){if(meta[key]!==undefined&&bounded(meta[key])===null)return malformed();if(meta[key]!==undefined){if(target==='rowsRead')rowsRead=add(rowsRead,meta[key]);else if(target==='rowsWritten')rowsWritten=add(rowsWritten,meta[key]);else if(target==='changes')changes=add(changes,meta[key]);else totalAttempts=add(totalAttempts,meta[key]);}}if(meta.duration!==undefined&&timing(meta.duration)===null)return malformed();if(meta.duration!==undefined)providerTimingMs=add(providerTimingMs,meta.duration);}}
  if([rowsRead,rowsWritten,changes,totalAttempts].some(value=>value!==null&&bounded(value)===null)||(providerTimingMs!==null&&timing(providerTimingMs)===null))return malformed();
  const lastMeta=payload.result.at(-1).meta,completionChanges=lastMeta&&bounded(lastMeta.changes);
  return Object.freeze({classification:E2_LIVE_RESPONSE_CLASS.SUCCESS,rowsByStatement:Object.freeze(rowsByStatement),rowsRead,rowsWritten,changes,providerTimingMs,totalAttempts,completionChanges});
}
