...
The environment-neutral Official FPL contract lives in `src/decision-intelligence/official-fpl-history-canonical.mjs` and is re-exported by `official-fpl-canonical.mjs`. It is the single implementation of season evidence, strict payload and relationship validation, canonical entities and logical facts, and explicit-null and scalar representation. Disappearance diffing, deterministic ordering, observation materialisation and SHA-256 observation identity remain in `official-fpl-canonical.mjs`. EIA-2I5B reuses that same normalizer to issue a content-bound 20-club authority; it does not define a weaker Official FPL team universe. The Worker adapter in `official-fpl-history.mjs` imports and re-exports that contract while retaining fixed upstream fetching, environment bindings, D1 reads/batches, run persistence, request wiring, and scheduled-event policy.
