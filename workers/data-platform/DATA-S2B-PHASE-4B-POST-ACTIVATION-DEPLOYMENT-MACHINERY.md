...
The collector sanitizes an error message with `.replace(/[^a-z0-9_-]/gi,'_').slice(0,64)`. From that sanitizer behavior, the earlier remediation inferred that the 30 and 31 August rows stored `Invalid_redirect_value__must_be_one_of__follow__or__manual_`, and PR #190 encoded that shorter exact value instead of the broad `TypeError` prefix anticipated by the first post-activation validator. Protected diagnostic run `33428846434` later disproved that inference for row index `0`: `error_class` was its sole mismatch and the exact live value was `Invalid_redirect_value__must_be_one_of__follow__or__manual____er`. No claim is made that every historical row shares that value; later rows remain unproven pending the next separately approved mutation-free preflight. All source, revision, run-type, mode, status, zero-record, zero-observation/head, migration, governance, D1, binding and Cron invariants remain unchanged.
...
Protected diagnostic preflight run `33428846434` on exact main `84cc0c38460d289134a889929cff0d576e1b5d92` passed its repository gate and failed closed in the Cloudflare read-only job. For historical row index `0`, the sole mismatch was `error_class`; every other emitted structural field matched. The exact live bounded value was `Invalid_redirect_value__must_be_one_of__follow__or__manual____er`, proving the previous shorter constant incorrect. This candidate replaces that one exact value without adding an alternative, prefix or wildcard; diagnostics and every other invariant remain fail-closed. Later historical rows remain unproven because validation stopped at index `0`. The run performed no production mutation. After merge, another mutation-free preflight remains separately owner-approved; Worker Version upload and Deployment remain later separate owner decisions.
