...
import {classifyE2TransportFailure,decodeE2DatabaseMetadataResponse,decodeE2QueryResponse,E2_LIVE_RESPONSE_CLASS} from './e2-live-response-decoder.mjs';
...
export function createE2LiveHttpAdapter({accountId,databaseId,token,fetchImpl,identity}){
  ...
  return Object.freeze({
    readExactMetadata:async()=>{let response;try{response=await fetchImpl(base,{method:'GET',redirect:'error',headers});}catch{return Object.freeze({classification:E2_LIVE_RESPONSE_CLASS.TRANSPORT});}return decodeE2DatabaseMetadataResponse(response,{databaseId,databaseName:identity.databaseName});},
