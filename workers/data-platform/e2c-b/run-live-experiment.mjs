...
import {validateE2PreMutationDiagnostic} from '../e2-live-response-decoder.mjs';
...
const approvedSha=required('APPROVED_SHA'),startedAt=canonicalNow();let rawEvidence=[],preMutationDiagnostics=[],state='FAILED',failureClassification=null;
try{
...
}catch(error){rawEvidence=Array.isArray(error?.evidence)?error.evidence:[];preMutationDiagnostics=Array.isArray(error?.preMutationDiagnostics)?error.preMutationDiagnostics.map(validateE2PreMutationDiagnostic):[];failureClassification=typeof error?.code==='string'&&/^e2[a-z0-9_-]{1,63}$/.test(error.code)?error.code:'e2c_b_failed';}
const endedAt=canonicalNow(),evidence=finalizeE2Evidence(rawEvidence,endedAt);
const report=Object.freeze({evidenceSchemaVersion:'e2c-b-report-v1',sourceSha:approvedSha,startedAt,endedAt,state,failureClassification,cleanupState:'NOT_PERFORMED_BY_E2C_B',preMutationDiagnostics:Object.freeze(preMutationDiagnostics),evidence});
