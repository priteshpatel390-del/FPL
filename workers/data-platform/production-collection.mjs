...
import {buildCommitBatch,buildCompleteUnchangedMutation,buildCurrentHeadsRead,buildProductionGovernanceRead,buildProductionPostflightRead,buildRunRead,buildStartRunMutation,estimateRoutineCommitRowsWritten,ROUTINE_WRITE_AMPLIFICATION} from './official-fpl-d1-rest-plan.mjs';
...
export async function runProductionCollection(options){
  ...
  async function finish(result,changed,recordsSeen){const stateRows=rows(await execute(buildProductionPostflightRead({runId,sourceRevisionId:DATA_S2_SOURCE_REVISION_ID})));const state=stateRows[0];validateProductionPostflight(stateRows,{runId,changed,recordsSeen});return Object.freeze({ok:true,result,runId,changed:Number(changed),recordsSeen:Number(recordsSeen),executionAt,state:Object.freeze({status:state.status,runObservations:Number(state.run_observations),observations:Number(state.observations),heads:Number(state.heads),logicalKeys:Number(state.logical_keys),orphanHeads:Number(state.orphan_heads),quarantined:Number(state.records_quarantined),rejected:Number(state.records_rejected)}),d1:Object.freeze({apiCalls:calls,rowsRead:read,rowsWritten:written,readClassification:classifyRowsRead(read),requestBytes:bytes})});}
}
