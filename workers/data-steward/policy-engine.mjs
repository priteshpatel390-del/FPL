...
const PROPOSAL_KEYS=['actionId','actionClass','incidentId','mainSha','expectedStateFingerprint','parameters','policyVersion','expiresAt','nonce'];
const CONTEXT_KEYS=['now','currentMainSha','currentIncidentId','currentClassification','currentClassificationReasonCode','currentExpectedStateFingerprint','authoritativeEvidence','consumedNonces','incidentActionCount','actionRetryCount','cooldownUntil','circuitBreakerTripped','killSwitches'];
...
function validEvidence(value){
  return Array.isArray(value)&&value.every(row=>exact(row,['incidentId','evidenceType','reference','hash'])&&text(row.incidentId,/^incident-[0-9a-f]{24}$/)&&text(row.evidenceType,/^[a-z][a-z0-9_]{1,63}$/)&&text(row.reference,/^[a-z][a-z0-9:/.@_-]{1,127}$/)&&text(row.hash,/^[0-9a-f]{64}$/));
}
...
export function evaluateActionPolicy(proposal,context){
  try{
    ...
    if(!iso(context.now)||!text(context.currentMainSha,/^[0-9a-f]{40}$/)||!text(context.currentIncidentId,/^incident-[0-9a-f]{24}$/)||!['GREEN','AMBER','RED'].includes(context.currentClassification)||!text(context.currentClassificationReasonCode,/^[A-Z][A-Z0-9_]{1,63}$/)||!text(context.currentExpectedStateFingerprint,/^[0-9a-f]{64}$/)||!validEvidence(context.authoritativeEvidence)||!Array.isArray(context.consumedNonces)||!context.consumedNonces.every(value=>text(value,/^[a-zA-Z0-9_-]{16,96}$/))||!nonNegative(context.incidentActionCount)||!nonNegative(context.actionRetryCount)||(context.cooldownUntil!==null&&!iso(context.cooldownUntil))||typeof context.circuitBreakerTripped!=='boolean'||!validSwitches(context.killSwitches)||secretFinding(context.authoritativeEvidence)!==null)return deny(proposal,'MALFORMED_REQUEST');
    if(!exact(proposal.parameters,Object.keys(proposal.parameters??{}))||secretFinding(proposal)!==null)return deny(proposal,'MALFORMED_REQUEST');
    ...
    if(action.actionId==='owner.d1.migration.review'&&!text(proposal.parameters.proposalId,/^[0-9a-f]{64}$/))return deny(proposal,'UNEXPECTED_PARAMETER');
    ...
    if(context.authoritativeEvidence.some(evidence=>evidence.incidentId!==context.currentIncidentId))return deny(proposal,'EVIDENCE_INCIDENT_MISMATCH');
    if(action.requiredEvidence.some(required=>!context.authoritativeEvidence.some(evidence=>evidence.evidenceType===required)))return deny(proposal,'MISSING_REQUIRED_EVIDENCE');
    if(!action.permittedClassifications.includes(context.currentClassification))return deny(proposal,'CLASSIFICATION_NOT_PERMITTED');
