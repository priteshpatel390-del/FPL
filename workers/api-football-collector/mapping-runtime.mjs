...
export const API_FOOTBALL_RUNTIME_MAPPING_SOURCE_REVISION_ID='api-football:eia-2i5a:1';
export const API_FOOTBALL_RUNTIME_MAPPING_PERSISTENCE_KIND='api-football-team-mapping-persistence-v1';
...
async function mappingIdFor(row,cryptoImpl){
  const hash=await sha256Hex(stableStringify(canonicalise({
    sourceRevisionId:API_FOOTBALL_RUNTIME_MAPPING_SOURCE_REVISION_ID,
...
function qualifiedProvenanceValid(row){
  return row&&row.state==='COMMITTED'&&row.fpl_season===API_FOOTBALL_FPL_SEASON&&
    row.source_revision_id===API_FOOTBALL_RUNTIME_MAPPING_SOURCE_REVISION_ID&&Number(row.mapping_count)===20&&
...
export async function readQualifiedTeamMappings(db,{season=API_FOOTBALL_FPL_SEASON,authority=null,cryptoImpl=globalThis.crypto}={}){
  ...
  let qualification;try{
    qualification=await db.prepare(
  ...
  }catch{return fail('qualified_mapping_unavailable');}
  ...
  if(rows.length!==20||new Set(rows.map(row=>String(row.provider_team_id))).size!==20||
    new Set(rows.map(row=>row.canonical_fpl_team_id)).size!==20)return fail('qualified_mapping_members_invalid');

  const mappings=[];
  for(const row of rows){
    if(row.source_revision_id!==API_FOOTBALL_RUNTIME_MAPPING_SOURCE_REVISION_ID||row.provider_entity_type!=='team'||
    ...
    const valid=validateProviderMapping(mapping,{entityType:'team',season});
  ...
  const persistenceBasis=canonicalise({
    kind:API_FOOTBALL_RUNTIME_MAPPING_PERSISTENCE_KIND,
    fplSeason:season,
    sourceRevisionId:qualification.source_revision_id,
    ...
    ownerReviewedAt:qualification.owner_reviewed_at,
    ...
    rows:rows.map(row=>({
      ...
      canonicalFplTeamId:row.canonical_fpl_team_id,
      mappingMethod:row.mapping_method,
      mappingVersion:Number(row.mapping_version),
      receiptIntegrityHash:row.receipt_integrity_hash
    }))
  });
  ...
  return deepFreeze({
    ok:true,
    fplSeason:season,
    verifiedPremierLeagueTeamCount:20,
    completeTwentyClubCoverage:true,
    qualificationId:qualification.qualification_id,
    persistenceIntegrityHash:qualification.persistence_integrity_hash,
