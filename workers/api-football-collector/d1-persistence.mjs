...
export async function reserveAttempt(db,input){
  ...
  const result=await db.prepare(`UPDATE api_football_runtime_state SET quota_utc_day=?,daily_attempt_count=?,quota_state=?,observed_daily_limit=?,observed_daily_remaining=?,observed_minute_limit=?,observed_minute_remaining=?,quota_observed_at=?,in_flight_attempt_id=?,in_flight_lease_expires_at=?,updated_at=? WHERE provider=? AND collection_enabled=1 AND (in_flight_attempt_id IS NULL OR in_flight_lease_expires_at<=?) AND (earliest_next_request_at IS NULL OR earliest_next_request_at<=?)`).bind(decision.utcDay,decision.nextCount,decision.probeRequired?'PROBE_REQUIRED':state.quota_state,decision.probeRequired?null:state.observed_daily_limit,decision.probeRequired?null:state.observed_daily_remaining,decision.probeRequired?null:state.observed_minute_limit,decision.probeRequired?null:state.observed_minute_remaining,decision.probeRequired?null:state.quota_observed_at,input.attemptId,decision.leaseExpiresAt,now,API_FOOTBALL_PROVIDER,now,now).run();
  if(result?.meta?.changes!==1)return {ok:false,reason:'request_lease_lost'};
  try{await db.prepare("INSERT INTO api_football_request_attempts (attempt_id,logical_request_id,attempt_number,operation_class,endpoint_class,generation_id,ingestion_run_id,source_revision_id,quota_utc_day,reserved_at,lease_expires_at,outcome) VALUES (?,?,?,?,?,?,?,?,?,?,?,'RESERVED')").bind(input.attemptId,input.logicalRequestId,input.attemptNumber,input.operationClass,input.endpointClass,input.generationId??null,input.ingestionRunId??null,input.sourceRevisionId??null,decision.utcDay,now,decision.leaseExpiresAt).run();}
  catch{return {ok:false,reason:'attempt_duplicate_or_write_failed'};}
...
export async function completeAttempt(db,{attemptId,completion,now}){
  ...
  await db.batch([
    ...
    db.prepare('UPDATE api_football_runtime_state SET quota_state=?,observed_daily_limit=?,observed_daily_remaining=?,observed_minute_limit=?,observed_minute_remaining=?,quota_observed_at=?,in_flight_attempt_id=NULL,in_flight_lease_expires_at=NULL,earliest_next_request_at=?,last_429_at=COALESCE(?,last_429_at),last_successful_request_at=CASE WHEN ?=\'SUCCEEDED\' THEN ? ELSE last_successful_request_at END,updated_at=? WHERE provider=? AND in_flight_attempt_id=?').bind(completion.quotaState,quota.dailyLimit??null,quota.dailyRemaining??null,quota.minuteLimit??null,quota.minuteRemaining??null,completion.quotaState==='KNOWN'?at:null,earliest,completion.last429At??null,completion.outcome,at,at,API_FOOTBALL_PROVIDER,attemptId)
  ]);
}
