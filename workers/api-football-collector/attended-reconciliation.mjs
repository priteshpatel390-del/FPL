...
export function classifyAttendedReconciliation(report){
  const runtime=report?.runtime||{},history=report?.priorState||{},inventory=report?.inventory||{};
  ...
  const success=runtime.credentialState==='AVAILABLE'&&history.requestAttempts===5&&history.attempt1Count===5&&history.attempt2Count===0&&
    history.succeededAttemptCount===5&&history.generations===1&&history.committedGenerationCount===1&&history.failedGenerationCount===0&&
    history.membershipConsistentCount===1&&history.headMatchCount===1&&history.fixtureRevisions>=0&&history.fixtureRevisions<=2500&&
    report.modelUiImportCount===0&&report.rawPayloadStoragePresent===false;
  return success?safe({ok:true,version:ATTENDED_RECONCILIATION_VERSION,classification:'ATTENDED_ACCEPTANCE_RECONCILED_SUCCESS',retryAuthorized:false}):stop('acceptance_state_ambiguous');
}
...
export async function main(){
  ...
  const output=safe({version:result.version,observedAt:report.observedAt,approvedSha:report.approvedSha,stage:report.stage,
    classification:result.classification,reason:result.reason??null,retryAuthorized:false,runtime:report.runtime,priorState:report.priorState,
    inventory:report.inventory,modelUiImportCount:report.modelUiImportCount,rawPayloadStoragePresent:report.rawPayloadStoragePresent,
