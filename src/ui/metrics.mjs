...
async function encodedBytes(value){ return typeof TextEncoder!=='undefined'?new TextEncoder().encode(String(value)).length:String(value).length*2; }
...
async function enforceMetricBounds(index){
  ...
  const payloads=[];
  for(const row of rows.filter(item=>item.hasFullRecord)){const raw=await rawEvidenceGet(K_METRIC_PREFIX+row.recordId);if(raw!=null) payloads.push({row,bytes:await encodedBytes(raw)});else row.hasFullRecord=false;}
...
function currentMetadata(index,logicalKey){return index.find(row=>row.current&&row.logicalKey===logicalKey)||null;}
...
async function processTransferHorizons(){
  ...
  for(const start of evaluations.slice().sort((a,b)=>a.gameweek-b.gameweek)){
    ...
    const key=`${start.season}|transfer|gw${start.gameweek}|h${start.decisions.transferBasis.horizon}`,priorMeta=currentMetadata(index,key),prior=priorMeta?await loadMetricRecord(priorMeta.recordId):null;
...
async function processOutcomeMetric(outcomeId){
  ...
  const index=await loadMetricIndex(),logicalKey=`${outcome.season}|gw${outcome.gameweek}`,priorMeta=currentMetadata(index,logicalKey),prior=priorMeta?await loadMetricRecord(priorMeta.recordId):null;
...
function sampleClass(level){return level==='potentially_stable'?'good':level==='raw_only'?'bad':'plain';}
async function renderMetricStatus(){
  ...
  if(summary)setChildren(summary,[el('div',{class:'kpis mt-10'},metricKpi('Player MAE',fmt(p.mae)),metricKpi('Player RMSE',fmt(p.rmse)),metricKpi('Bias',fmt(p.bias)),metricKpi('Pearson',fmt(p.pearson)),metricKpi('Spearman',fmt(p.spearman)),metricKpi('Coverage rows',report.coverage.playerRows)),el('div',{class:`note ${sampleClass(report.player.sample.level)}`},report.player.sample.message),el('div',{class:'kpis mt-10'},metricKpi('Minutes MAE',fmt(m.continuous.mae)),metricKpi('Within 15m',pct(m.continuous.within15)),metricKpi('Start Brier',fmt(m.start.brier,3)),metricKpi('Appear Brier',fmt(m.appearance.brier,3)),metricKpi('60m Brier',fmt(m.sixty.brier,3))),el('div',{class:'kpis mt-10'},metricKpi('P10–P90 cover',pct(u.p10P90.coverage)),metricKpi('P10–P90 width',fmt(u.p10P90.width)),metricKpi('P25–P75 cover',pct(u.p25P75.coverage)),metricKpi('Blank Brier',fmt(u.blank.brier,3)),metricKpi('Haul Brier',fmt(u.haul.brier,3))),el('div',{class:'note plain'},`Missing predictions ${report.coverage.missingPredictions} · missing outcomes ${report.coverage.missingOutcomes}. Positive bias means Teamsheet overpredicted.`)]);
...
export {
  ...
  compactMetricMetadata,storeMetricRecord,recoverMetricJournal,clearMetricStorage,currentMetadata,currentMetricRecords,
