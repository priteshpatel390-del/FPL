...
export function validateProviderMapping(mapping,{entityType,season}={}){
  if(!mapping||mapping.provider!==API_FOOTBALL_PROVIDER||mapping.entityType!==entityType||!providerId(mapping.providerEntityId)||!SEASON.test(mapping.season||'')||mapping.season!==season||!TEAM_MAPPING_STATES.includes(mapping.status)||!providerId(mapping.revision)||!String(mapping.method||'').trim()||!String(mapping.provenance||'').trim())return fail('mapping_invalid');
...
export function candidatePlayerMapping({providerPlayerId,season,normalizedFullName,providerTeamMapping,canonicalTeamId}={}){
  if(!providerId(providerPlayerId)||!SEASON.test(season||'')||!String(normalizedFullName||'').trim()||providerTeamMapping?.status!=='VERIFIED'||providerTeamMapping.canonicalFplId!==canonicalTeamId)return fail('candidate_evidence_insufficient');
...
export function qualifyProviderFixture(fixture,{teamMappings=[],registry=apiFootballCompetitionRegistry(fixture?.season)}={}){
  ...
  const verified=teamMappings.filter(row=>row.provider===API_FOOTBALL_PROVIDER&&row.entityType==='team'&&row.season===fixture.season&&row.status==='VERIFIED'&&[home,away].includes(String(row.providerEntityId)));
  ...
  return deepFreeze({ok:true,identity,state:verified.length?'PROVIDER_QUALIFIED':'DISCOVERED',canonicalCompetitionId:fixture.canonicalCompetitionId,canonicalHomeTeamId:verified.find(row=>String(row.providerEntityId)===home)?.canonicalFplId??null,canonicalAwayTeamId:verified.find(row=>String(row.providerEntityId)===away)?.canonicalFplId??null});
}
export function crossSourceQualify(providerQualification,candidates=[]){
  ...
  const providerTeams=[providerQualification.canonicalHomeTeamId,providerQualification.canonicalAwayTeamId];
  const exact=providerTeams.every(id=>typeof id==='string'&&id.length>0)?candidates.filter(row=>row.season===providerQualification.identity.slice(0,7)&&row.canonicalCompetitionId===providerQualification.canonicalCompetitionId&&row.canonicalHomeTeamId===providerQualification.canonicalHomeTeamId&&row.canonicalAwayTeamId===providerQualification.canonicalAwayTeamId&&row.identitiesResolved===true):[];
...
export function normalizeParticipation({explicitLineupRole='NO_LINEUP_EVIDENCE',directMinutes=null,substitutionOn=false,substitutionOff=false,noSubOnEvidence=false}={}){
  if(!LINEUP_ROLES.includes(explicitLineupRole)||directMinutes!==null&&(!Number.isInteger(directMinutes)||directMinutes<0||directMinutes>130))throw new Error('participation_invalid');
  ...
  else if(explicitLineupRole==='BENCH'&&directMinutes===0&&noSubOnEvidence)appearanceState='NOT_USED';
  const eventMinutes=substitutionOn||substitutionOff;
  const conflictState=directMinutes===0&&substitutionOn||directMinutes>0&&explicitLineupRole==='BENCH'&&noSubOnEvidence?'CONFLICTED':'NONE';
  return deepFreeze({lineupRole:explicitLineupRole,appearanceState,directMinutes,eventEvidence:{substitutionOn,substitutionOff,present:eventMinutes},conflictState});
}
