...
export function createApprovalLedger(records=[]){
  ...
  for(const raw of records){
    ...
    if(approvals.has(key))throw new Error('approval_duplicate');approvals.set(key,Object.freeze(row));
  }
...
export function createShadowRepository(){
  ...
  return Object.freeze({
    add(observation){if(observation?.boundary?.capability!=='shadow_only'||!/^obs-[0-9a-f]{64}$/.test(observation?.identity?.observationId||''))throw new Error('shadow_observation_invalid');observations.set(observation.identity.observationId,observation);return observation.identity.observationId;},
    list(){return Array.from(observations.values()).sort((a,b)=>a.identity.observationId.localeCompare(b.identity.observationId));},
