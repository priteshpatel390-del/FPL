...
export function normalizeApiFootballDiscoveryFixtures(payload,planItem,{fetchedAt,sourceRevision}={}){
  ...
  return deepFreeze({ok:true,fixtures});
}
...
export async function runApiFootballDiscoveryScan({
  apiKey,fetchImpl,sleepImpl,budget,nowImpl,sourceRevision,rights,teamMappings=[],independentCandidates=[],kickoffObservations=[]
}={}){
  ...
  const coverage=mappingCoverage(teamMappings,plan.fplSeason);
  ...
  const result={
    ...
    attempts,fixtures,audit,mappingCoverage:coverage,limitations,origin:API_FOOTBALL_ORIGIN,collectionMode:API_FOOTBALL_COLLECTION_MODE
  };
