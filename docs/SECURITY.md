...
Phase 3 repository support separates the exact-main/CI job from a protected deployment environment and serializes all runs through one non-cancelling concurrency group. Cloudflare Create Deployment requires **Workers Scripts Write**, a known coarse permission that can also authorize Worker script/Version, Cron, Custom Domain and Worker-secret mutations; the API token cannot permission-scope those surfaces away. **D1 Read** is additionally required for bounded validation. Do not grant D1 Write, Workers Routes Write, Zero Trust Edit or unrelated permissions. Protected-environment owner review, exact-main/exact-head-CI gating, serialized execution and executable endpoint restrictions compensate for the coarse Workers permission; the helper itself has no Version upload, script, Cron, domain or secret mutation path. API token, account ID, D1 UUID, retained bearer token and Access service credentials are masked; raw Cloudflare responses are not persisted. Pre-mutation old-production health, candidate health and rollback health use the existing Access service-token path plus retained Worker bearer boundary without exposing either value. Mutation ambiguity is reconciled read-only and never automatically retried. This path is not dispatched and changes no live security state. See [Phase 3 deployment gating](../workers/data-platform/DATA-S2B-PHASE-3-DEPLOYMENT-GATING.md).
