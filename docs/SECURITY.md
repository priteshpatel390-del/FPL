## Owner-crosswalk mapping closeout security boundary

The approved 20-club crosswalk is not committed to this public repository. Its exact content is accepted only through protected GitHub Environment secret `API_FOOTBALL_OWNER_CROSSWALK_JSON` and must match repository-pinned SHA-256 `d48c7980d1c39e7d4a6f82cb56d25a750286c925fd4996d0f8c728a4fb45c7f5`. The secret is scoped to one execution step and is never written to disk, echoed, summarized, artifacted or forwarded to either Official FPL or API-Football. The closeout workflow never receives `API_FOOTBALL_API_KEY` and makes zero API-Football requests. Provider evidence is the exact prior sanitized artifact, downloaded through a read-only GitHub token and verified against pinned artifact SHA-256 before use. Current Official FPL bootstrap+fixtures are fetched directly in memory through a two-request closed allowlist; raw payloads are discarded with the job and never serialized. Mapping receipts and the full qualification table remain in memory only. Persisted output is an allowlisted sanitized result containing counts, timestamps and integrity hashes, not provider IDs or mapping pairs. Exact-main checks, `run_attempt == 1`, branch-protected environment admission and immutable action SHAs remain required.
