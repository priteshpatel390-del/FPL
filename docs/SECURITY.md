...
## API-Football team-universe attended workflow remediation candidate

The owner-approved remediation adds a new manual-only workflow dedicated to the already-reviewed `GET /teams?league=39&season=2026` harness rather than reusing the historical R7 11-request workflow. The new workflow executes exact current `main`, uses environment `eia-api-football-qualification`, exposes `API_FOOTBALL_API_KEY` to exactly one step, pins all external actions to immutable SHAs, blocks UI reruns with `run_attempt == 1`, reconfirms remote `main` after environment admission and immediately before egress, and wraps transport with an independent two-fetch ceiling. The result is written only under `RUNNER_TEMP`, uploaded with seven-day retention, and summarized from an allowlist of normalized evidence fields. It cannot issue mapping receipts, apply migration 0005, enable collection, declare Cron, mutate D1 or connect provider evidence to model/UI paths. This repository-only remediation makes zero provider requests and accesses zero secrets. Before any later dispatch, an active independently verified ruleset protecting `refs/heads/main` and the protected-environment reviewer/bypass/branch settings must be confirmed.
