...
Draft PR #262 adds no live credential or mutation. A future migration-0004 dispatch is manual-only, run-attempt 1 only, exact protected current main only and requires exact-head Verify success before credentials. Read admission uses the existing `data-steward-readonly` boundary; mutation uses the existing protected D1-write environment and no API-Football key. All production writers are serialized through `data-s2-production-collection`, and the migration-specific preflight hard-stops if the historical direct `teamsheet-data-platform` Cron has reappeared. The migration file is byte-pinned by Git blob SHA and has no arbitrary SQL/path/version input. Time Travel bookmarks remain in process memory; only SHA-256 digests and a checkpoint timestamp enter sanitized evidence. No provider payload, private crosswalk, mapping receipt, secret, token, account identifier or raw Cloudflare body is uploaded. See [migration 0004 production application foundation](API-FOOTBALL-MIGRATION-0004-PRODUCTION-APPLICATION.md).
