...
The remediation applies the PR #215 pattern: the fingerprint is removed from job-level `env:`; a new first step reads only the already-secret `DATA_STEWARD_CLOUDFLARE_ACCOUNT_ID`, fails closed if absent, derives its SHA-256 locally, and registers `::add-mask::<derived hash>` with no network request and no `GITHUB_ENV`/`GITHUB_OUTPUT` write; the fingerprint is materialised only at step level in the final `Execute one read-only observation` step, after the mask is registered. The account id and Cloudflare read token stay `secrets.*` and are therefore already masked by GitHub; the fingerprint stays `vars.*` by design — it is an independently supplied non-secret identity proof, not converted into a secret. No credential is created, rotated or widened; workflow permissions, the exact `refs/heads/main` guard, `deployment: false` and the schedule/activation gates below are all unchanged. No mutation, no live credential value and no schedule activation occurred in this remediation.

<!-- DATA-OPS-A1-3-2026-09-08 -->
## DATA-OPS A1.3 security boundary

Dedicated observer workflow permissions are exactly `contents: read`, `actions: read`, `checks: read`. Ephemeral `${{ github.token }}` is mapped to `DATA_STEWARD_GITHUB_TOKEN`; no PAT exists. Protected environment `data-steward-readonly` holds Cloudflare account identity/fingerprint and a token limited to Workers Scripts Read plus D1 Read; the fingerprint is materialised only in the final observer-execution step (see the masking remediation above), never at job level. Scheduled execution requires exact `DATA_STEWARD_SCHEDULED_ENABLED=true`; absent or any other value skips it. Runtime logs use closed repository-owned fields and exclude identifiers, tokens, raw bodies/logs, headers and provider text. No write endpoint, mutation credential, deployment credential, AI key or D1 write surface is added.
