...
A3-SC-1 is the narrow reviewed-deletion checkpoint that follows Route-Aware Rendering and Performance M1. The authoritative base is GitHub `main` `02ea634464cc415ac43d4b9cb13b4005fc276646`, merge of PR #115. Permanent Verify Teamsheet run #183 / `31459954883` succeeded on that exact base, on an **866-test** baseline.
...
A temporary branch-only workflow, used earlier to bridge a restricted execution environment that could not clone GitHub, was removed before the final diff. `.github/workflows/` contains only `verify.yml` on `main`, on the PR head and in the final tree, and the repository security test rejecting write-enabled workflow permissions is unchanged.

## Verification status

Local verification on the corrected tree: `./run-tests.sh` passed **868 tests, 868 passed, 0 failed, 0 skipped, 0 cancelled**, and `scripts/verify-build-provenance.mjs` verified committed provenance from the reachable source revision. That is 866 inherited from `main` plus the two A3-SC-1 structural regressions.

Permanent Verify Teamsheet evidence on the exact final PR head is the governing gate; the exact head SHA and run number are recorded in the PR #116 description. Earlier helper/workflow runs are not substitutes for that permanent repository gate.

No physical iPhone testing has been performed or claimed. This cleanup has no intended visual or interaction change, and the automated evidence above is the only evidence for that claim.
...
Pritesh approved this narrow implementation and the factual stale-documentation reconciliation. **Merge remains separately approval-gated.** Do not merge PR #116 until permanent verification is green and Pritesh explicitly approves the merge.
