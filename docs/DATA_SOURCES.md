## API-Football inactive staging does not change source approval

The dormant collector staging candidate adds no provider, field, retention right or model input. API-Football remains shadow-only. The future inactive Version would contain no `API_FOOTBALL_API_KEY`, make no provider request and retain `EIA_2I5D_ACTIVATION=REPOSITORY_ONLY_BLOCKED`. The explicit production D1 binding is infrastructure connectivity only; it is not approval to collect, persist new provider responses, enable workload ingestion or influence projections/recommendations.
...
## API-Football migration 0005 application foundation — source remains dormant

The migration-0005 production application candidate does not change API-Football qualification or activate collection. A later separately approved application would seed only the existing `api-football:eia-2i5a:1` normalized private-use source revision and dormant runtime/provenance schema, with the runtime row explicitly disabled. The repository workflow receives no API-Football credential and contains no provider request path. Migration 0005 contains no private 20-team crosswalk; qualified mapping persistence belongs to separately gated migration 0006. Official FPL remains authoritative and no model/product read path changes. No live migration occurs in this repository checkpoint. See [migration 0005 production application foundation](API-FOOTBALL-MIGRATION-0005-PRODUCTION-APPLICATION.md).
