## Owner-approved 20-club mapping closeout source boundary

This checkpoint adds no provider and no new API-Football request. The provider-side input is the already captured normalized, integrity-bound team-universe evidence from attended run `35328500278`, artifact `10540321648`; the closeout workflow pins its archive digest and provider-universe revision. The owner-approved provider-ID crosswalk is not published in the repository and is admitted only from protected-environment secret `API_FOOTBALL_OWNER_CROSSWALK_JSON`, whose approved content is hash-bound to `d48c7980d1c39e7d4a6f82cb56d25a750286c925fd4996d0f8c728a4fb45c7f5`. Official FPL remains the authoritative Premier League source: the closeout fetches the existing bootstrap-static and fixtures endpoints in memory and requires successful DATA-S2A canonical validation before receipts can qualify. Raw bodies, the crosswalk and receipts are not retained. Current durable admitted mapping remains 2/20 until the attended closeout succeeds; no collector/runtime/model/UI source path changes.

## API-Football 2026 Premier League team-universe qualification path

API-Football remains shadow-only and inactive in production. The current qualification-only source operation for Premier League team identity review is exactly `GET https://v3.football.api-sports.io/teams?league=39&season=2026`. A dedicated attended workflow may execute that request only after repository/security preconditions pass. It retains provider team ID plus bounded display name/code and integrity/provenance metadata; raw provider bodies and credentials are not retained. These facts establish the provider universe only and do not create Official FPL mappings. Chelsea 49→6 and Leeds 63→13 remain the only admitted current-season team mappings.

## Current API-Football mapping and response-ceiling status

PR #254 merged the qualification-specific `/teams?league=39&season=2026` evidence path and explicit owner-reviewed mapping receipts to `main` as `83c7d5ce86077502ca691f7ff4e1941568dc083d`. Post-merge verification passed 2,162/2,162 tests and deterministic-build identity. EIA-2I5F's production repository ceiling is exactly **720,896 bytes**, bound to attended EIA-2I5E R7 evidence. The separate team-mapping gate remains **NO-GO 2/20** because PR #254 made zero provider requests and admitted no new mappings. `teams` is still absent from the collector source surface. API-Football remains `owner_risk_accepted_private_use`; raw bodies and credentials are not retained. See [mapping qualification](API-FOOTBALL-20-CLUB-MAPPING-QUALIFICATION.md) and [EIA-2I5F](EIA-2I5F-QUALIFIED-RESPONSE-CEILING.md).

## Retained EIA-2I5E qualification history — response-size status superseded

At the earlier R1/R3 stage, API-Football remained `owner_risk_accepted_private_use`; R3 made zero provider requests and preserved R1's 11 sanitized byte measurements. Because R1 discarded raw bodies without preserving exact response parameters and row identity, that checkpoint correctly remained response-size NO-GO and treated 720,896 bytes as historical arithmetic only. That response-size status is now superseded by the later attended EIA-2I5E R7 qualification and EIA-2I5F implementation: the current repository ceiling is **720,896 bytes**. The historical request-count and observed quota facts remain evidence: R1 used 11 provider calls; R2 and R3 used zero; observed headers showed 7,500/day and 300/minute, which are observed account limits rather than guaranteed entitlement or collector budget. Teamsheet's separate conservative internal ceiling remains 100 attempts/day. Team mapping remains independently NO-GO 2/20. See [EIA-2I5E](EIA-2I5E-PRELIVE-API-FOOTBALL-EVIDENCE-QUALIFICATION.md) and [EIA-2I5F](EIA-2I5F-QUALIFIED-RESPONSE-CEILING.md).
