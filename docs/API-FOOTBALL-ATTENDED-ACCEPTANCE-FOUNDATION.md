...
> **Current status — 22 September 2026:** attended preparation and credential recovery are live-complete through run `35736888644` on main `5a1bd41185e94639a8fc9aec2488f7ee4caf6f8c`. The inactive attended Version is `04d79556-3070-429f-9944-b5b53d799842`, created from immutable provenance SHA `69bb84fadbcce94e9fece3ff438d985866cce183`; credential state is `AVAILABLE`, collection remains disabled, Deployment/Cron/routes/domains remain zero and no attended acceptance provider request has occurred. The acceptance path requires repository remediation before dispatch because its initial admission, final critical recheck and reconciliation previously coupled immutable Version identity to the later current execution SHA. Live five-request acceptance remains separately owner-gated after remediation merge and exact-main verification.
...
## Immutable attended Version provenance remediation

The attended Version's historical identity is now represented as a pinned immutable contract: exact Version provenance SHA, exact message/tag, main module, compatibility date, binding contract and the exact 17 uploaded module SHA-256 values. Current workflow `APPROVED_SHA` remains the exact-current-main execution identity and cannot substitute for Version provenance. Initial admission, protected execution's final pre-mutation recheck and independent reconciliation all require `API_FOOTBALL_ATTENDED_VERSION_APPROVED_SHA=69bb84fadbcce94e9fece3ff438d985866cce183`; a missing, malformed or different SHA fails closed before Preview/D1 mutation or trigger-secret egress. The immutable module manifest is not reconstructed from a later working tree, so legitimate future source edits cannot rewrite the historical Version identity.
