## API-Football owner-approved 20-club crosswalk closeout

The owner approved the exact 2026/27 API-Football ↔ Official FPL crosswalk after attended provider-universe run `35328500278`. The public repository does not carry the 18 new provider-ID pairs: it carries only crosswalk hash `d48c7980d1c39e7d4a6f82cb56d25a750286c925fd4996d0f8c728a4fb45c7f5`. The actual crosswalk is supplied only through protected-environment secret `API_FOOTBALL_OWNER_CROSSWALK_JSON`. The repository candidate adds a manual exact-`main` closeout workflow that reuses the exact sanitized provider artifact, fetches current Official FPL bootstrap+fixtures in memory, creates/validates 18 new owner-reviewed receipts plus two trusted legacy receipts, and retains only a sanitized qualification result. No API-Football request, API key access, raw-body retention, mapping persistence, collector activation, migration, Cron, model or UI change is part of this checkpoint. Mapping is still NO-GO 2/20 until the post-merge attended closeout proves GO against current content-bound Official FPL authority.
