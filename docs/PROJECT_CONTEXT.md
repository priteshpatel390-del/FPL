## API-Football migration 0004 foundation — merged, workflow-context remediation pending

PR #262 merged as `3892f2c6448d951538cf318ad28392ca95ca2449`. Post-merge Verify run `35439821265` passed **2,214/2,214** tests with deterministic production rebuilds and Pages run `35439819493` succeeded. GitHub then recorded migration-workflow run `35439820507` as a zero-job failure on the merge push. This was not a migration attempt: no job, environment, Cloudflare credential, D1 operation or API-Football request started. The cause is a server-side context-scope error: `runner.temp` was referenced from job-level `env`, where the `runner` context is unavailable. The narrow remediation branch moves that report path to the already-existing mutation step `env` and adds a regression; its corrected pushes no longer generate the zero-job workflow-validation failure. Migration 0004 remains unapplied. GitHub remains only the administrative one-off migration runner; the earlier qualification key was temporary attended infrastructure and any future collector key remains a Cloudflare Worker secret. The next gate is merge/verify this repository fix, then a **separate explicit owner approval** before any production migration dispatch. See [migration 0004 production application foundation](API-FOOTBALL-MIGRATION-0004-PRODUCTION-APPLICATION.md).
