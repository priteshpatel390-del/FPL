...
Owner-approved run `35777295834` on exact main `cdcd0ed1ad2a2906b2a3c20aecd125ae3d711e4b` passed exact-head repository gating and fresh read-only admission. Protected execution then stopped before mutation as `attended_critical_state_drift__data_platform_binding_mismatch`. The successful admission and independent reconciliation, both using the dedicated steward read credential, prove the production data-platform D1 binding remained valid and the live state remained pristine: collection disabled, credential `AVAILABLE`, no lease, 0 attempts/generations/fixture revisions, Preview/workers.dev off, zero Deployments/Cron/routes/domains, exact attended Version/two-Version inventory, 0 production mutations and 0 API-Football requests. The run is consumed and must not be rerun.

The remediation removes the mutation credential from the critical read-only preflight. Protected execution now requires a distinct `CLOUDFLARE_ATTENDED_READ_TOKEN`; the mutation credential is retained only for the exact Preview-subdomain POST and production-D1 control POST allowlist. Repository code rejects identical read/mutation token values before network. Provisioning the new protected-environment secret and any future acceptance dispatch remain separate owner gates.

## API-Football attended run 35751021431 — protected critical recheck stopped before mutation

Owner-approved run `35751021431` on exact main `1981871d30315ad31cf5274f3f88529592b36856` passed exact-head repository gating and fresh read-only admission. All required protected execution values were present, but the executor's independent fresh critical recheck returned only the generic repository stop `attended_critical_state_drift`. The retained reconciliation artifact independently proves collection `0`, credential `AVAILABLE`, no active lease, 0 attempts, 0 generations, 0 fixture revisions, Preview/workers.dev off, zero Deployments/Cron/routes/domains, exact attended Version/two-Version inventory, 0 production mutations, 0 API-Football requests and 0 secret-value reads. The run is consumed and retry is not authorized.

Repository remediation now preserves the hard stop while adding a closed sanitized diagnostic enum for that pre-mutation recheck. This is diagnostic evidence only: it does not infer the failed Cloudflare permission, widen the mutation token, alter activation/model/provider behaviour, or authorize another acceptance run.

## API-Football attended acceptance run 35748566087 — safe Preview-identity false negative

The first owner-approved attended acceptance dispatch on exact main `ea423c7089a41287683e8d34f6e65175980e8a4c` stopped in fresh read-only admission before protected execution. Final reconciliation proved the live state remained pristine: credential `AVAILABLE`, collection `0`, no active lease, 0 attempts, 0 generations, 0 fixture revisions, Preview off, zero Deployments/Cron/routes/domains, exact attended Version, exact two-Version inventory, and zero provider requests or production mutations. The only attended admission predicate that failed was Preview URL identity. Investigation confirmed the code requested `preview_url_suffix` from Cloudflare's legacy Script Subdomain endpoint, whose response contract contains only `enabled` and `previews_enabled`. The remediation reads exact Preview suffix from the modern Get Worker metadata endpoint while retaining the legacy endpoint for state flags. Run `35748566087` is consumed; no automatic retry is authorized.

## API-Football attended acceptance provenance remediation candidate — 22 September 2026

Successful credential recovery run `35736888644` left the exact two-Version collector state independently reconciled with attended Version `04d79556-3070-429f-9944-b5b53d799842`, immutable creation/provenance SHA `69bb84fadbcce94e9fece3ff438d985866cce183`, credential `AVAILABLE`, collection disabled, zero Deployments/Cron/routes/domains and pristine request/generation/fixture-revision history. Investigation of the separately gated attended acceptance found the same repository provenance coupling previously fixed for recovery: acceptance admission, its final pre-mutation critical recheck and reconciliation supplied current execution `APPROVED_SHA` but not the immutable attended-Version provenance. The repository candidate pins the existing Version ID/provenance and historical 17-module SHA-256 identity, threads `versionApprovedSha` through all three phases, and fails closed on any different provenance without weakening module/binding/inventory validation. No live acceptance, Preview/D1 mutation, provider request or secret cleanup is authorized by this repository remediation.

## API-Football attended credential recovery provenance remediation — 22 September 2026

Recovery run `35735507066`, attempt 1, on main `34aa60616baac58a3edc30fc6b0a12c9dcac9621` failed closed during the independent Version proof with `preparation_version_inventory_unexpected`. The credential mutation and final closeout did not run; evidence recorded 0 production mutations, 0 API-Football requests and 0 secret-value reads. Root cause is repository-side provenance coupling: the immutable attended Version `04d79556-3070-429f-9944-b5b53d799842` was created/proved from `69bb84fadbcce94e9fece3ff438d985866cce183`, while recovery incorrectly used the newer workflow execution SHA to reconstruct its annotation identity. The approved repository remediation separates exact current-main execution identity from immutable Version provenance and pins both the known Version ID and creation SHA while retaining all existing fail-closed inventory, module, binding and mutation ceilings. A future production recovery dispatch remains a separate owner action after merge and exact-main verification.
...
**Supersedes the current-state Migration 0006 candidate/reconciliation sections below; they remain historical records.** Migration 0006 schema and the private qualified 20/20 mapping are live and independently verified. Persistence run `35607354636` on exact SHA `051a68841fd04dde8f5f239dadbfb285231646c3` returned `DEFINITELY_APPLIED_SUCCESSFULLY` with HTTP 200, 43/43 D1 statements successful, durable 20 entity mappings / 1 qualification / 20 members / 1 head, complete 20-club coverage, successful durable reread/reconstruction, and 0 API-Football requests. Earlier mutation-submitting runs `35575463178` and `35587588095` are consumed and forbidden from rerun.

Read-only reconciliation `35607762989` was a repository false negative, not a production-state failure: it incorrectly equated the fresh current qualification integrity hash with the historical approval hash. PR #276 corrected only that invariant and merged as `8c5c7fbf80b70961f2e39d8acc73c806d8c95659`; post-merge Verify Teamsheet `35608962183` and Pages succeeded. Fresh independent reconciliation `35609344995` then returned `COMPLETE_QUALIFIED_MAPPING_VISIBLE` with 24,536 rows read, 0 production mutations and 0 API-Football requests. Artifact `10643366243`, SHA-256 `15cead166619fbf07a99c6f7aa5a09b5416080f1c0d122e56b33771fa7b4c2c8`.

Migration 0006 mapping persistence is closed successfully. No further mapping write, repair, restore or retry is required. The next work is collector activation investigation/design only; production deployment, D1 collector binding, Worker secret, Cron, provider egress, workload ingestion and model/product use remain separately owner-gated.

## API-Football migration 0006 — reconciliation complete; attended persistence candidate

PR #272 merged as `93cfbe47f8c4c17783dc7126026d1a9d9b9da645`; exact-head Verify run `35581847822` succeeded. The approved read-only reconciliation run `35584694866`, attempt 1, completed successfully and classified the fixed aggregate diagnostic as `NO_SUBMITTED_MAPPING_STATE_VISIBLE`, with 0 production mutations and 0 API-Football requests. Its artifact is `10631743543`, SHA-256 `d0bd4c795a561f5dc3fe8c68c3ed28e256ec31cbff598bb0ea42130e6f02b014`. This establishes only that no submitted mapping state was visible to the diagnostic's fixed aggregate queries; it does not prove uninspected row facts. Recovery run `35575463178` submitted a mutation, remains consumed and must never be rerun. The new repository-only candidate is a distinct manual persistence gate: exact current main, first attempt, exact-head Verify, approved reconciliation identity/integrity/classification, fresh independent `mapping_pre`, pinned provider-universe evidence and nonempty protected crosswalk all precede the unchanged Migration 0006 mapping runner. Independent `mapping_post` remains read-only and sanitized. No production workflow dispatch or mutation is authorized by this candidate.
...
## API-Football migration 0005 — live production closeout complete

**Supersedes the migration-0005 candidate/remediation sections below; those remain historical.** PR #266 merged as `f07c8acee8aecacfd55ae0e7f015e7799d190294`; post-merge Verify run `35497426515` passed 2,225/2,225 with byte-identical exact-identity builds and Pages `35497425916` succeeded. Owner-attended run `35497565060` then completed successfully. Preflight admitted exact 0004 state as `READY_FOR_MIGRATION_0005` with only 1,439 account-wide D1 rows written that UTC day. Application classified `DEFINITELY_APPLIED_SUCCESSFULLY`, issued one production mutation, required no recovery, used 6 D1 API calls / 417,635 rows read / 41 rows written / 24,014 request bytes, and preserved the Official FPL authority digest exactly. Postflight independently accepted `READY_FOR_MIGRATION_0006`: ledger 0001–0005, zero FK violations, fresh 20-team Official FPL authority, runtime present but collection disabled, zero API-Football attempts/fixture revisions/mappings/mapping qualifications, collector absent, no Cron/API-key binding and zero API-Football requests. Migration 0005 is live; migration 0006 and every later activation/persistence/model/product gate remain separately owner-controlled.

## API-Football migration 0005 — first production dispatch failed safely; remediation candidate

Attended run `35496920942` on exact main `985cd1427dfa358c45be0e0dc54f1c746f16648d` passed the one-shot identity/main/Verify gates and 27/27 focused tests, then stopped at the repository cleanliness assertion. The production preflight, mutation and postflight jobs were skipped. Migration 0005 is therefore still unapplied. The exact dirty path was not logged, so its identity is not claimed. The approved remediation isolates the local Wrangler/D1 migration harness outside the checkout and adds a before/after git-status regression. Candidate Verify Teamsheet run `35497159414` on head `158da28c0884c48f72931d798da389dc924611c7` passed 2,225/2,225 tests with byte-identical exact-identity builds and exact manifest identity. A fresh production attempt is not authorized by this remediation.

## API-Football migration 0005 production application foundation — repository candidate

Owner approval now covers repository implementation of a dormant migration-0005 production application boundary only. Baseline protected `main` remains `094409e5dd31ca71bef8ff4905d7dbd8ba9fba07`; exact-main Verify run `35468057507` passed 2,214/2,214 tests and deterministic build identity, Pages `35468056858` succeeded, and the 20 September 01:17 UTC Official FPL external collection `35481020246` completed successfully. The candidate pins exact migration-0005 bytes/shape, shares the existing production-writer lock, requires the live-storage preflight plus zero historical direct Cron and conservative account-wide D1 daily write headroom, and isolates that analytics read behind a future Account Analytics Read-only secret that is not provisioned here. It captures a Time Travel bookmark, submits one mutation only and never automatically restores the whole database. No workflow dispatch or production mutation is part of this checkpoint. Migration 0006, private mapping, collector deployment/binding/secret/Cron/provider execution and model/UI influence remain separate owner gates. See [migration 0005 production application foundation](API-FOOTBALL-MIGRATION-0005-PRODUCTION-APPLICATION.md).

## API-Football migration 0004 — live production closeout

PR #263 merged as `bc87334dbdebce4e19d92374e6640a9fed0f44fa`; post-merge Verify run `35442144198` passed **2,214/2,214** tests with deterministic byte-identical production rebuilds and Pages run `35442143907` succeeded. Owner-attended migration run `35467787882` then completed successfully on exact protected main with four green jobs: repository gate, read-only preflight, migration0004 and postflight. Preflight classified production `STOP_0004_NOT_APPLIED`; the application classified `DEFINITELY_APPLIED_SUCCESSFULLY`, issued exactly one production mutation and required no recovery; postflight classified `READY_FOR_MIGRATION_0005`. The run made 0 API-Football requests and 0 Worker Cron/secret changes. Migration 0004 is now live; 0005/0006, private mapping persistence, collector/secret/Cron activation, provider egress and model/UI influence remain inactive and require separate owner approval. See [migration 0004 production application foundation](API-FOOTBALL-MIGRATION-0004-PRODUCTION-APPLICATION.md).

## API-Football live storage foundation — first live read and authority remediation

PR #260 merged as `d71be7f95b31093d167ce8e33ad3e3fb41785650`; exact-main Verify run `35407292664` passed 2,201/2,201 tests with deterministic production rebuilds. Owner-attended read-only preflight run `35426768910` then observed production directly with zero writes and zero API-Football requests. It proved the reviewed `teamsheet-data` identity and deployed binding match, migration ledger exactly 0001–0003, 0004/0005/0006 absent, zero FK violations, and no API-Football collector Worker/deployment/Cron/API-key secret binding. Its final `STOP_REVIEW_REQUIRED` classification is attributable to a repository validation mismatch: the query returned all ten canonical DATA-S2A team metrics per club while the authority validator required exactly 20 rows. The owner-approved remediation is repository-only: read only active `present=true` team heads, require the exact 20 canonical present logical keys, and permanently test the real multi-metric team shape. No migration or live mutation is authorized before the corrected read-only preflight is merged and rerun. See [API-Football Live Storage Foundation](API-FOOTBALL-LIVE-STORAGE-FOUNDATION.md).

## API-Football post-20/20 pre-live runtime completion — repository candidate

Owner-approved repository work now prepares two missing pre-live pieces without activating API-Football: migration 0006 can durably bind the private 20/20 mapping to its qualification provenance, and the dormant scheduled Worker can build a read-only pre-egress request plan from the existing scheduler. The real crosswalk remains private/hash-bound; current Wrangler stays `REPOSITORY_ONLY_BLOCKED`; no live migration, D1 mapping write, Worker secret/deployment, Cron, provider request, workload ingestion, model/calculation influence or UI use is authorized. Final-enrichment and correction remain blocked as `workload_ingestion_not_approved`. See [pre-live runtime completion](API-FOOTBALL-PRELIVE-RUNTIME-COMPLETION.md).

## API-Football 20-club mapping live closeout — GO 20/20

**This section supersedes earlier API-Football mapping-status sections below; those remain historical.** PR #257 merged as `aa694bc1d57faa6f197d4118ea0563808fa4ac0b` and exact-main Verify Teamsheet run `35332702502` passed 2,180/2,180 tests with deterministic byte-identical production rebuilds. Attended closeout run `35333898797` then executed once from protected `main` with `run_attempt=1`; both jobs succeeded. Sanitized result: **GO 20/20**, complete coverage true, 18 new owner-reviewed receipts, 2 trusted legacy receipts, 0 unresolved mappings and 0 receipt failures. It made exactly 2 Official FPL requests, 0 API-Football requests, and bound the decision to current Official FPL authority fetched at `2026-09-18T10:17:32.351Z`. Qualification integrity hash: `670ccccbf62764bc076293962ae4aa85085e06db48b503f597397d882f771367`. The approved private crosswalk remains hash-bound and undisclosed in repository content; raw bodies, crosswalk and receipts were not persisted. Team identity qualification is complete, but API-Football remains shadow-only: no live D1 mapping store, migration 0005 application, collector, Cron, model/calculation influence or UI use is activated.

## Earlier API-Football team-universe attended workflow remediation

Owner-approved remediation prepares a dedicated manual-only workflow for the merged Premier League team-universe qualification contract. It is deliberately separate from the historical EIA-2I5E R7 11-request response-size workflow. The new path executes only exact current `main`, invokes only `runAttendedApiFootballTeamUniverseQualification()`, independently caps HTTP egress at two, retains only sanitized integrity-bound team-universe evidence, and cannot issue mapping receipts or activate collector/runtime/model/UI paths. This change itself performs zero provider requests and zero credential accesses. Before any dispatch, `refs/heads/main` protection and the existing protected environment must be independently verified. At that remediation stage, mapping remained NO-GO 2/20 and the qualified byte ceiling was 720,896.

## Earlier API-Football mapping checkpoint — qualification contract merged

PR #254 merged the 20-club mapping qualification contract to `main` as `83c7d5ce86077502ca691f7ff4e1941568dc083d`. Post-merge Verify Teamsheet run `35321967633` passed 2,162/2,162 tests, byte-identical production rebuilds and exact manifest identity. EIA-2I5F's qualified response ceiling is 720,896 bytes. The repository now contains the closed qualification-only `/teams?league=39&season=2026` evidence harness and explicit owner-reviewed receipt contract, while admitted mapping at that checkpoint remained NO-GO 2/20 (Chelsea 49→6, Leeds 63→13). PR #254 made no API-Football provider request or credential access and did not activate the collector, D1 migration, Cron, model or UI. The next separate owner gate is an attended provider-universe observation and explicit mapping review/admission. See [20-club mapping qualification](API-FOOTBALL-20-CLUB-MAPPING-QUALIFICATION.md).
...
PR #237 (payload-decode split) and PR #238 (Cron semantic normalisation) both merged; post-merge `main` is `174a7ece2f6c52257902c79ac9a46de846edeb91` with exact-`main` Verify success (run `34345865860`). After that Verify, the owner performed the attended `Data Steward Read-Only Observer` dispatch recorded here: workflow run `34346126189`, run number 8, head `174a7ece2f6c52257902c79ac9a46de846edeb91`, job `observe-production-chain` (job id `102448024505`), result **SUCCESS**. Sanitized output: `verdict: HEALTHY`, `evaluationReason: HEALTHY_EXPECTED_STATE`, `heartbeat: COMPLETE`, `escalationRequired: false`; Cloudflare `OBSERVED`/`CLOUDFLARE_CONFIGURATION_OBSERVED`; D1 `OBSERVED`/`D1_STATE_OBSERVED`, `rowsRead: 89066`; GitHub `OBSERVED`/`GITHUB_CHAIN_OBSERVED`. No collection, repair, D1 write, Cloudflare mutation or schedule activation occurred.

This is A1.3's live technical acceptance: the merged Cron semantic canonicaliser was sufficient for the live Cloudflare configuration to be observed successfully, and all three sentinels agreed in one live evaluation. It is **not** a claim that scheduled/automatic monitoring is active — `DATA_STEWARD_SCHEDULED_ENABLED` was last owner-verified absent; this run and this PR do not create, set, modify or read it, and scheduled execution stays fail-closed until a later, separate, explicit owner action. A1.3 remains observe-only: no actuator, repair, AI, autonomous edit, Cloudflare mutation, D1 write or auto-merge capability. See [A1.3](DATA-OPS-A1-3-LIVE-READONLY-OBSERVER.md).
...
PR #236's response-layer remediation merged; post-merge `main` was `dea6a3239443970dd2e5495fe7759e187fb34e20` with exact-`main` Verify success. An attended dispatch, run `34325772296`, head `dea6a3239443970dd2e5495fe7759e187fb34e20`, proved identity admission continues to succeed, the request reaches HTTP 200, and both `response.json()` and `decodeEnvelope()` succeed (GitHub/D1 sentinels observed successfully, D1 `rowsRead: 89066`); this rules out every other broad `/schedules` category and places the failure precisely inside `decodeSchedules()`. The then-collapsed `CLOUDFLARE_SCHEDULES_PAYLOAD_INVALID` code covered five distinct predicates: the result not being an object (or being an array), `result.schedules` missing or non-array, the array exceeding the 16-entry bound, a row's cron not being a string, and a cron string failing the `CRON` regular expression. This remediation, merged as PR #237, replaced it with five closed categories (`CLOUDFLARE_SCHEDULES_RESULT_INVALID`, `CLOUDFLARE_SCHEDULES_ARRAY_INVALID`, `CLOUDFLARE_SCHEDULES_COUNT_EXCEEDED`, `CLOUDFLARE_SCHEDULES_CRON_NOT_STRING`, `CLOUDFLARE_SCHEDULES_CRON_PATTERN_REJECTED`); which predicate first rejected the result is read only internally to select one and never leaves the sentinel. `decodeSchedules()`'s accepted behaviour is not modified: as merged, a single internal `analyseSchedulesPayload()` is the sole shared definition and evaluation point of these predicates, and the exported `classifySchedulesPayload()` and `decodeSchedules()` are both one-line delegations to it, proven by a permanent parity test to never disagree. The other six `/schedules` categories, and `/deployments`/`/settings`, are unchanged. No permission widening, no fourth read, no decoder-behaviour change, no credential rotation, no Cloudflare/D1 mutation, no Cron change and no schedule activation occurred. See [A1.3](DATA-OPS-A1-3-LIVE-READONLY-OBSERVER.md).
...
PR #235's schedules-classification remediation merged; post-merge `main` is `90d7851d0084f45577c330e9fa7f1c15432f80c0` with exact-`main` Verify success. A fourth attended dispatch, run `34319945520`, head `90d7851d0084f45577c330e9fa7f1c15432f80c0`, proved identity admission continues to succeed and the request now reaches HTTP 200 (GitHub/D1 sentinels observed successfully, D1 `rowsRead: 89066`); this rules out the four other broad `/schedules` categories and places the failure inside HTTP-200 response processing. The still-collapsed `CLOUDFLARE_SCHEDULES_RESPONSE_INVALID` code covered three distinct response-processing layers: an unparseable JSON body, an invalid Cloudflare envelope, and a valid envelope carrying a result `decodeSchedules()` rejects. This remediation, merged as PR #236, replaced it with three closed categories (`CLOUDFLARE_SCHEDULES_JSON_INVALID`, `CLOUDFLARE_SCHEDULES_ENVELOPE_INVALID`, `CLOUDFLARE_SCHEDULES_PAYLOAD_INVALID`); which step first failed is read only internally to select one and never leaves the sentinel. `decodeEnvelope()` and `decodeSchedules()` are not modified; the other four `/schedules` categories, and `/deployments`/`/settings`, are unchanged. Its statement that the precise underlying failure category was unknown has since been answered by run `34325772296` above: `CLOUDFLARE_SCHEDULES_PAYLOAD_INVALID`, now itself further subdivided by the block above. No permission widening, no fourth read, no decoder change, no credential rotation, no Cloudflare/D1 mutation, no Cron change and no schedule activation occurred. See [A1.3](DATA-OPS-A1-3-LIVE-READONLY-OBSERVER.md).
...
PR #234's fixed-read diagnostic remediation merged: a third attended dispatch, run `34311398342`, head `465e54260005c96591bd77be0a1fe1cb44547631`, proved identity admission continues to succeed and narrowed the failure to the first fixed read, `/schedules` (GitHub/D1 sentinels observed successfully, D1 `rowsRead: 89066`; `/deployments` and `/settings` were not attempted, per the fail-closed sequential order). The per-stage code `CLOUDFLARE_SCHEDULES_READ_FAILED` was itself still a collapsed code covering a transport error, an authorization refusal, a missing-resource response, any other non-200, or a successful-but-unusable response. This remediation, merged as PR #235, replaced it, for `/schedules` only, with five closed categories (`CLOUDFLARE_SCHEDULES_AUTH_REFUSED`, `CLOUDFLARE_SCHEDULES_NOT_FOUND`, `CLOUDFLARE_SCHEDULES_HTTP_FAILED`, `CLOUDFLARE_SCHEDULES_RESPONSE_INVALID`, `CLOUDFLARE_SCHEDULES_TRANSPORT_FAILED`); the HTTP status is read only internally to select one and never leaves the sentinel. `/deployments` and `/settings` keep their single collapsed codes unchanged. Its statement that the precise underlying failure category was unknown has since been answered by run `34319945520` above: `CLOUDFLARE_SCHEDULES_RESPONSE_INVALID`, now itself further subdivided by the block above. No permission widening, no fourth read, no decoder change, no credential rotation, no Cloudflare/D1 mutation, no Cron change and no schedule activation occurred. See [A1.3](DATA-OPS-A1-3-LIVE-READONLY-OBSERVER.md).
...
The PR #233 identifier-masking remediation merged and worked live: a second attended dispatch, run `34277208819`, head `d9599c4aa557ce0727c4f8b6ddd24a4778b21497`, proved the masking step succeeded and Cloudflare identity admission now succeeds. GitHub and D1 sentinels observed successfully (D1 `rowsRead: 88580`); the Cloudflare sentinel failed closed at `CLOUDFLARE_READ_FAILED` — it reached its read phase, one of its three already-approved fixed `GET` reads (`/schedules`, `/deployments`, `/settings`) failed, but the collapsed code could not say which. This remediation, merged as PR #234, replaced that single code with three closed, stage-named reason codes (`CLOUDFLARE_SCHEDULES_READ_FAILED`, `CLOUDFLARE_DEPLOYMENTS_READ_FAILED`, `CLOUDFLARE_SETTINGS_READ_FAILED`), preserving the exact sequential fail-closed read order, the three-request cap, every decoder byte-for-byte, and the exact Workers Scripts Read plus D1 Read credential contract. Its statement that the precise failing stage was unknown has since been answered by run `34311398342` above: `/schedules`. No collection, repair, write, schema, Cloudflare mutation or provider/model change occurred. See [A1.3](DATA-OPS-A1-3-LIVE-READONLY-OBSERVER.md).

<!-- DATA-OPS-A1-3-2026-09-08-REMEDIATION -->
## Current Data-Ops checkpoint — A1.3 first-live-observation remediation (draft, unmerged)

The owner performed the first attended `Data Steward Read-Only Observer` dispatch: run `34269989975`, head `2f8a4850f911779d2ec48db2f835d0f6af5a45c5`. GitHub and D1 sentinels observed successfully (D1 `rowsRead: 88580`); the Cloudflare sentinel failed closed at `CLOUDFLARE_IDENTITY_MISMATCH` because the live fingerprint was provisioned as `sha256:<hash>` rather than the runtime contract's required raw `^[0-9a-f]{64}$` shape; overall verdict was `UNHEALTHY`. The same run proved GitHub Actions echoes a job-level `vars.*` fingerprint into every step's resolved-environment log. This draft, unmerged remediation removes the job's `env:` block entirely (no steward credential at job level at all), adds a first step that registers the account id's SHA-256 mask before any other step runs, and materialises the full runtime contract, fingerprint included, only in the final observer-execution step, following the PR #215 pattern tightened on owner review. It changes no live credential value, dispatches nothing, and activates no schedule. No collection, repair, write, schema, Cloudflare mutation or provider/model change occurs. See [A1.3](DATA-OPS-A1-3-LIVE-READONLY-OBSERVER.md).

<!-- DATA-OPS-A1-3-2026-09-08 -->
## Current Data-Ops checkpoint — A1.3 repository-ready, dormant observer

A1.2 merged in PR #231. A1.3 adds a dedicated GitHub Actions read-only observer with manual dispatch and two UTC schedule declarations (`17 4 * * *`, `17 8 * * *`). Scheduled execution is fail-closed unless `DATA_STEWARD_SCHEDULED_ENABLED` is exact `true`; no activation variable, environment, credential or live run is created here. Authenticated repository tooling received HTTP 403 reading that live variable and proves nothing about it; the owner verified through the GitHub owner UI that it is absent, with nothing created, edited or deleted, so the scheduled observer is dormant on merge. Scheduled activation remains a separate explicit owner-approved step. Every job also requires exact `refs/heads/main` and uses `deployment: false`. Ephemeral `github.token` supplies GitHub read access. Cloudflare remains Workers Scripts Read plus D1 Read. No collection, repair, write, schema or provider/model change occurs. See [A1.3](DATA-OPS-A1-3-LIVE-READONLY-OBSERVER.md).
...
internally consistent. The GitHub sentinel uses governed job/step metadata only to identify a
