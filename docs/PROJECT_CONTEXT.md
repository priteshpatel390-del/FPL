...
The owner-approved next checkpoint is now live storage foundation, beginning with a mutation-free production preflight. PR #260 adds a manual-only exact-current-main workflow using the existing least-privilege `data-steward-readonly` Cloudflare credential contract (Workers Scripts Read + D1 Read). It can verify the reviewed `teamsheet-data` identity/binding, ordered migration ledger, migration columns/triggers/indexes, API-Football schema/FKs/counts/runtime flags, current-and-fresh Official FPL authority and whether any collector Worker/Cron/API key secret binding unexpectedly exists. It makes zero production mutations and zero API-Football requests. Migration 0004 is an explicit stop gate: if production stops at 0003, 0005/0006 must not proceed until the owner separately decides 0004. No live preflight result exists yet. See [API-Football Live Storage Foundation](API-FOOTBALL-LIVE-STORAGE-FOUNDATION.md).
