...
> **FPL-T1 current work:** Teamsheet 2.0.7 is merged through PR #68 at `2f7e4ba6978ccd68e9d6c36b56d4063cea06beaf`. The owner-approved narrow Official FPL Cloudflare Worker is deployed at `teamsheet-fpl-gateway.fpltsheet.workers.dev`, the application is configured to its exact `/fpl` base, and a physical iPhone Safari request returned live 2026/27 bootstrap data. Draft PR #69 passes **566 passed, 0 failed, 0 skipped** with deterministic builds and unchanged model/rules. Transfers now opens without synchronous optimisation, runs the exact optimiser in a cancellable browser Web Worker and invalidates active work when controls or verified inputs change. Full populated application acceptance remains pending; no provider fields or football calculations changed.
