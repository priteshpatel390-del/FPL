## API-Football inactive collector staging — repository implementation candidate

Owner approval now covers repository implementation only of the dormant staging machinery designed in PR #281. The candidate adds one manual exact-main workflow, a closed two-mutation helper, independent read-only postflight and permanent tests. A future approved execution could create the inert `teamsheet-api-football-shadow-collector` shell with workers.dev/previews disabled and then upload exactly one undeployed Version with the explicit production D1 binding, three reviewed plain-text variables, blocked activation and zero secret bindings. The upload graph is the exact transitive 19-module repository graph rooted at `workers/api-football-collector/collector.mjs`. No live Cloudflare request, Worker, Version, D1 mutation, provider request, secret provisioning, Deployment or Cron is authorized by this repository candidate.
