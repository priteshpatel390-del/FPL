...
PR #236's response-layer remediation merged; post-merge `main` is `dea6a3239443970dd2e5495fe7759e187fb34e20` with exact-`main` Verify success. A fifth attended dispatch, run `34325772296`, head `dea6a3239443970dd2e5495fe7759e187fb34e20`, proved identity admission continues to succeed, the request reaches HTTP 200, and both `response.json()` and `decodeEnvelope()` succeed (GitHub/D1 sentinels observed successfully, D1 `rowsRead: 89066`); this rules out every other broad `/schedules` category and places the failure precisely inside `decodeSchedules()`. The still-collapsed `CLOUDFLARE_SCHEDULES_PAYLOAD_INVALID` code covered five distinct predicates: the result not being an object (or being an array), `result.schedules` missing or non-array, the array exceeding the 16-entry bound, a row's cron not being a string, and a cron string failing the `CRON` regular expression. This draft, unmerged remediation replaces it with five closed categories (`CLOUDFLARE_SCHEDULES_RESULT_INVALID`, `CLOUDFLARE_SCHEDULES_ARRAY_INVALID`, `CLOUDFLARE_SCHEDULES_COUNT_EXCEEDED`, `CLOUDFLARE_SCHEDULES_CRON_NOT_STRING`, `CLOUDFLARE_SCHEDULES_CRON_PATTERN_REJECTED`); which predicate first rejected the result is read only internally to select one and never leaves the sentinel. `decodeSchedules()`'s accepted behaviour is not modified: a new pure `classifySchedulesPayload()` is the single shared definition of its predicates, and `decodeSchedules()` is derived from it, proven by a permanent parity test to never disagree. The other six `/schedules` categories, and `/deployments`/`/settings`, are unchanged. No permission widening, no fourth read, no decoder-behaviour change, no credential rotation, no Cloudflare/D1 mutation, no Cron change and no schedule activation occur. See [A1.3](DATA-OPS-A1-3-LIVE-READONLY-OBSERVER.md).
