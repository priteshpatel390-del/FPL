...
## API-Football attended team-universe workflow remediation candidate

Owner-approved repository-only remediation follows the 18 September pre-execution stop. The historical EIA-2I5E R7 workflow remains historical and still owns the old 11-request response-size qualification; it is not reused for the new Premier League team-universe observation. This candidate adds a separate manual-only workflow that executes exact current `main` and invokes only `runAttendedApiFootballTeamUniverseQualification()` under protected environment `eia-api-football-qualification`, with one secret-bearing step, immutable action SHAs, `run_attempt == 1`, repeated remote-`main` equality checks, an independent maximum of two fetches, focused pre-secret tests and sanitized evidence retention. No API-Football provider request or secret access occurs in this remediation, and no mapping, collector, migration 0005, Cron, D1, model or UI authority changes. Current mapping remains NO-GO 2/20 and response ceiling remains 720,896 bytes. An active owner-configured ruleset protecting `refs/heads/main` and current environment protection verification remain mandatory before a later attended dispatch. No new EIA identifier is assigned.
