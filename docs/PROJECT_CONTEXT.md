...
## Current Data-Ops checkpoint — A1.3 schedules-read failure classification (draft, unmerged)

PR #234's fixed-read diagnostic remediation merged: a third attended dispatch, run `34311398342`, head `465e54260005c96591bd77be0a1fe1cb44547631`, proved identity admission continues to succeed and narrowed the failure to the first fixed read, `/schedules` (GitHub/D1 sentinels observed successfully, D1 `rowsRead: 89066`; `/deployments` and `/settings` were not attempted, per the fail-closed sequential order). The per-stage code `CLOUDFLARE_SCHEDULES_READ_FAILED` was itself still a collapsed code covering a transport error, an authorization refusal, a missing-resource response, any other non-200, or a successful-but-unusable response. This draft, unmerged remediation replaces it, for `/schedules` only, with five closed categories (`CLOUDFLARE_SCHEDULES_AUTH_REFUSED`, `CLOUDFLARE_SCHEDULES_NOT_FOUND`, `CLOUDFLARE_SCHEDULES_HTTP_FAILED`, `CLOUDFLARE_SCHEDULES_RESPONSE_INVALID`, `CLOUDFLARE_SCHEDULES_TRANSPORT_FAILED`); the HTTP status is read only internally to select one and never leaves the sentinel. `/deployments` and `/settings` keep their single collapsed codes unchanged. No permission widening, no fourth read, no decoder change, no credential rotation, no Cloudflare/D1 mutation, no Cron change and no schedule activation occur. See [A1.3](DATA-OPS-A1-3-LIVE-READONLY-OBSERVER.md).
