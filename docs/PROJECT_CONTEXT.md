## API-Football migration 0004 production application foundation — repository candidate

Draft PR #262 prepares a dedicated owner-gated path for exactly migration 0004 without applying it. Baseline main is `22c58f87571393422f38f24615eae1b8cc01da32`; Verify run `35427541775` passed 2,203/2,203 tests with deterministic production rebuilds, and attended live-storage preflight `35427688308` proved `STOP_0004_NOT_APPLIED`, exact ledger 0001-0003, zero FK violations, fresh 20-team Official FPL authority and no API-Football runtime/collector state. The candidate adds exact-main/manual-only admission, the shared production-writer lock, an explicit hard stop if the historical direct data-platform Cron has reappeared, a pinned 40-statement transaction, D1 Time Travel checkpoint/recovery, strong history/schema/Official-FPL postflight and a final read-only preflight rerun. No production write, migration 0005/0006, mapping persistence, API-Football request/secret/collector/Cron, model or UI change occurs in this checkpoint. GitHub is only the administrative one-off migration runner here: the earlier `eia-api-football-qualification` API key was temporary attended qualification infrastructure, while any future collector credential remains a Cloudflare Worker secret under the existing security decision. See [migration 0004 production application foundation](API-FOOTBALL-MIGRATION-0004-PRODUCTION-APPLICATION.md).
