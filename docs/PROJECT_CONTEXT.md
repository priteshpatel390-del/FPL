...
> **Teamsheet 2.0.5 current status:** the owner-approved Mini-League Intelligence scope is implemented on `agent/teamsheet-2.0.5-mini-league-intelligence` for review and is not merged. It extends the merged 2.0.4 foundation with an ID-free on-demand selected-rival exposure view, a maximum-five explicit comparison group, concurrency two, exact complete/fresh selected-rival player/captain/vice/chip counts, version-2 local choice state and validation of the optional Official FPL fields used by aggregation. It adds no provider, model, rank prediction, rival-score prediction, remaining-player simulation, effective-ownership strategy or protect/chase logic. The branch verification target is 510/510 tests with deterministic exact-identity builds and root/deployable equality. Physical iPhone testing, VoiceOver acceptance and live populated-data acceptance have not been performed. After review and merge, Teamsheet 2.0.6 is next.
Purpose: master overview — the first document any new developer or session reads after CLAUDE.md.
Audience: anyone joining the project. Last updated: 2026-08-02. Related: ARCHITECTURE.md, ROADMAP.md, DECISIONS.md, TEAMSHEET2-PRODUCT-BLUEPRINT.md.
...
- Teamsheet 2.0.5 is owner-approved and implemented on the review branch, not merged: explicit maximum-five selected rivals, on-demand current-Gameweek public picks, concurrency two, exact complete/fresh selected-rival exposure, stale/incomplete/unavailable separation, ID-free `#/leagues/exposure`, version-2 minimal persistence and stricter optional-field validation. The verification target is **510/510 tests** with deterministic exact-identity builds. Projected or strategic Mini-League intelligence remains excluded.
...
Current development item: **Teamsheet 2.0.5 — Mini-League Intelligence** is implemented on the owner-approved review branch and remains unmerged. The implementation is limited to selected-rival factual exposure and supporting validation/state/UI/tests. Projected rank, projected rival scores, remaining-player simulation, effective-ownership strategy, protect/chase strategy and recommendation changes remain unapproved. After review and merge, **Teamsheet 2.0.6 — Research, Evidence and Diagnostics Organisation** is next.
...
Teamsheet 2.0.4 is complete and merged. Teamsheet 2.0.5 is explicitly approved and implemented on its review branch, but is not merged. It changes League presentation, versioned choice state and validation of already-used Official FPL fields only, while preserving projection, fixture, captaincy, squad, transfer, simulation, rank-prediction and strategy calculations. The verification target is **510/510 tests** with deterministic exact-identity builds. Prospective validation remains in progress, and physical iPhone, VoiceOver and live populated-data acceptance remain unperformed.
