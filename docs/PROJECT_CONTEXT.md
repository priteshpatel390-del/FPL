...
> **Teamsheet 2.0.5 merged baseline:** Mini-League Intelligence is complete and merged through PR #63 at `0b04dd68194207d301667a7100c3ed804ec1e056`. It extends the merged 2.0.4 foundation with an ID-free on-demand selected-rival exposure view, a maximum-five explicit comparison group, concurrency two, exact complete/fresh selected-rival player/captain/vice/chip counts, version-2 local choice state and validation of the optional Official FPL fields used by aggregation. It adds no provider, model, rank prediction, rival-score prediction, remaining-player simulation, effective-ownership strategy or protect/chase logic. Verified baseline: **510 passed, 0 failed, 0 skipped**, deterministic exact-identity production builds and root `index.html` equal to `dist/index.html`; model version remains `2.4.0` and rules version remains `2026-27.3`. Physical iPhone testing of the actual repository build, VoiceOver acceptance and live populated-data acceptance were not performed. Teamsheet 2.0.6 investigation and design is next; implementation requires explicit owner approval.
Purpose: master overview — the first document any new developer or session reads after CLAUDE.md.
Audience: anyone joining the project. Last updated: 2026-08-03. Related: ARCHITECTURE.md, ROADMAP.md, DECISIONS.md, TEAMSHEET2-PRODUCT-BLUEPRINT.md.
...
Current development item: **Teamsheet 2.0.6 — Research, Evidence and Diagnostics Organisation**. Investigation and design may begin from the merged 2.0.5 baseline. Implementation remains unapproved and must not change calculations, providers or data sources.
...
Teamsheet 2.0.5 is complete and merged through PR #63 at `0b04dd68194207d301667a7100c3ed804ec1e056`. It changes League presentation, versioned choice state and validation of already-used Official FPL fields only, while preserving projection, fixture, captaincy, squad, transfer, simulation, rank-prediction and strategy calculations. The verified baseline is **510 passed, 0 failed, 0 skipped** with deterministic exact-identity builds and root/deployable equality. Prospective validation remains in progress, and physical iPhone, VoiceOver and live populated-data acceptance remain unperformed. Teamsheet 2.0.6 investigation and design is the next gate; implementation requires explicit owner approval.
