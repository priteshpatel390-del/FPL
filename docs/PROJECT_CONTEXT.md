...
A dedicated manual, protected, standalone read-only workflow prepares the fresh production-state evidence required before any future Phase 4B Version-upload approval. Two dispatches have failed closed at the same credential-presence gate, each before its first request: run `33170157089` under the Phase 0 environment, and run `33171701995` under the Phase 2 upload environment selected from repository inference. That live result disproved the inference that the Phase 2 environment supplied the Worker bearer and Access service credentials; in both runs those three variables were empty, and neither run performed any Cloudflare read, D1 query, production health request or mutation. The workflow now uses the Phase 3 deployment environment, whose successful run `33142804502` showed all five required credentials populated and masked, and maps its existing deploy-token secret into the helper's unchanged read-token variable. The executable is unchanged: it remains limited to exact Worker/D1 reads and validated D1 `SELECT` transport and has no upload, Deployment, Cron, collector or D1-write path. No successful preflight is established and no current live state is claimed as healthy or reconciled; a dispatch and every later mutation remain separately owner-approved.
