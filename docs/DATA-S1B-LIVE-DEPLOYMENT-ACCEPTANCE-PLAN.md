...
- Protect the single Worker with Worker-level Cloudflare Access set to **All traffic**. Current first-party Cloudflare guidance says this covers that Worker's routes, Custom Domains, `workers.dev` hostname and previews. Use one dedicated DATA-S1 service-token-only allow policy, with no human-identity allow policy, no shared evidence token and no permanent browser credential. `/v1/health` remains protected. The documented Worker-level WebSocket limitation is irrelevant because DATA-S1 has no WebSocket path.
...
4. Re-run inventory and prove that exactly one matching D1 exists and the reviewed config resolves to its UUID. A missing or mismatched UUID is a hard stop.
5. Use an approved Wrangler version's documented dry-run/config inspection. If that version offers a specific disable-auto-provision switch, require it. Current first-party configuration guidance documents the trigger (missing resource IDs), but no general disable flag; the primary fail-closed control is therefore the present, exact UUID plus pre/post inventory. Never answer an interactive resource-creation prompt affirmatively.
