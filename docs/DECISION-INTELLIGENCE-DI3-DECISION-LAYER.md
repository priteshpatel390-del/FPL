...
The direction is strictly production output → adapter → artifact. Model, squad, optimiser and provider modules do not consume the runtime or artifact. A complete artifact is formed only when Team and Transfers bases exactly agree on season, event/Gameweek, deadline/cutoff, source/model/rules lineage, squad, bank, free transfers and price basis; stale or conflicting snapshots fail closed. Partial, no-artifact and validation-failure results cannot suppress or mutate the prior recommendation. Permanent executable tests compare the production fixture output directly with artifact actions and consequences for every integrated domain, mandatory roll, optimiser selection, hits, bank and FT. The 14 synthetic descriptors remain Stage-A contract coverage rather than claimed Stage-B executions. This is representation evidence only, not improved accuracy.
