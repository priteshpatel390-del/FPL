...
- [GW1 readiness safety guard](GW1-READINESS-SAFETY-GUARD.md) — the narrow implementation checkpoint after the GW1 readiness audit's *go with conditions* outcome. Before the first Official FPL deadline of a season the Transfers screen suppresses the weekly free-transfer/hit optimiser and states that initial squad changes are unlimited; the rule is derived from verified Official FPL event data, normal weekly behaviour resumes at the deadline instant, and no optimiser mathematics changes. It also carries the surgical operator and current-status documentation corrections.
