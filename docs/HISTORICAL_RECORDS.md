...
- [GW1-P2 Browser evidence delivery and durable outbox](GW1-P2-BROWSER-EVIDENCE-DELIVERY.md) — connects the existing Stage 10 browser capture path to the GW1-P1 archive through a durable local outbox, a transport-independent delivery state machine, content-hash idempotency and fail-closed provider retention. The direct credentialled cross-origin transport is approved as a feasibility implementation only; physical iPhone Safari acceptance under normal privacy settings and live Cloudflare Access CORS confirmation remain outstanding.
- [GW1-P1 Cloudflare Evidence Foundation](GW1-P1-CLOUDFLARE-EVIDENCE-FOUNDATION.md) — approved backend-only predeadline evidence-custody foundation: separate authenticated Worker, private R2-first archive, minimal D1 manifest/receipt layer, fail-closed optional-provider retention gates and recovery/reconciliation contract; live backend deployment acceptance passed on 11 August 2026, while Teamsheet client integration remains the separate GW1-P2 checkpoint.
