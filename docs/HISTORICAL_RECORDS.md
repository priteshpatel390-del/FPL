...
- [API-Football Collector Activation Read-Only Preflight](API-FOOTBALL-COLLECTOR-ACTIVATION-READONLY-PREFLIGHT.md) — manual exact-main activation-specific production observation using only the existing read-only steward credential; targets repository/infrastructure-staging readiness only and authorizes no Worker upload/deployment, secret, runtime, Cron or provider action.
- [API-Football Migration 0006 and Private Mapping Production Foundation](API-FOOTBALL-MIGRATION-0006-PRODUCTION-FOUNDATION.md) — dormant manual two-phase schema/private-mapping foundation with fresh admission, parameterized private values, one-shot reconciliation and sanitized evidence; no production execution occurred.
- [API-Football Migration 0005 Production Application Foundation](API-FOOTBALL-MIGRATION-0005-PRODUCTION-APPLICATION.md) — repository-only manual production migration foundation with exact-byte admission, shared writer serialization, daily D1 write-headroom protection, one-shot reconciliation and no automatic Time Travel restore; no production dispatch or D1 mutation occurs in this checkpoint.
- [API-Football Migration 0004 Production Application Foundation](API-FOOTBALL-MIGRATION-0004-PRODUCTION-APPLICATION.md) — repository-only manual production migration foundation with exact-main admission, shared writer serialization, Time Travel recovery, post-write reconciliation and explicit stop after 0004; no production dispatch or D1 mutation occurred in this checkpoint.
...
Purpose: make the repository's operational guidance and immutable project history easy to distinguish without deleting, moving or rewriting historical evidence. Last reconciled: 22 September 2026.
...
- [DATA-OPS A1.4 Persistent Incident Lifecycle + Independent Watchdog](DATA-OPS-A1-4-WATCHDOG-LIFECYCLE.md) — isolated Cloudflare Worker reading A1.3's own GitHub Actions history, a replay-safe NEW/ONGOING/CHANGED/RECOVERED/REOPENED incident lifecycle, its own three-table D1 database, and bounded, deduplicated owner email notification; repository candidate only, with no deployment, credential, D1 database or live activation.
- [DATA-OPS A1.4 Live Acceptance Closeout](DATA-OPS-A1-4-LIVE-ACCEPTANCE-CLOSEOUT.md) — records the first complete genuine 14 September 2026 positive healthy automatic chain from DATA-S2 through A1.3 receipt-backed observation to A1.4 watchdog classification; live-accepts only that positive path, preserves negative-branch and delivery limitations, and grants no A1.5, repair, provider, model, scheduler or credential expansion.
