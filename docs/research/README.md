...
| [Availability / Team News](05-AVAILABILITY/RESEARCH-AVAILABILITY.md) | §§3.4, 7–8, 14 | Which factual injury/suspension/team-news observations are reliable, fresh and rights-safe? | Research complete | Factual availability is the first prospective expected-minutes evidence hypothesis; current production already uses Official FPL `starts`, and Sportmonks is the strongest currently researched structured factual candidate subject to fresh approval-time verification | Incremental value is unproven; exact Sportmonks plan/price/PL entitlement and approval-time terms remain to be verified | One rights-cleared factual stream linked to exact Stage 10 baseline, prospective timing/provenance, outcomes and untouched later evaluation | Separate Prospective Factual Availability Collection Approval — Minimum Shadow Protocol | Not approved |
...
The next possible decision checkpoint is **Prospective Factual Availability Collection Approval — Minimum Shadow Protocol**. It must present the exact source, fields, account/cost, acquisition cadence, credentials, storage, retention/deletion, rights classification, fallback/failure behaviour and structural zero-production-effect tests before any collection code is written.
