...
| [Non-PL Player Workload](04-CALENDAR-WORKLOAD/RESEARCH-NON-PL-PLAYER-WORKLOAD.md) | §§3.4–3.5, 14 | Can non-PL starts/minutes/substitutions/extra time improve expected-minutes evidence? | Research complete | Completed-match workload is factual and mostly reconstructible; it is a medium/high-value research class but lower priority than factual availability and predicted line-ups, and no fatigue penalty/provider is approved | Detailed provider rights/retention and exact competition coverage remain unverified; incremental signal is unproven | Trustworthy identity/provenance, valid historical or prospective workload, predeclared paired ablation and untouched future validation | Separate provider/shadow/model proposal after rights review and evidence protocol | Not approved |
| [Availability / Team News](05-AVAILABILITY/RESEARCH-AVAILABILITY.md) | §§3.4, 7–8, 14 | Which factual injury/suspension/team-news observations are reliable, fresh and rights-safe? | Research complete | Factual availability is the highest-priority additional expected-minutes evidence class; Official FPL remains production baseline and facts/interpretations/predictions must remain separate | Exact rights/retention for candidate sources, mutable point-in-time capture and incremental value are unresolved | Prospective frozen factual evidence, source reliability/freshness/conflict results, lawful retention and untouched expected-minutes evaluation | Separate expected-minutes evidence protocol/provider proposal | Not approved |
| [Predicted Line-ups / Start Probability](05-AVAILABILITY/RESEARCH-PREDICTED-LINEUPS.md) | §§3.4, 11–12, 14 | Can lineup predictions add calibrated evidence for `pStart`, `pAppear`, `p60`, `expMin`? | Research complete | Predicted line-ups are promising tactical/rotation evidence but require prospective point-in-time freezing; FFS is the clearest verified research candidate and a source prediction must never become direct `pStart = 1/0` | No independent accuracy evidence; access/licence/retention and exact revision preservation must be verified | Lawful frozen pre-deadline predictions, source-specific reliability/calibration, paired shadow ablation and untouched future validation | Separate expected-minutes evidence protocol/provider proposal | Not approved |
...
### Expected-minutes evidence research conclusion — 13 August 2026

The completed Availability, Predicted Line-ups and Non-PL Player Workload branches establish the research priority **factual availability first -> predicted line-ups second -> non-PL workload third**. This ordering reflects likely independent information, error asymmetry, temporal provenance and evaluation feasibility; it is not a model-weight decision.

The investigation also identified a current-model evidence/design issue: production `pStart` is inferred from a >=45-minute recent-minutes proxy despite detailed FPL history containing a `starts` field. This finding is recorded for later evaluation only. It does **not** approve a formula change during the production freeze.

No new essential pre-GW1 evidence-preservation requirement was found. Mutable predicted-lineup/team-news states would require prospective freezing for faithful evaluation, but missing GW1 specifically would not make later scientific evaluation impossible. Completed non-PL workload is generally reconstructible. The completed GW1 Evidence Readiness conclusion therefore remains unchanged.

The recommended next decision checkpoint is an **Expected Minutes Prospective Evidence Protocol**: design/research only, defining outcome labels, temporal provenance, a pre-registered baseline-versus-candidate analysis plan and a small shortlist for first-party terms/licensing verification before any collection code is considered. That checkpoint is not approved by this closeout.
