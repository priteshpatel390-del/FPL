...
## EIA-2I5B storage boundary

EIA-2I5B produces only in-memory sanitized discovery results and run audits. It does not create migration 0005, change live D1 schema, write discovered fixtures, or add request-lifecycle, quota, scheduler, queue, correction-state or collector-state tables. Durable provider persistence remains a later separately approved checkpoint.

## EIA-2I5A additive shadow schema

Migration 0004 extends canonical `data_source_revisions` with tightly checked API-Football private-use fields and source-consistency triggers while preserving every historical rights class. Because D1 keeps foreign keys enabled, it uses transaction-scoped deferred enforcement and rebuilds the full connected revision/mapping/run/observation/relation/head/rejection graph, dropping children before parents and renaming replacements parent-first. Wrangler local D1/Miniflare verifies populated dependent data and zero foreign-key violations after migration. It adds stable provider fixture identities and append-only participation revisions, including enforceable substitution-evidence completeness for `NOT_USED`. It omits request, quota and scheduler state; EIA-2I5B keeps that omission and adds only an in-memory sanitized audit. Optional canonical player mapping permits later historical linking without refetch. No parallel rights store, second datastore or production read path is introduced.
