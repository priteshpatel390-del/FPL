...
Before extraction, both adapters invoke the same pure canonical integrity validators used by the Stage 10 snapshot and outcome subsystems. `src/evidence/integrity.mjs` recomputes snapshot section/rule/timing/content hashes and snapshot ID, and outcome section/data/content hashes and outcome ID. Stage 10 delegates its existing validation entry points to this shared implementation; capture and finalisation remain unchanged. Hash-shaped or mutually self-consistent forged metadata cannot authorize mutated evidence.
