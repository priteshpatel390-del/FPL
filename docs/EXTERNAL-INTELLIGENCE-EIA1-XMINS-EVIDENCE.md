...
The DATA-S2B adapter consumes the existing `/v1/shadow/observations` export shape. It permits only `official-fpl-r1` and the DATA-S2A event/team/player/fixture category/metric allowlist, requires page `as_of` equality, preserves complete observation rows, and orders by `fetched_at, observation_id`. Manager/account fields fail closed. No Worker, endpoint, D1 table, collector, Cron, Wrangler, Cloudflare or authentication change was needed.
