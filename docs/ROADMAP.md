...
- **Stage 3 items 1–6** · Fixture deduplication, Anthropic-key removal, per-endpoint schema validation,
  bounded retry, Provider Health, DOM-builder rendering and AI/Markdown sanitisation are complete.
  Stage 3.6 is verified at 202 passing tests with deterministic builds.

## Current
- **Stage 3 — Security & provider hardening** · IN PROGRESS under the approved
  docs/STAGE3-DESIGN.md.
- **Stage 3 remaining work** · Remaining odds-key hygiene and hash-based CSP. The existing
  documentation records a “Forget API key” action; remaining key work must be scoped against the
  actual implementation rather than re-implementing it blindly.

## Upcoming (order fixed; later stages depend on earlier)
- **Stage 3 completion** · Remaining odds-key hygiene and hash-based CSP.
...
## Current blockers
1. Remaining Stage 3 key-hygiene and CSP scope/design approval.
