...
Production run `35496920942` failed closed before credential-bearing preflight or mutation: 27/27 focused tests passed, then the repository-gate cleanliness check failed. Migration 0005 remains unapplied. Current approved work is repository-only remediation to isolate the local Wrangler/D1 migration harness from the checkout and prove no git-status delta. After merge and exact-main verification, the next step would still require a fresh owner decision for a new attended migration-0005 dispatch; do not use GitHub rerun on the failed run. Migration 0006, private mapping persistence, collector deployment/secret/Cron/provider execution and model/product influence remain later gates.
