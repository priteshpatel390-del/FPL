...
## Current next gate — verify checkout-isolation remediation before any new 0005 dispatch

Production run `35496920942` failed closed before credential-bearing preflight or mutation: 27/27 focused tests passed, then the repository-gate cleanliness check failed. Migration 0005 remains unapplied. Current approved work is repository-only remediation to isolate the local Wrangler/D1 migration harness from the checkout and prove no git-status delta. Candidate Verify Teamsheet run `35497159414` on head `158da28c0884c48f72931d798da389dc924611c7` passed 2,225/2,225 tests plus deterministic exact-identity builds. After merge and exact-main verification, the next step would still require a fresh owner decision for a new attended migration-0005 dispatch; do not use GitHub rerun on the failed run. Migration 0006, private mapping persistence, collector deployment/secret/Cron/provider execution and model/product influence remain later gates.

## Current next gate — review migration 0005 production application foundation

Migration 0004 is live. Owner-approved repository implementation now prepares a **dormant migration-0005 production application foundation only**: exact migration bytes/shape, manual exact-main workflow, shared production-writer serialization, mutation-free pre/post admission, conservative account-wide D1 write-budget protection, one-shot transactional application and no automatic Time Travel restore. Baseline exact-main Verify `35468057507` passed 2,214/2,214 tests and Pages `35468056858` succeeded; the 20 September 01:17 UTC Official FPL external collection `35481020246` also succeeded. Candidate-head full verification remains required. Do not dispatch 0005 merely because this foundation exists. After merge and exact-main verification, the next separate owner gate is one attended migration-0005 production application preceded by provisioning/verification of the separately scoped Account Analytics Read token, fresh read-only state and write-budget evidence. Migration 0006, private mapping persistence, collector deployment/secret/Cron/provider requests and model/product influence remain later gates. See [migration 0005 application foundation](API-FOOTBALL-MIGRATION-0005-PRODUCTION-APPLICATION.md).

## Current next gate — migration 0005 production application design/review

Migration 0004 is live and accepted. PR #263 merged at `bc87334dbdebce4e19d92374e6640a9fed0f44fa`; exact-main Verify/Pages passed, then attended production run `35467787882` applied exactly 0004 with no recovery and independent postflight `READY_FOR_MIGRATION_0005`. The next step is **investigation/design and owner review for migration 0005 production application only**. Do not apply 0005 yet. Migration 0006, private 20/20 mapping persistence, API-Football collector deployment, Worker secret provisioning, Cron activation, provider requests and all model/product influence remain later separate gates. See [migration 0004 application](API-FOOTBALL-MIGRATION-0004-PRODUCTION-APPLICATION.md).

## API-Football live storage foundation — current gate

Current work is the **read-only production preflight** for the approved live storage foundation. PR #260 prepares a manual exact-main Cloudflare/D1 observation using the existing read-only steward credential. After merge and exact-main verification, one attended dispatch must establish the real migration ledger, API-Football row/runtime state, production binding and collector Worker/Cron/secret state before any mutation. If migration 0004 is absent, stop and return for a separate owner decision; do not apply 0005 out of order or silently broaden scope. If 0004 is proven and no other hard stop exists, the next separately reviewed mutation step may be migration 0005 only. Migration 0006 and private mapping persistence remain subsequent ordered steps inside the same owner-approved storage foundation but are not executed by this preflight. See [API-Football Live Storage Foundation](API-FOOTBALL-LIVE-STORAGE-FOUNDATION.md).
