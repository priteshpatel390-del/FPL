...
Two final closeout items own the current work: canonical documentation reconciliation and Preview/version URL security. The repository candidate now explicitly disables Cloudflare Preview URLs; the hardened config must be deployed and the live Worker must show Preview URLs disabled before that security item is accepted. Exact final-head repository verification is also mandatory after the documentation/config changes.
