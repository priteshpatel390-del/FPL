## Current next gate — migration 0004 workflow-context remediation

PR #262 is merged at `3892f2c6448d951538cf318ad28392ca95ca2449` and exact-main Verify/Pages both passed. Its manual migration workflow is not yet dispatch-ready because GitHub rejected the definition before job creation: `runner.temp` was placed in job-level `env`. A narrow repository follow-up moves that one report-path expression into step scope and permanently tests the context boundary; corrected branch pushes no longer create the zero-job failure. Migration 0004 remains unapplied. After this follow-up is reviewed, merged and exact-main verified, the **next separate gate is still owner approval to dispatch migration 0004**. Nothing in this remediation authorizes 0005, 0006, private mapping persistence, API-Football collector/secret/Cron activation, provider requests or model/product influence. See [migration 0004 application](API-FOOTBALL-MIGRATION-0004-PRODUCTION-APPLICATION.md).
