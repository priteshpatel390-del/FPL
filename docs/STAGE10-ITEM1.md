...
- Compact global status and phone-first More > Deadline evidence controls.
- Optional providers are awaited only for an explicit evidence capture; ordinary app loading remains unchanged.
...
## Verification
- Verified source `__SOURCE_SHA__`: full `./run-tests.sh` **337/337 passing**.
