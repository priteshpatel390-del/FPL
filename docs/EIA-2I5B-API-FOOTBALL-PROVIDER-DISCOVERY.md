...
Provider fixture identity remains `<season>:api-football:fixture:<providerFixtureId>` and is independent of kickoff, venue, round, score, status or names. Home/away orientation is preserved. Qualification reuses the EIA-2I5A contract: a fixture is workload-relevant only when at least one oriented team passes the complete current-season `VERIFIED` API-Football → Official FPL mapping validation. Names, fuzzy matching and kickoff proximity cannot map. Contradictory verified mappings remain `CONFLICTED`. Non-PL opponents keep provider-scoped identity. Chelsea–Leeds fixture `1636205` keeps stable identity while 19:00 UTC versus 19:15 UTC remains an explicit kickoff conflict with no time tolerance; canonical kickoff may stay null.

Complete current-season 20-club mapping coverage is **not** claimed and is a pre-live limitation/gate. Mapping incompleteness is reported honestly.
