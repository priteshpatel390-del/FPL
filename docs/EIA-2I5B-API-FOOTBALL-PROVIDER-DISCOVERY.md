...
Complete current-season 20-club mapping coverage is **not** claimed and is a pre-live limitation/gate. Coverage is true only when mappings pass EIA-2I5A `validateProviderMapping()` **and** form an exact one-to-one bijection with the current-season Official FPL club universe established by a provenance-bearing Official FPL snapshot. That snapshot must carry Official FPL source identity (`sourceKey: official-fpl`, `sourceKind: official_fpl`), the canonical DATA-S2A revision (`official-fpl-r1`), schema/validation/transform versions, the requested season, a valid `fetchedAt`, and exactly 20 unique bootstrap-shaped team rows (`id`, `name`, `short_name`). A caller-supplied array of 20 IDs, `{id}` rows, or any other unlabeled set cannot self-certify. Wrong season, missing/malformed revision or timestamp, 19/21/duplicate/malformed teams, a `VERIFIED` label, extra non-authoritative targets, provider→multiple-FPL, multiple-provider→same-FPL, missing mapping provenance, invalid revision, non-approved method, or non-API-Football source all fail closed.
