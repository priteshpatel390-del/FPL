...
Reason: retrospective archives cannot prove what the production app knew before a live deadline, particularly for optional providers, expected minutes and uncertainty. Approach: versioned immutable pre-deadline JSON records use the official FPL deadline, two same-origin network-clock observations, a 60-second clock-skew limit and a two-minute safety cutoff. Complete network-attested records alone can become official. Records freeze allowlisted model inputs/outputs, provider consequences and available decisions; FPL Team ID, manager name, league identifiers, configuration and secrets are excluded. Whole-record and section/provider SHA-256 hashes make tampering detectable. Consequences: local storage is only a bounded compressed recovery mirror and exported JSON is the owner-controlled durable copy. GitHub Pages does not provide external timestamp notarisation. Outcomes and metrics remain later checkpoints; no formula or accuracy claim changes.

**D-24 · 2026-07-29 · Accepted · Preserve 5,000 uncertainty samples through invariant-component reuse (Stage 10.1)**
Reason: the first 620-player evidence benchmark exceeded three minutes because deterministic fixture components were recalculated inside every Monte Carlo sample. Reducing sample count would violate the approved evidence design. Approach: calculate each player/fixture component expectation once, pass it into the unchanged sample loop, omit raw sample arrays only for summary-only evidence callers, and yield projection work in browser-sized batches. Consequences: seed, distribution, sample count, scoring and summaries remain unchanged; the synthetic 620-player benchmark completes in roughly 2.8–3.5 seconds in the repository environment. Physical iPhone performance remains an explicit acceptance gate.
...
Reason: the owner should enter Teamsheet with one complete verified dataset and should not manually refresh, verify or import routine football data. Approach: a minimalist startup gate runs the existing validated provider stack, waits for every approved source to resolve, suppresses intermediate renders and releases one final consistent state. The same cycle runs after a ten-minute foreground gap. Runtime provider names are closed to Official FPL, Understat, The Odds API and the versioned Teamsheet archive. Eligible deadline evidence is captured automatically after verification; progressively better deadline windows can replace earlier automatic records. JSON restore remains available only under recovery controls and is forced non-official. Consequences: Official FPL failure without an acceptable cache disables recommendations; optional sources may use their existing verified fallback/disabled states. The app cannot refresh while fully closed on static GitHub Pages. No provider endpoint, formula, blend weight, retry budget, expected-minutes rule, captaincy rule, squad rule or optimiser objective changes.

**D-26 · 2026-07-29 · Accepted · Official FPL-only immutable outcome collection (Stage 10.2)**
Reason: prospective evaluation needs the realised player, fixture and squad facts corresponding to each frozen deadline snapshot without introducing another provider or allowing post-match data to alter predictions. Approach: `/event/{gw}/live/`, filtered fixtures and official event completion are authoritative globally; optional public picks/history provide squad outcomes. `finished` alone is insufficient: finalisation also requires official `data_checked`. Valid in-flight data is provisional; later complete changes append corrected revisions. Duplicate players and conflicting fixtures fail closed. Automatic checks run after app access, catch up six Gameweeks at a time and cannot run while the static app is fully closed. Consequences: Stage 10.2 collects facts only, imports remain recovery-only, Team ID is request-only, and no Stage 10.3 metric or model behaviour changes.
