...
Consequences: no projection, minutes, scoring, fixture, captaincy, squad, affordability, hit, free-transfer, roll-value or ordering change; no candidate filter of any kind; no ceiling, horizon, depth or Top-8 change; fail-closed behaviour is unchanged. A Node reproduction of the failure shape that previously did not finish after more than ten minutes now completes in about 1.2 seconds with `status: 'ok'`, and equality with the independent oracle is proved on controlled pools across seven adversarial shapes. Reduced evaluation counts are a search-efficiency result only and create no prediction-accuracy claim. The corrected search has not yet been physically retested on iPhone Safari, so Track A remains unaccepted and unapproved for merge.
