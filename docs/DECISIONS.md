## Decision — first collector Version is staged through an inert shell and explicit bindings

For the first live collector Version, Teamsheet does not reuse DATA-S2B binding inheritance. The Worker does not yet exist, so the staging sequence is an inert Beta Worker object with `subdomain.enabled=false` and `subdomain.previews_enabled=false`, followed only after readback by the stable non-deploying Version Upload API. That first Version explicitly carries the production `TEAMSHEET_DATA_DB` binding, exactly three reviewed plain-text variables and zero secret bindings. The upload source is the exact closed 19-module repository graph rather than a rewritten/bundled copy. Definite 4xx rejection is terminal; uncertain mutation transport is reconciled read-only and never blindly retried. No automatic cleanup is permitted.
...
**Decision:** treat attended read-only activation run `35631979158` as a consumed safe STOP, not as evidence that the committed 20/20 mapping is absent. The run proved every mapping cardinality predicate but failed because activation preflight v2 compared Migration 0006's immutable historical authority-provenance hash directly with the latest Official FPL authority digest. Replace that invalid equality with two independent requirements: (1) historical provenance remains a valid persisted hash/timestamp; and (2) the committed mapping's 20 canonical FPL team IDs exactly equal the latest fresh Official FPL authority's 20 canonical IDs. Preserve exact 20/20 bijection, current-head/COMMITTED state, zero private provider-ID disclosure and all other read-only gates. No production state is changed by this correction. A new live preflight requires a fresh owner approval after merge and exact-main verification.
...
**Decision:** accept owner-attended persistence run `35607354636` on exact SHA `051a68841fd04dde8f5f239dadbfb285231646c3` plus final independent reconciliation run `35609344995` on exact merged main `8c5c7fbf80b70961f2e39d8acc73c806d8c95659` as the completed Migration 0006 private-mapping production closeout. Persistence returned `DEFINITELY_APPLIED_SUCCESSFULLY`, HTTP 200, 43/43 successful D1 statements and durable 20 entity mappings / 1 qualification / 20 members / 1 head with complete 20-club coverage and zero API-Football requests. The first post-write reconciliation `35607762989` was rejected as acceptance evidence because repository code incorrectly required the fresh current qualification hash to equal the historical approval hash; PR #276 corrected that comparison without changing production state. Final reconciliation returned `COMPLETE_QUALIFIED_MAPPING_VISIBLE`, 24,536 rows read, 0 production mutations and 0 API-Football requests. Artifact `10643366243` is bound by SHA-256 `15cead166619fbf07a99c6f7aa5a09b5416080f1c0d122e56b33771fa7b4c2c8`.

**Boundary:** no additional mapping write, repair, retry or restore is required. Earlier mutation-submitting runs `35575463178` and `35587588095` remain consumed and forbidden from rerun. This acceptance does not deploy/bind/credential the collector, enable Cron, activate `PRELIVE_PLANNER_ONLY`, make provider requests, ingest workload evidence, or change any model/calculation/product path. Those remain later explicit owner gates.

## 21 September 2026 — remediate Migration 0006 deterministic writer failure without widening recovery authority

**Decision:** Correct the proven qualification-insert placeholder mismatch in the existing 43-statement private-mapping writer and keep the existing workflow/security boundary. Add only closed allowlisted D1 outcome metadata and a comprehensive aggregate reread including relevant `entity_mappings`. An explicit D1 rejection may become `DEFINITELY_NOT_APPLIED` only when the complete reread proves exact empty state; transport uncertainty, malformed/lost response, partial state and reread uncertainty remain `AMBIGUOUS_REQUIRES_OWNER_ATTENTION`. Preserve no retry and no automatic restore. Replace the obsolete hard-coded first-attempt persistence hash with strict dynamic-hash format, qualification-ID binding, fixed provenance, member relationship and exact-cardinality checks. This repository work authorizes no production action; runs `35575463178` and `35587588095` remain consumed and must not be rerun.
...
## 20 September 2026 — isolate local migration harness before retrying migration 0005

**Decision:** treat run `35496920942` as a safe pre-production failure, not a migration attempt that reached production state. The run passed 27/27 focused tests but failed the checkout-cleanliness guard, and GitHub skipped preflight, migration0005 and postflight. The log proves only that the checkout changed; it does not identify the path, so no exact artefact is asserted. Remediate by running Wrangler/D1 local-migration commands from the harness-owned OS temp directory while resolving repository migration files to absolute paths, and permanently test that the harness leaves git status unchanged. Do not weaken or remove the cleanliness gate. Any later production dispatch requires a new owner approval and a new run attempt rather than rerunning `35496920942`.

## 20 September 2026 — migration 0005 production application foundation may be implemented, not executed

**Decision:** implement a repository-only, dormant production application foundation for exactly migration 0005. Preserve the exact-current-main/Verify gate, shared `data-s2-production-collection` writer lock, read-only live-storage admission, historical direct-Cron hard stop, exact migration byte/statement pinning, one-shot mutation and exact post-state reconciliation. Add a conservative current-UTC-day account-wide D1 write-usage admission gate because migration 0004 consumed 99,713 writes and exposed the Free-plan 100,000-row daily limit. The GraphQL analytics read must use a distinct future Account Analytics Read-only token; this repository approval neither provisions that credential nor widens the existing Workers/D1 read token. Capture a pre-mutation Time Travel bookmark, but **do not automatically restore** for 0005: the migration is additive and transactional, and any contradictory/partial post-state requires attended owner reconciliation rather than an unexercised whole-database destructive restore. This approval does not authorize workflow dispatch, production migration, 0006, private mapping persistence, collector deployment/binding/secret/Cron, provider egress, workload ingestion, model/calculation or product/UI change. After merge and exact-main verification, a fresh read-only production observation plus explicit owner approval is required before any 0005 mutation. See [migration 0005 production application foundation](API-FOOTBALL-MIGRATION-0005-PRODUCTION-APPLICATION.md).

## Decision — migration 0004 uses exact transactional application plus D1 Time Travel recovery

For API-Football migration 0004, do not reuse the smaller 0003 migration path unchanged. Use one dedicated manual-only exact-main workflow with no arbitrary SQL/path/version input; serialize it with the production Official FPL GitHub writer group; hard-stop if the historical direct data-platform Cron is present; pin the reviewed migration bytes and 40 statements; require exact pre/post graph, FK and Official FPL authority invariants; and obtain a current D1 Time Travel bookmark before any write. If a mutation was issued but exact post-state cannot be accepted, issue no second migration and make at most one restore to that exact pre-mutation bookmark while the same writer lock is held, then prove exact pre-state again. A recovered run still requires owner review. An unproved restore remains ambiguous and must not be retried automatically. This decision originally authorized repository foundation only. Owner approval was later granted for one attended production application; run `35467787882` on exact protected main `bc87334dbdebce4e19d92374e6640a9fed0f44fa` applied migration 0004 successfully with no recovery and independent postflight `READY_FOR_MIGRATION_0005`. That live closeout does not authorize migration 0005/0006, private mapping persistence, collector/secret/Cron activation, provider requests or model/product use. See [migration 0004 production application foundation](API-FOOTBALL-MIGRATION-0004-PRODUCTION-APPLICATION.md).

## API-Football live storage foundation — read-only preflight decision

**Decision:** before any live D1 migration or private mapping write, observe current production state through a new manual-only exact-current-main workflow using the already-provisioned `data-steward-readonly` environment. The preflight may perform only fixed Cloudflare reads and fixed D1 `SELECT`/`PRAGMA foreign_key_check` queries, must emit sanitized evidence only, and must make zero API-Football requests. Migration 0004 is a mandatory prerequisite decision point: if the live ledger does not prove exact 0004 plus its required rights columns, source-consistency triggers, participation index and identity tables, the approved 0005/0006 storage sequence stops. Unexpected collector Worker, Cron, API key secret binding, enabled collection, stale/invalid Official FPL authority, binding drift, migration drift or FK violations also stop the sequence. This decision does not authorize any production mutation. See [API-Football Live Storage Foundation](API-FOOTBALL-LIVE-STORAGE-FOUNDATION.md).

## 18 September 2026 — approve repository-only post-20/20 pre-live runtime completion

**Decision:** after API-Football ↔ Official FPL team identity reached GO 20/20, the next approved implementation step is repository-only pre-live runtime completion. Add a new migration rather than rewriting migration 0005; preserve the private crosswalk by storing only normalized qualified mappings plus hash/receipt/provenance bindings; complete the scheduled path only through pre-egress planning; keep final-enrichment/correction workload operations blocked; keep the shipped Wrangler activation blocked, D1 inert, Cron empty and provider execution impossible. This approval includes tests and canonical documentation only. It does **not** authorize applying migrations live, writing the private mapping to production D1, provisioning Cloudflare, creating/updating Worker secrets, deploying/enabling the collector, enabling Cron, making an API-Football request, ingesting workload detail or changing any model/calculation/UI path. See [pre-live runtime completion](API-FOOTBALL-PRELIVE-RUNTIME-COMPLETION.md).

## 18 September 2026 — accept attended API-Football 20-club mapping qualification GO

**Decision:** accept the attended closeout evidence from GitHub Actions run `35333898797` on exact protected `main` `aa694bc1d57faa6f197d4118ea0563808fa4ac0b` as the successful current-season API-Football ↔ Official FPL team-identity qualification. The sanitized result is **GO 20/20**: 20 verified Premier League teams, complete coverage true, 18 new owner-reviewed receipts, 2 trusted legacy receipts, 0 unresolved mappings and 0 receipt failures. The run used exactly 2 Official FPL requests and 0 API-Football requests and produced qualification integrity hash `670ccccbf62764bc076293962ae4aa85085e06db48b503f597397d882f771367`. Approved crosswalk hash `d48c7980d1c39e7d4a6f82cb56d25a750286c925fd4996d0f8c728a4fb45c7f5` and provider-universe revision `api-football-team-universe:5b404fb0a736a72973b6a1772d984992acea1dff6e2d2d7336c8253a078ae5f1` remain the identity anchors. This decision closes the 20-club identity gate only. It does **not** approve durable mapping persistence, migration 0005 application, collector/Cron activation, production provider use, expected-minutes/model influence, projections, captaincy, transfers, rank, Mini Leagues, recommendations or UI use. Those remain separate owner gates.

## 18 September 2026 — keep approved API-Football crosswalk private and hash-bound

Decision: do not publish the 18 newly approved API-Football provider-ID mappings in this public repository. The owner-approved 20-pair crosswalk is represented publicly only by SHA-256 `d48c7980d1c39e7d4a6f82cb56d25a750286c925fd4996d0f8c728a4fb45c7f5` and review metadata. Runtime closeout reads the exact crosswalk from protected-environment secret `API_FOOTBALL_OWNER_CROSSWALK_JSON`, validates that hash and the existing Chelsea/Leeds anchors, and combines it with the exact attended provider-universe evidence and a freshly content-bound Official FPL authority. This preserves the private/non-commercial/no-redistribution posture while still allowing deterministic admission. New receipts are generated and validated in memory only; the closeout artifact excludes provider-ID pairs, mappings and receipts. This decision does not activate collection, migration 0005, Cron, D1 persistence, model logic or UI.
...
Pritesh accepts unresolved underlying third-party rights risk for narrowly normalized API-Football facts used only in private, non-commercial, one-user research. `owner_risk_accepted_private_use` is valid only for exact API-Football/EIA-2I1 metadata and an exact API-Football workload source; normalized retention is allowed, but redistribution, public/commercial use and raw-payload retention are not. Objection stops collection/use pending review. R1 corrects identity semantics: verified PL players/teams use FPL identity, but non-PL fixtures and competitions use explicit external identities. Prior research-only blocking was correct under the former stricter standard and remains historical truth. Foundation stays disabled, direct-minutes-only and shadow-isolated; live qualification needs a separate EIA-2I2 gate. See [EIA-2I1](EIA-2I1-API-FOOTBALL-SHADOW-FOUNDATION.md).
...
actuator or durable authorization state is implemented. Registered state predicates, trusted-context
evidence, repository-owned provider policy and absence of generic authority are mandatory integrity
boundaries. [Full decision boundary](DATA-OPS-A1-1-POLICY-OBSERVE-ONLY-FOUNDATION.md).
...
## DATA-S1A post-merge deployment ordering — 22 August 2026
