## API-Football owner-approved 20-club mapping closeout coverage

`tests/api-football-owner-mapping.test.mjs` pins the public contract without publishing the real 18 new provider-ID pairs: exact-20/bijection validation, SHA-256 binding, Chelsea/Leeds anchors, exact current Official FPL 2026/27 ID/name/short-name review labels, approved provider-universe provenance, 18 new receipt + two legacy receipt accounting, exact 20/20 GO behaviour, Official FPL identity-drift failure, different-provider-observation failure and production/model isolation. Its positive unit path uses synthetic non-production provider IDs with injected deterministic crypto so the real crosswalk remains private; this unit evidence is not a substitute for the post-merge attended closeout.

`tests/api-football-owner-mapping-workflow.test.mjs` pins the manual-only exact-`main` workflow, `run_attempt == 1`, protected environment, one crosswalk-secret reference, absence of `API_FOOTBALL_API_KEY` and API-Football egress, exact provider artifact/run/archive digest, two-request Official FPL allowlist, focused tests before secret introduction, immutable action SHAs and sanitized-only output. The EIA-2I5B workflow-isolation guard now allowlists exactly three API-Football workflows and applies dedicated restrictions to the new owner-mapping closeout path. Full repository `./run-tests.sh`, production build, deterministic rebuild/root equality and exact manifest identity remain required before merge. After merge, the attended closeout is separately required to prove the real secret-bound crosswalk against current Official FPL authority.
