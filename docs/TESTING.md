...
## API-Football team-universe workflow remediation coverage

`tests/api-football-team-universe-workflow.test.mjs` permanently distinguishes the new one-request/two-attempt team-universe execution boundary from the historical R7 response-size workflow. It pins manual-only zero-input dispatch, read-only repository permissions, `run_attempt == 1`, exact-current-`main` checkout and three remote-`main` checks, protected environment `eia-api-football-qualification`, one secret reference/one secret-bearing step, immutable action SHAs, focused mapping/workflow tests before secret introduction, only `runAttendedApiFootballTeamUniverseQualification()`, an independent maximum-two-egress wrapper, 720,896-byte contract verification, sanitized seven-day evidence retention, and absence of mapping-admission, collector, migration 0005, Cron, D1, Official-FPL fetch, model or repository-write capabilities. Existing `tests/api-football-team-mapping-qualification.test.mjs` remains authoritative for the runner's transient retry categories, exact response identity, exactly-20 universe, integrity binding and mapping-review separation.
