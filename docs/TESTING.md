...
`tests/eia1-evidence-activation.test.mjs` covers exact cutoffs and malformed/leaking timestamp rejection; deterministic snapshot/outcome/DATA-S2B adaptation; finalized outcome identity, provenance, allowlist, account-data, provisional/corrected/supersession and missingness semantics; complete paginated DATA-S2B cursor chains and rejection of truncation, discontinuity, mixed cutoffs, duplicates and ordering errors; recursive secret/keyed-URL/raw-payload rejection; aggregate-only TheSportsDB reporting; production dependency isolation; and the unchanged empty DI-3 signal allowlist. See [EIA-1](EXTERNAL-INTELLIGENCE-EIA1-XMINS-EVIDENCE.md).
