...
GW1-P1 is merged at `main` `58b834a…`. Its backend implementation added permanent Node coverage for canonical Stage 10 parity/mutation rejection, provider-retention gates, R2-before-D1 ordering and R2 read-back verification, duplicate/idempotent ingestion, first-custody preservation, idempotency conflict, R2 failure with no D1 claim, D1-after-R2 orphan recovery, Access RS256 validation, route/origin/method/rate boundaries, safe manifest reads, Cloudflare runtime adapter behaviour and source/deployment mirror parity.
