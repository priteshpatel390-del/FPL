...
`tests/api-football-live-storage-preflight.test.mjs` pins the closed read-only SQL/PRAGMA registry, dependency-aware optional queries for partial schemas, secret-value non-disclosure, exact migration-0004 structural admission (rights columns, triggers, participation index and identity tables), stale Official FPL authority refusal, ordered 0005/0006 admission states, collection-disable enforcement, unexpected collector/secret hard stops, provider-egress absence and the manual-only exact-main protected-workflow contract with immutable action SHAs. The attended post-merge preflight remains live evidence rather than a repository test: CI can prove that the code cannot request the provider or declare a production mutation path, but only a real read-only Cloudflare/D1 run can establish current migration/binding/collector state. See [API-Football Live Storage Foundation](API-FOOTBALL-LIVE-STORAGE-FOUNDATION.md).
