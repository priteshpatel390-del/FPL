...
`tests/api-football-migration-0006-foundation.test.mjs` pins exact migration path/blob/bytes/ledger identity, 12-statement shape, tables/triggers/index/anchors, exact pre/post cardinalities, parameterized private values, sentinel non-serialization, manual one-shot exact-main/Verify workflow gates, shared non-cancelling writer lock, separate schema/mapping admission, 50,000-row budget, zero provider/activation surface, reconciliation without retry and no automatic restore. Existing local D1, live-storage and pre-live mapping suites remain complementary regression coverage.
