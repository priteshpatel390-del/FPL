...
## DATA-OPS A1.3 /schedules response-layer diagnostic coverage

`tests/data-ops-a1-2-observe-only-sentinels.test.mjs` pins the three new closed response-processing category codes proven necessary by run `34319945520`: a `response.json()` throw returns `CLOUDFLARE_SCHEDULES_JSON_INVALID`; a parsed body that is not a valid Cloudflare envelope (including a non-object body, `success !== true`, a missing `result`, and a bare array) returns `CLOUDFLARE_SCHEDULES_ENVELOPE_INVALID` without any change to `decodeEnvelope()`; a valid envelope carrying a result the existing `decodeSchedules()` rejects (a non-array `schedules`, an invalid cron string, an over-length list, a missing `schedules` key) returns `CLOUDFLARE_SCHEDULES_PAYLOAD_INVALID` without any change to `decodeSchedules()`; a healthy envelope and payload still succeed and proceed to `/deployments`/`/settings`; every one of the three new categories issues exactly one Cloudflare request and never a numeric status, provider text, a URL or exception/parse-error text in the returned result; the four earlier `/schedules` categories, the deployments/settings-stage failures and the fully healthy three-request cycle are unchanged; a dedicated behavioural-contract test proves `decodeEnvelope`/`decodeSchedules` are invoked with unchanged semantics; and the seven schedules category codes are closed, distinct, uppercase identifiers. `tests/data-ops-a1-3-readonly-observer.test.mjs` additionally proves the new codes pass through `sanitizedSummary()` with nothing else attached — no token, account id, fingerprint, URL, status or provider message. Existing masking, format-contract, workflow-contract and migration-inventory tests remain intact and unweakened.
