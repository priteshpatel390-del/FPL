...
`tests/api-football-migration-0006-foundation.test.mjs` pins exact migration path/blob/bytes/ledger identity, 12-statement shape, tables/triggers/index/anchors, exact pre/post cardinalities, parameterized private values, sentinel non-serialization, closed D1 transport/HTTP/JSON/envelope/cardinality/result diagnostics, conservative definite-versus-ambiguous reconciliation, manual one-shot exact-main/Verify workflow gates, shared non-cancelling writer lock, separate schema/mapping admission, 50,000-row budget, zero provider/activation surface, no retry and no automatic restore. `tests/api-football-migration-0006-transaction.test.mjs` applies migrations 0001–0006 to SQLite and proves the full 43-statement mapping transaction succeeds after the qualification-placeholder correction and rolls back completely for induced entity, qualification, member, commit and head failures. Reconciliation tests require dynamic integrity-hash shape and qualification-ID binding, fixed approved provenance, valid receipt hashes, exact member relationships and exact cardinality without pinning one fresh authority snapshot. Pre-live coverage proves identical inputs reproduce one persistence hash while fresh authority evidence produces a different valid hash.

## Migration 0005 live production acceptance evidence

Repository verification before live execution is complete: post-merge Verify Teamsheet run `35497426515` on `f07c8acee8aecacfd55ae0e7f015e7799d190294` passed 2,225/2,225 tests with byte-identical exact-identity builds and exact manifest identity; Pages `35497425916` succeeded. Live run `35497565060` then exercised the real production gates. Preflight returned `READY_FOR_MIGRATION_0005` with 1,439 current-day account D1 rows written. Application returned `DEFINITELY_APPLIED_SUCCESSFULLY`, one mutation, no recovery, and observed 6 D1 API calls / 417,635 rows read / 41 rows written / 24,014 request bytes. Postflight returned `READY_FOR_MIGRATION_0006`, zero FK violations and zero API-Football request/mapping/runtime-workload rows beyond the disabled seed. This is live acceptance for migration 0005 only; it is not test evidence for migration 0006, provider collection or model accuracy.

## Migration 0005 checkout-isolation regression

Run `35496920942` passed all 27 focused migration tests but then failed the workflow's clean-checkout assertion. The focused local D1 harness must therefore be treated as having a repository-side-effect risk when its child Wrangler process runs with `cwd` set to the repository root. The remediation runs Wrangler from the harness-owned temporary directory, uses absolute paths for repository migration files, and extends `tests/eia2i5d-migration.test.mjs` to snapshot `git status --porcelain --untracked-files=all` before and after the local migration harness and require exact equality. The workflow-level clean-checkout assertion remains unchanged and authoritative. Candidate Verify Teamsheet run `35497159414` on head `158da28c0884c48f72931d798da389dc924611c7` passed 2,225/2,225 tests, production build, byte-identical exact-identity rebuild and exact manifest identity. Exact-main verification remains required after merge.

## API-Football migration 0005 production-application foundation coverage

`tests/api-football-migration-0005-foundation.test.mjs` pins the exact 9,923-byte migration blob, 20-statement/7-trigger parser shape, exact 0004 pre-state and 0005 post-state, migration-0006 rejection, empty provider pre-state, preserved Official FPL/history populations, disabled runtime seed, one-shot success/unknown-transport reconciliation, definitely-not-applied reconciliation, ambiguous post-state handling and the absence of any automatic Time Travel restore. It also tests account-wide current-UTC-day D1 write-usage aggregation, the 50,000-row admission threshold, fail-closed absence and separation of the Account Analytics Read credential, historical direct-Cron rejection, shared non-cancelling production-writer serialization, manual exact-main/Verify gating, immutable action SHAs and complete API-Football credential/provider isolation. Existing `tests/eia2i5d-migration.test.mjs` remains complementary local D1 evidence for applying 0001-0006 and enforcing runtime/mapping constraints. Full `./run-tests.sh`, production build and deterministic exact-identity rebuilds are required at the candidate PR head; repository tests do not substitute for a later fresh production preflight. See [migration 0005 production application foundation](API-FOOTBALL-MIGRATION-0005-PRODUCTION-APPLICATION.md).

## API-Football live storage read-only preflight tests

`tests/api-football-live-storage-preflight.test.mjs` pins the closed read-only SQL/PRAGMA registry, dependency-aware optional queries for partial schemas, secret-value non-disclosure, exact migration-0004 structural admission, stale Official FPL authority refusal, ordered 0005/0006 admission states, collection-disable enforcement, unexpected collector/secret hard stops, provider-egress absence and the manual-only exact-main protected-workflow contract. The first live run `35426768910` exposed a missing production-shape regression: DATA-S2A stores ten `official_fpl_team` metrics per club, but the preflight fixture modeled one row per club and even used a non-canonical zero-padded logical key. Remediation coverage now models all 200 team-metric heads, proves the live read selects exactly the 20 active `present=true` heads, requires exact canonical `official-fpl|2026-27|team|<id>|present` keys, rejects zero-padded/non-canonical spellings, and preserves the 48-hour freshness gate. CI still cannot substitute for the attended Cloudflare/D1 rerun: repository tests prove the corrected contract; the live rerun must separately prove current authority and storage state. See [API-Football Live Storage Foundation](API-FOOTBALL-LIVE-STORAGE-FOUNDATION.md).

## API-Football post-20/20 pre-live runtime completion coverage

`tests/api-football-prelive-runtime-completion.test.mjs` pins private qualified-mapping plan admission, exact-20/hash/receipt binding, legacy anchors, deterministic five-query discovery planning, closed known-fixture request planning, stale-authority/incomplete-mapping failure and explicit blocking of workload enrichment/correction. EIA-2I5D runtime coverage additionally pins explicit `PRELIVE_PLANNER_ONLY` activation and proves the shipped `REPOSITORY_ONLY_BLOCKED` handler touches neither D1 nor credential. Local D1 migration coverage now applies 0001–0006, verifies exact-20 committed mapping qualification/head constraints and committed-provenance immutability. Full repository tests, production build and deterministic build identity remain required before review.

## API-Football 20-club mapping live-closeout verification

Repository tests continue to pin the private/hash-bound mapping contract and workflow isolation. PR #257 exact-head Verify Teamsheet run `35331263084` passed 2,180/2,180 tests, and post-merge exact-main run `35332702502` independently passed 2,180/2,180 with byte-identical production rebuilds and exact manifest identity. The real crosswalk was then proven by attended workflow run `35333898797` on exact protected `main`, not by synthetic test fixtures: decision **GO**, verified teams 20/20, complete coverage true, 18 new receipts, 2 legacy receipts, 0 unresolved mappings, 0 receipt failures, 2 Official FPL requests and 0 API-Football requests. Sanitized result artifact `10542293263` records final qualification hash `670ccccbf62764bc076293962ae4aa85085e06db48b503f597397d882f771367` and no raw-body/crosswalk/receipt persistence.

`tests/api-football-owner-mapping.test.mjs` remains permanent contract coverage using synthetic non-production provider IDs so the private real crosswalk is never published by CI. `tests/api-football-owner-mapping-workflow.test.mjs` permanently pins manual-only exact-`main` execution, `run_attempt == 1`, protected environment, one crosswalk-secret reference, no `API_FOOTBALL_API_KEY`, exact provider-artifact evidence, two-request Official FPL allowlist, immutable actions and sanitized-only output. Synthetic unit GO and the completed attended live GO are distinct evidence types; both are now satisfied.

## API-Football team-universe attended workflow coverage

`tests/api-football-team-universe-workflow.test.mjs` permanently pins the dedicated attended workflow: manual-only/no-input trigger, attempt-1 rerun rejection, read-only permissions, single protected secret step, exact-current-`main` execution, absence of the historical PR #251 candidate/runner, pre-secret focused tests, exact 2-attempt and 720,896-byte constants, the exact `/teams?league=39&season=2026` contract, correct team-universe runner invocation, integrity validation, 20-team success postcondition, raw-body/secret exclusion, sanitized seven-day evidence retention, SHA-pinned actions and absence of collector/D1/Cron/mapping/model activation. An executable regression extracts the independent `boundedFetch` wrapper and proves a third attempted network invocation is rejected before the underlying fetch is called.
...
`tests/eia2i5b-provider-discovery.test.mjs` pins the five approved league/season plans, closed known-ID versus discovery surfaces, origin pinning, header-only credentials, serial attempts, 1s injected delay, one retry bound, ten-attempt ceiling, 429 stop including absent/malformed quota headers, no-retry auth/schema paths, quota-header normalization, fixture-scoped independent candidates, duplicate provider-fixture-ID core-identity collisions, identity/orientation/mapping/conflict behaviour, canonical Chelsea 49→FPL 6 and Leeds 63→FPL 13 on 9 September 2026 with 19:00 vs 19:15 kickoff conflict and no time tolerance, final-state workload relevance, output-atomic five-competition generation (incomplete/failed scans return `fixtures: []` and zero final admission counts while retaining sanitized diagnostics and provider-row observations; valid zero-row competitions do not invalidate a complete scan), Official FPL team-universe authority issued only from DATA-S2A `normaliseOfficialFplHistory()` (not a self-certified 20-team snapshot), exact 20-club bijection, receipt tampering/wrong-season/conflicting-representation fail-closed cases, malformed optional team representations failing closed without throw, repository-controlled 15-second `AbortSignal.timeout` abort (hung transport fails as sanitized `provider_timeout` without waiting the production deadline; at most one retry; 429/auth/schema still do not retry; 5xx retry unchanged; generic transport remains distinct; known-ID shares the same bound; duration is not caller-overridable), and structural isolation from production, wrangler, workflows, live Official FPL fetch and migration 0005. Tests use injected fetch, an injected `timeoutSignal` factory and injected canonical Official FPL bootstrap/fixtures only. Existing EIA-2I1/EIA-2I3/EIA-2I5A tests remain the duration, rights and identity owners.
...
`tests/eia2i1-api-football-foundation.test.mjs` pins the narrow source-bound owner-risk contract, unchanged legacy rights behavior, verified FPL player/PL-team mapping, same-name safety, external non-PL fixture/competition identities, explicit competition configuration, per-instance maximum-100 pre-live budget, disabled/missing-secret behavior, exact-origin and closed-endpoint credential safety, sanitized network/schema/quota failures, the shared 15-second `AbortSignal.timeout` bound (`provider_timeout` versus generic `provider_unavailable`), strict acquisition timestamp/revision validation, duration-backed direct-minute and substitution/extra-time/PEN/dismissal facts, unknown missingness, malformed-response rejection, deterministic hashing, absence of raw payloads/secrets and structural separation from production model/application paths.
...
**Deterministic fixture/contract coverage (PR #238, merged).** `tests/data-ops-a1-2-observe-only-sentinels.test.mjs` pins the new `canonicaliseCron()` behaviour inside the single-pass `analyseSchedulesPayload()`: a full `1..31` day-of-month enumeration for each of the three approved daily schedules canonicalises to the repository's textual wildcard form; a mixed result of wildcard and full-enumeration rows still canonicalises to exactly the approved set; a live cycle using full-enumeration crons still succeeds and uses exactly three Cloudflare reads; a canonicalised-but-different schedule (a different minute, or a different hour via full enumeration) still reaches `CLOUDFLARE_CRON_SET_MISMATCH` rather than being accepted; a partial or incomplete day-of-month list, a duplicate day value, an out-of-range day, an out-of-range minute/hour, malformed number syntax, a six/seven-field string, and unsupported month/day-of-week values all still fail closed as `CLOUDFLARE_SCHEDULES_CRON_PATTERN_REJECTED`; a rejected day-of-month enumeration never exposes the raw expanded text in the observation result. All 92 A1.2 sentinel tests, all A1.3 observer tests, and the existing five payload-decode categories and eleven-category closed-enum test remain green and unweakened. **This fixture coverage proves the canonicaliser's own logic against constructed inputs — it does not by itself prove the live Cloudflare `/schedules` response.**

**Live acceptance evidence, kept separate.** After PR #238 merged and exact-`main` Verify (`34345865860`) succeeded, one attended manual `Data Steward Read-Only Observer` dispatch — run `34346126189`, head `174a7ece2f6c52257902c79ac9a46de846edeb91` — returned `verdict: HEALTHY` with Cloudflare, D1 and GitHub all `OBSERVED`. This is the actual live-provider proof the fixture tests above cannot substitute for: the deterministic unit tests establish that the canonicaliser behaves correctly against every constructed shape this repository could anticipate, and this one live dispatch is the separate evidence that the real Cloudflare `/schedules` response matched one of those shapes and canonicalised correctly. Neither claim stands in for the other.

<!-- DATA-OPS-A1-3-2026-09-09-SCHEDULES-PAYLOAD-DECODE -->
## DATA-OPS A1.3 `decodeSchedules` payload-decode diagnostic coverage, extended by the Cron semantic-normalisation coverage above

`tests/data-ops-a1-2-observe-only-sentinels.test.mjs` pins the five new closed payload-decode category codes proven necessary by run `34325772296`: a result that is not an object, or is an array, returns `CLOUDFLARE_SCHEDULES_RESULT_INVALID`; a missing or non-array `schedules` property returns `CLOUDFLARE_SCHEDULES_ARRAY_INVALID`; a `schedules` array over the 16-entry bound returns `CLOUDFLARE_SCHEDULES_COUNT_EXCEEDED`; a row with no usable string cron value returns `CLOUDFLARE_SCHEDULES_CRON_NOT_STRING`; a cron string the existing `CRON` regular expression rejects returns `CLOUDFLARE_SCHEDULES_CRON_PATTERN_REJECTED`; a healthy envelope and payload still succeed and proceed to `/deployments`/`/settings`; every one of the five new categories issues exactly one Cloudflare request and never a numeric status, provider text, a URL or exception text in the returned result; the six earlier `/schedules` categories, the deployments/settings-stage failures and the fully healthy three-request cycle are unchanged; a dedicated parity test proves `decodeSchedules()` and the new `classifySchedulesPayload()` can never disagree about which live results are valid, across every existing and new rejection shape, and that the extracted cron list matches exactly when both accept; and the eleven schedules category codes are closed, distinct, uppercase identifiers. `tests/data-ops-a1-3-readonly-observer.test.mjs` additionally proves the new codes pass through `sanitizedSummary()` with nothing else attached — no token, account id, fingerprint, URL, status or provider message. Existing masking, format-contract, workflow-contract and migration-inventory tests remain intact and unweakened.

<!-- DATA-OPS-A1-3-2026-09-09-SCHEDULES-RESPONSE-LAYER -->
## DATA-OPS A1.3 /schedules response-layer diagnostic coverage, extended by the payload-decode split above

`tests/data-ops-a1-2-observe-only-sentinels.test.mjs` pinned the three closed response-processing category codes proven necessary by run `34319945520`: a `response.json()` throw returns `CLOUDFLARE_SCHEDULES_JSON_INVALID`; a parsed body that is not a valid Cloudflare envelope (including a non-object body, `success !== true`, a missing `result`, and a bare array) returns `CLOUDFLARE_SCHEDULES_ENVELOPE_INVALID` without any change to `decodeEnvelope()`; a valid envelope carrying a result the existing `decodeSchedules()` rejects (a non-array `schedules`, an invalid cron string, an over-length list, a missing `schedules` key) returned `CLOUDFLARE_SCHEDULES_PAYLOAD_INVALID`. That third category is now itself subdivided into five by the block above, and `CLOUDFLARE_SCHEDULES_PAYLOAD_INVALID` is no longer emitted or tested for; the other two response-processing categories remain covered exactly as before.

<!-- DATA-OPS-A1-3-2026-09-09-SCHEDULES-CLASSIFICATION -->
## DATA-OPS A1.3 schedules-read failure classification coverage, extended by the response-layer split above

`tests/data-ops-a1-2-observe-only-sentinels.test.mjs` pinned the five closed `/schedules` category codes proven necessary by run `34311398342`: a transport exception returns `CLOUDFLARE_SCHEDULES_TRANSPORT_FAILED`; HTTP 401/403 return `CLOUDFLARE_SCHEDULES_AUTH_REFUSED`; HTTP 404 returns `CLOUDFLARE_SCHEDULES_NOT_FOUND`; any other non-200 (429/500/503 exercised) returns `CLOUDFLARE_SCHEDULES_HTTP_FAILED`; and HTTP 200 with unparseable JSON, an invalid envelope, or a decoder-rejected result all returned `CLOUDFLARE_SCHEDULES_RESPONSE_INVALID`. The last of those five categories is now itself subdivided into three by the block above, and `CLOUDFLARE_SCHEDULES_RESPONSE_INVALID` is no longer emitted or tested for; the other four categories remain covered exactly as before.

<!-- DATA-OPS-A1-3-2026-09-08-CLOUDFLARE-READ-DIAGNOSTICS -->
## DATA-OPS A1.3 Cloudflare fixed-read diagnostic coverage, merged as PR #234

`tests/data-ops-a1-2-observe-only-sentinels.test.mjs` pinned the three closed reason codes proven necessary by run `34277208819`: an identity mismatch still issues zero Cloudflare requests; a deployments-stage failure returns `CLOUDFLARE_DEPLOYMENTS_READ_FAILED` after exactly two requests; a settings-stage failure returns `CLOUDFLARE_SETTINGS_READ_FAILED` after exactly three; a fully healthy cycle still issues exactly three `GET` requests, in order, against `/schedules`, `/deployments` and `/settings`, with no request body. `CLOUDFLARE_SCHEDULES_READ_FAILED` no longer exists: its former coverage is superseded by the five-category schedules coverage above.

<!-- DATA-OPS-A1-3-2026-09-08-REMEDIATION -->
## DATA-OPS A1.3 first-live-observation masking coverage

`tests/data-ops-a1-3-readonly-observer.test.mjs` additionally pins the identifier-masking remediation proven necessary by run `34269989975`, tightened on owner review: no job-level `env:` block exists at all; the first step's step-level env contains exactly the account id secret and nothing else; checkout and setup-node declare no steward env; the final `Execute one read-only observation` step's step-level env contains exactly the four required runtime values, with the fingerprint materialised exactly once, only there; and the rest of the workflow contract (permissions, exact-`main` guard, `deployment: false`, the two schedules, the exact activation condition) is unchanged. `tests/data-ops-a1-2-observe-only-sentinels.test.mjs` additionally proves the account-identity contract rejects a `sha256:`-prefixed and an uppercase fingerprint, pinning that the runtime check requires exactly `^[0-9a-f]{64}$`. A repository-text scan proves no full live fingerprint or account id value is present anywhere in tracked source or documentation.
...
`tests/eia1-evidence-activation.test.mjs` covers real canonical Stage 10 snapshot/outcome finalisation followed by shared integrity validation; adversarial post-finalisation mutation of xMins probabilities, baseline inputs, player minutes/starts/60-minute state, provenance and completeness; syntactically self-consistent forged identities; legitimate corrected outcomes; exact cutoffs and malformed/leaking timestamp rejection; deterministic DATA-S2B adaptation; finalized outcome identity, provenance, allowlist, account-data and missingness semantics; complete paginated DATA-S2B cursor chains and rejection of truncation, discontinuity, mixed cutoffs, duplicates and ordering errors; recursive secret/keyed-URL/raw-payload rejection; aggregate-only TheSportsDB reporting; production dependency isolation; and the unchanged empty DI-3 signal allowlist. See [EIA-1](EXTERNAL-INTELLIGENCE-EIA1-XMINS-EVIDENCE.md).
...
## API-Football inactive Version staging permanent coverage

`tests/api-football-collector-inactive-version-staging.test.mjs` covers the exact Worker shell body, workers.dev/Preview disablement, exact D1 plus three plain-text Version bindings, zero API-key binding, deterministic closed transitive module graph and per-module/metadata/SHA identity, exact two-endpoint mutation allowlist, definite-rejection no retry, ambiguous shell and Version no-retry reconciliation, inactive postflight with zero Deployment/Cron/routes/domains/secrets, first-attempt exact-main/exact-Verify workflow gates, fresh read-only admission before protected staging, dedicated upload-environment credential isolation, sibling-workflow isolation, final read-only D1/runtime closeout and unchanged production model/UI isolation. Full-suite and deterministic-build evidence must come from the exact draft-PR head before this implementation is considered complete.
