## Owner-reviewed API-Football 20-club mapping closeout architecture

The owner-approved crosswalk is intentionally separated from public repository content. Source code stores only the approved crosswalk hash plus public Official FPL 2026/27 labels and the exact prior provider-evidence provenance. The crosswalk itself is admitted only from protected-environment secret `API_FOOTBALL_OWNER_CROSSWALK_JSON`. A manual-only workflow on exact current `main` downloads the previously captured sanitized provider-universe artifact by pinned artifact ID and archive digest, validates its integrity/provenance, then performs exactly two public Official FPL reads (`bootstrap-static` and `fixtures`) in memory. The mapping module verifies the secret hash, exact 20↔20 bijection, Chelsea/Leeds anchors, the approved provider-universe revision and current Official FPL ID/name/short-name labels before issuing receipts. Eighteen new receipts and two legacy receipts exist only in memory for qualification; output persistence is limited to counts, provenance hashes and the final qualification integrity hash. No API-Football credential/provider egress, D1 mapping write, collector import, model import or browser path is introduced.
