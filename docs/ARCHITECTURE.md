## Dormant API-Football collector infrastructure staging boundary

The collector now has a repository-only control-plane staging candidate, separate from its runtime execution path. A manual workflow first uses `data-steward-readonly` for fresh admission, then a dedicated `api-football-collector-version-upload` environment for at most one inert Worker-shell create plus one inactive Version upload, and finally returns to the read-only steward for independent closeout. The Version upload preserves source bytes for the closed 19-module transitive ES-module graph rooted at `workers/api-football-collector/collector.mjs`; repository-relative module names are the multipart identities and every module plus metadata/graph identity is SHA-256 bound to the approved repository SHA. Deployment, scheduling, secrets, routes, D1 writes and provider egress are outside this architecture stage.
...
A dedicated activation preflight now sits between the merged dormant execution foundation and any future infrastructure mutation. It is manual, exact-main and exact-head-Verify gated, uses only `data-steward-readonly`, performs five bounded Cloudflare metadata/settings GETs plus one D1 query POST containing ten fixed `SELECT`/`PRAGMA foreign_key_check` statements, and rejects any D1 `rows_written != 0`. It combines live D1/mapping/runtime evidence with the shipped inert Wrangler contract and the live absence of the collector Worker, then delegates classification to the existing `REPOSITORY_INFRASTRUCTURE_STAGING` contract. It contains no Version upload, Deployment, secret mutation, Cron mutation, runtime mutation or provider transport path. See [activation read-only preflight](API-FOOTBALL-COLLECTOR-ACTIVATION-READONLY-PREFLIGHT.md).

## Dormant API-Football collector activation composition

Collector now has one repository-only execution owner for `plan -> reserve -> bounded transport -> semantic validation -> generation ceiling -> concrete normalized D1 persistence -> attempt completion -> generation/head commit`. Transport owns no D1 state; the D1 adapter owns no provider egress. Discovery creates `STAGING` and keeps previous head authoritative until all five attempt-1 requests persist and complete. Concrete 1,250-row bulk chunks enforce a 2,500-fixture, 7,525-row-write and 50-total-statement ceiling. Shipped Wrangler does not select execution state. See [collector activation foundation](API-FOOTBALL-COLLECTOR-ACTIVATION-FOUNDATION.md).
...
This closes storage/mapping readiness only. The dedicated `workers/api-football-collector/` package is still repository-only: shipped Wrangler activation is `REPOSITORY_ONLY_BLOCKED`, Cron list is empty, the production collector is not thereby deployed/bound/credentialed, and the scheduled handler still stops after planning with `provider_execution_not_approved`. Provider execution and all workload/model/product consumption remain later explicit gates.
...
## API-Football migration 0005 application architecture — dormant repository boundary

The owner-approved repository candidate adds a dedicated exact-migration-0005 administrative path rather than generalising an existing migration runner. A manual-only GitHub workflow shares `data-s2-production-collection` with Official FPL production writers, requires exact protected current `main` plus exact-head Verify, then runs a mutation-free read-only admission under `data-steward-readonly`. Admission reuses the live-storage contract, hard-stops if the historical direct `teamsheet-data-platform` Cron exists, and adds an account-wide current-UTC-day D1 `rowsWritten` analytics gate that refuses above 50,000 rows. That GraphQL read is isolated to a future `DATA_STEWARD_CLOUDFLARE_ANALYTICS_TOKEN` with Account Analytics Read only; the existing Workers/D1 read token is not widened or reused and no token is provisioned by this candidate. The write runner pins migration blob `a65c57924a5e18ad8e72e491c376fe0f1c149e88`, 9,923 bytes and 20 trigger-aware statements, obtains a Time Travel bookmark, submits exactly one batch and reconciles common schema before reading newly-created tables. Unlike migration 0004 it exposes **no automatic Time Travel restore**; partial/contradictory state is owner attention. No provider host, API-Football credential, Worker deploy/binding, Cron, migration 0006, mapping persistence or model/product path exists in this boundary. See [migration 0005 production application foundation](API-FOOTBALL-MIGRATION-0005-PRODUCTION-APPLICATION.md).

## API-Football migration 0004 application architecture — dormant repository boundary

Draft PR #262 adds a dormant manual-only migration boundary, not a live schema change. The workflow is serialized with Official FPL GitHub D1 writers through `data-s2-production-collection`; because the 01:17 Cloudflare timer dispatches the external GitHub collection rather than writing D1 itself, that lock covers the active production collection path. A migration-specific read-only gate additionally requires the historical direct `teamsheet-data-platform` Cron count to remain zero. The write runner pins the exact migration-0004 Git blob and 40-statement shape, captures a D1 Time Travel bookmark, submits one transactional batch, verifies preserved graph counts/FKs/authority and either accepts exact 0004 or makes one restore-to-pre-bookmark attempt while the writer lock remains held. No 0005/0006, provider egress, mapping persistence, collector or product/model dependency is introduced. See [migration 0004 application](API-FOOTBALL-MIGRATION-0004-PRODUCTION-APPLICATION.md).

## API-Football live storage preflight architecture

Before live storage mutation, a dedicated manual-only preflight now owns production-state observation. It reuses the existing `data-steward-readonly` protected environment, pins the reviewed production D1 identity, reads the active `teamsheet-data-platform` D1 binding, and addresses the API-Football collector only through fixed settings/schedules/deployments reads. D1 access is limited to a closed registry of `SELECT` statements plus fixed read-only `PRAGMA foreign_key_check` / `PRAGMA table_info` statements, at most two query calls, with a second-stage query admitted only when all referenced tables were observed in `sqlite_master`. Any reported D1 write is rejected. The adapter emits sanitized counts/hashes/state only and never selects private mapping pairs for output. A production ledger without exact migration 0004 and its required rights columns, source-consistency triggers, participation index and identity tables stops before 0005; stale Official FPL authority also stops progression. See [API-Football Live Storage Foundation](API-FOOTBALL-LIVE-STORAGE-FOUNDATION.md).

## API-Football post-20/20 pre-live runtime architecture

The next repository layer separates **qualified mapping persistence** from **provider execution**. Migration 0006 adds an append-only 20-member mapping qualification/head contract bound to crosswalk, receipt, provider-universe and Official FPL authority hashes; it does not publish private provider-ID pairs. A new pre-egress planner reads only fresh Official FPL authority, one committed exact-20 mapping head, current discovery state and completed request identities, then converts due scheduler opportunities into the existing closed request contracts. Workload-detail enrichment/correction remains blocked. The shipped Wrangler activation remains `REPOSITORY_ONLY_BLOCKED`, and the scheduled handler still cannot execute provider requests. No production/browser/model import exists. See [pre-live runtime completion](API-FOOTBALL-PRELIVE-RUNTIME-COMPLETION.md).

## Live-qualified API-Football 20-club mapping architecture

The owner-reviewed 2026/27 crosswalk remains intentionally separated from public repository content. PR #257 merged the hash-bound closeout path, and attended run `35333898797` on exact protected `main` `aa694bc1d57faa6f197d4118ea0563808fa4ac0b` proved **20/20 GO** against a freshly content-bound Official FPL authority. The workflow validated the exact prior provider-universe evidence, the protected crosswalk hash, current Official FPL ID/name/short-name identities, 18 new owner-reviewed receipts and 2 trusted legacy receipts, with zero unresolved mappings or receipt failures. The run made 2 Official FPL requests and 0 API-Football requests. Receipts and the full mapping table existed only in memory; persisted output contains only sanitized counts, provenance/timestamps and integrity hashes. This architecture still has no live D1 mapping writer, collector import into production, Cron activation, browser/model import or recommendation path. Mapping qualification and runtime activation remain separate layers.
...
A dedicated GitHub Actions workflow now hosts one read-only A1.2 observation run, independently of the Cloudflare clock it observes. Manual dispatch is available; schedules at `17 4 * * *` and `17 8 * * *` are declared but the scheduled job runs only when `DATA_STEWARD_SCHEDULED_ENABLED` equals exact `true` — that variable was last owner-verified absent, so scheduled execution stays dormant. `workers/data-steward/run-observer.mjs` is the sole ambient-environment/logging adapter; deterministic sentinel modules remain dependency-injected. GitHub authority is the ephemeral read-only job token. Cloudflare authority remains Workers Scripts Read and D1 Read. **Manual attended dispatch has since passed live acceptance**: run `34346126189` on exact `main` `174a7ece2f6c52257902c79ac9a46de846edeb91` returned `HEALTHY`/`HEALTHY_EXPECTED_STATE` with all three sentinels `OBSERVED`, proving this runtime operational end to end against live Cloudflare, D1 and GitHub state; no remediation/actuator/mutation capability was added, and scheduled activation remains a separate unapproved gate. See [A1.3](DATA-OPS-A1-3-LIVE-READONLY-OBSERVER.md).
...
surface exists. Registered predicates prove GREEN/AMBER from exact state; trusted context, not the
proposal, supplies incident-bound evidence; repository definitions own provider policy; and no
generic authority action exists. See [A1.1 foundation](DATA-OPS-A1-1-POLICY-OBSERVE-ONLY-FOUNDATION.md).
...
Stage B Option A adds only a scoped bundle runtime and two post-output calls from Team and Transfers presentation. The calls submit immutable copies of outputs already chosen by existing code; no artifact read returns to model, optimiser, provider, state or UI logic. Runtime results are ephemeral and failure-isolated.
...
## Decision Intelligence DI-4 product boundary (29 August 2026)

The deployable graph now contains `DI3_PARITY_RUNTIME -> product-read-model.mjs -> weekly-decision.mjs`. The versioned read model accepts only the canonical DI-3 artifact and has no model, optimiser, provider, persistence, network or account-write dependency. Team owns the presentation; Transfers remains the deeper unchanged workflow. Missing/unsupported artifacts fail closed while existing views remain available. See [DI-4 Product Integration](DECISION-INTELLIGENCE-DI4-PRODUCT-INTEGRATION.md).
