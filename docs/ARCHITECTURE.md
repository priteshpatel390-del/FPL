...
**GW1-P2 — Browser evidence delivery and durable outbox** is an unmerged candidate in draft PR #119, exact head `414eb58f6207b3e2a78515a93d0c4c98685b3863`. It connects the existing Stage 10 browser capture path to that merged backend through a durable local outbox, a transport-independent delivery state machine, content-hash idempotency and fail-closed provider retention. It is repository-verified but **not accepted**: its acceptance test failed, because the decisive credentialled cross-site upload from physical iPhone Safari with Prevent Cross-Site Tracking ON did not deliver on 20 August 2026, and the cause is unisolated. On deployed `main` no browser call into the evidence Worker exists. See [GW1-P2 Browser evidence delivery](GW1-P2-BROWSER-EVIDENCE-DELIVERY.md).

**GW1-P2D-P1 — Diagnostic transport probe** is a further unmerged candidate that adds owner-initiated diagnostic instrumentation only, so the failed transport stage can be isolated before any replacement architecture is chosen. It adds no delivery path, no persistence and no Worker change. See [GW1-P2D-P1 Diagnostic transport probe](GW1-P2D-DIAGNOSTIC-TRANSPORT-PROBE.md).
...
Last reconciled: 2026-08-20. Related: PROJECT_CONTEXT.md, TEAMSHEET2-PRODUCT-BLUEPRINT.md, DATA_SOURCES.md, TESTING.md, SECURITY.md, DATA-ARCHITECTURE-D1.md, GW1-P1-CLOUDFLARE-EVIDENCE-FOUNDATION.md, GW1-P2-BROWSER-EVIDENCE-DELIVERY.md, EXTERNAL-INTELLIGENCE-FOUNDATION.md.
