...
PR #262 merged at `3892f2c6448d951538cf318ad28392ca95ca2449`, but its first merged workflow definition is not dispatch-ready: GitHub rejected it before creating any job because `runner.temp` was referenced at job-level `env`, where the `runner` context is unavailable. The narrow follow-up moves that value to step scope; until that remediation is merged and exact-main verified, do not dispatch migration 0004. The zero-job failures admitted no environment or credential and made no production mutation. Separately, the merged recovery capability does not make migration 0004 live or risk-free. D1 Time Travel is a whole-database destructive restore, not a table-level rollback; it is safe to automate here only while the shared production-writer lock is held and the historical direct data-platform Cron is proved absent. Time Travel retention is finite (7 days on Workers Free, 30 days on Workers Paid), and a later recovery after an ambiguous run cannot be assumed safe if any out-of-band writer has changed D1. The runner therefore makes at most one same-run restore attempt, never blindly retries a lost restore response, spends the remaining bounded read on exact pre-state reconciliation while the writer lock is still held, exposes only bookmark digests/checkpoint time, and otherwise stops as ambiguous for attended reconciliation. No claim of live reversibility exists until the production database itself successfully returns a Time Travel bookmark during an owner-approved application attempt. Migration 0004 remains unapplied in this checkpoint. See [migration 0004 production application foundation](API-FOOTBALL-MIGRATION-0004-PRODUCTION-APPLICATION.md).
