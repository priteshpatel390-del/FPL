...
| REFRESH-3 | Refresh-Load R1's corrected offline disclosure lacks its focused physical iPhone retest | The first device pass accepted online, cached, manual-refresh, background and in-app offline-resilience paths but exposed misleading fresh/live FPL wording while offline; corrected source `d1b6ac0…` stops before networking, preserves the saved timestamp and labels FPL Fallback | R1 draft review | Open (merge gate — focused retest) |
