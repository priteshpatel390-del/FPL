...
`filter=all`, so a re-run whose newest attempt skips `collect` cannot hide an earlier attempt that
collected, and the daily window is judged on the `collect` job's own `started_at` rather than the
