...
| REFRESH-2 | Foreground refresh mutates runtime provider state before one final render instead of constructing a fully atomic replacement | Atomic Foreground Refresh stages collection, applies one synchronous no-throw commit, keys account carry-forward, and separates collection/commit/render/persistence errors | Atomic Foreground Refresh | **Implementation candidate — merge and physical iPhone acceptance owner-gated** |
