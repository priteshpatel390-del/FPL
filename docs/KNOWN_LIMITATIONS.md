...
| UI-13 | UX-A2 Player Detail scroll and rotation correction has not yet received populated physical iPhone Safari acceptance | Automated tests cover the controller's capture/restore/lock/reset transitions and the presence and ordering of the CSS scroll, viewport-fallback, compact-landscape and safe-area contracts, but cannot prove real layout on a device: momentum scrolling, Safari toolbar expansion/collapse, live rotation, notch insets and actual reachability of the close control remain unverified until the owner device script is performed. VoiceOver is not a Teamsheet acceptance gate. | UX-A2 owner acceptance | Open (acceptance gate) |
