...
| SCOR-1 | Linear approximations of stepped FPL rules | Replaced in draft PR #9 by deterministic expected-value distributions for saves and goals conceded plus threshold probability for defensive contributions | Stage 5 | **CLOSED in draft; verification pending** |
| SCOR-2 | Bonus uses BPS/90 heuristic | Replaced in draft PR #9 by shrunk awarded bonus per estimated appearance | Stage 5 | **CLOSED in draft; verification pending** |
| FIX-1 | Blank/double ease constants are arbitrary | Removed in draft PR #9; blanks contribute zero and doubles add both fixture values over the requested GW denominator | Stage 5 | **CLOSED in draft; verification pending** |
