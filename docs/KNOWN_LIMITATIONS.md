...
Current evidence boundary: A3-R0 is merged at `5ee735f864aaea2b6c423dfaeb267f18f5fe3b2f` with 685 passing tests and reachable exact build provenance. DTR-1 is an unmerged implementation candidate with 691 local passing tests; physical iPhone and permanent remote acceptance are pending. Earlier tested Team behaviour remains accepted, but cannot be transferred to the DTR-1 build until Pritesh performs the exact-device script. Leagues post-Gameweek evidence remains deferred.
...
| UI-18 | Team on `main` still uses a legacy-render-then-reconstruct boundary | The DTR-1 branch directly creates the accepted final DOM and passes 691 local tests, but exact-device equivalence and merge remain unverified | DTR-1 | Implementation candidate; physical/remote acceptance pending |
...
| REFRESH-2 | Foreground refresh mutates runtime provider state before one final render instead of constructing a fully atomic replacement | R1 reduces repeat requests and exposure duration but does not redesign this state boundary | Separate structural investigation and approval | Open (gated) |
...
- Startup and explicit manual loading own the interaction gate. Foreground refresh remains interactive; fully atomic state replacement is still open under REFRESH-2.
