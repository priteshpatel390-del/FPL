...
| PERSIST-4 | `fpl:calib` is unchanged and remains outside this boundary | A legacy calibration record can still affect projections without carrying a current season/model identity; correcting it crosses the model approval gate | Separate model-gated design | Open (gated) |
