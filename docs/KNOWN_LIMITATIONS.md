## API-Football owner-crosswalk closeout limitations

The exact 20-club crosswalk has owner approval but is not yet live-qualified against a current content-bound Official FPL authority. Repository tests can prove the closed contract with synthetic secret material, but they deliberately cannot reveal the real provider-ID pairs. The attended closeout therefore remains required after merge. It depends on protected-environment secret `API_FOOTBALL_OWNER_CROSSWALK_JSON` and sanitized provider artifact `10540321648`, which expires on 25 September 2026; after expiry a new separately approved provider-universe observation would be required rather than weakening the evidence pin. Official FPL network failure, schema/season/identity drift, a crosswalk hash mismatch, provider-universe provenance mismatch, any missing/duplicate mapping, or either legacy-anchor conflict fails closed. No durable mapping store is created here, so even a closeout GO would establish qualification evidence rather than activate the collector or production influence.
