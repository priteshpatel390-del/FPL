...
| RET-2 | No per-provider circuit breaker: each call retries independently, so a pooled sweep retries once per item | Needs a durable provider-down signal, which the seven-state health model (item 4) is being built to produce | On a total outage a 20-rival sweep issues ~200 doomed requests rather than ~100 | Stage 3 item 4 | Open |
