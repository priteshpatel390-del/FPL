...
Repository code can now prepare a least-privilege read-only production preflight, but **no new live Cloudflare/D1 evidence has yet been captured by that workflow**. Therefore the exact current `schema_migrations` ledger, whether migration 0004 is actually present, API-Football live row counts/runtime state, and collector Worker/Cron/secret-binding state remain unproven until a post-merge attended dispatch. The repository record that 0005/0006 were unapplied remains the latest canonical evidence, not a substitute for the live read. A ledger stopping at 0003 is an explicit NO-GO for applying 0005 under the present approval. See [API-Football Live Storage Foundation](API-FOOTBALL-LIVE-STORAGE-FOUNDATION.md).
