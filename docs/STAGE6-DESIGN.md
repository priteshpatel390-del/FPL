...
Status: owner-approved and in implementation on branch `agent/stage6-transfer-optimiser`.
...
A pure optimiser module owns legality, prices, free transfers, hits, best-XI horizon scoring, deterministic plan comparison, exhaustive/reference behaviour and fail-closed limits. Dedicated direct-module tests cover those contracts. The full repository suite, deterministic two-build comparison and generated-artifact identity remain required before completion.
