...
## Current checkpoint
Teamsheet 2.0.4 — Mini Leagues is complete and merged through PR #59 at `a2841b0831193f645548cfc4155809b82a520d92`. Teamsheet 2.0.5 — Mini-League Intelligence was explicitly approved on 2 August 2026 and is implemented on `agent/teamsheet-2.0.5-mini-league-intelligence` for review: an ID-free on-demand exposure route, up to five explicitly selected rivals, bounded two-request concurrency, exact fresh/complete selected-rival player/captain/vice/chip counts, version-2 minimal local choice state and stricter validation of the Official FPL fields used by the comparison. Projected rank, rival-score prediction, remaining-player simulation, effective-ownership strategy, protect/chase logic and every Team/Transfers/model calculation remain unchanged and unapproved. The current branch verification target is **510/510 tests** with deterministic exact-identity builds and root/deployable equality before the draft PR is called complete. Physical testing of the actual repository build on an iPhone, VoiceOver acceptance and live populated-data acceptance have not been performed; source and build tests do not substitute for those checks. After review and merge, the next formal checkpoint is **Teamsheet 2.0.6 — Research, Evidence and Diagnostics Organisation**.
