...
**This section supersedes the older DATA-S1B preflight current-state wording below.** The production `teamsheet-data-platform` Worker, `teamsheet-data` D1, `TEAMSHEET_DATA_DB` binding and `data.fpltsheet.co.uk` Custom Domain already exist; Access remains the proven live outer protection. DATA-S1C now includes a permanent, manual-only GitHub Actions procedure for a separately owner-approved private read-RPC acceptance. GitHub Actions is used because the Codex remote-preview WebSocket transport failed with `ENETUNREACH` before reaching the caller. The procedure creates only Wrangler's temporary edge-preview/remote-binding session, binds the existing read-only caller, and has no direct D1, write or ingest capability. It requires the dedicated `data-s1c-private-acceptance` environment and exactly `CLOUDFLARE_API_TOKEN` and `CLOUDFLARE_ACCOUNT_ID`. After an established persistent PRE-state, POST-state verification and local cleanup run unconditionally, while final enforcement preserves functional failure or topology drift without automatic remediation. First manual run `32896875022` failed safely at PRE-state with both secrets present and functional acceptance skipped. Second run `32899866456` authenticated and validated the caller, then exposed a deployment-history parser defect; read-only reconciliation found no target drift. Third run `32902010718` passed both PRE checks and reached the caller's inert fetch, then Cloudflare recorded one `health` JSRPC call ending in a runtime hang exception before query. PR #156 corrected the caller to await downstream RPC custom thenables, but the following live attempts still executed old caller version `43d28a3a-5720-48b3-950e-b081e33bcc8b` because that source had not been deployed. The corrected caller alone has now been deployed as `cf9c150d-84b0-46f9-a166-530b7243e863` at 100%; its private single-read-binding topology is unchanged, the target remains `5edbe951-4be4-46bc-b2cf-17b550396105`, and the workflow pin is reconciled to the new caller version. Live RPC confirmation still requires separate owner approval; no successful GitHub Actions RPC acceptance is claimed. See [DATA-S1C private Service Binding and RPC architecture](docs/DATA-S1C-PRIVATE-SERVICE-BINDING-RPC.md).
