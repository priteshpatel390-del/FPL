...
**This section supersedes the older DATA-S1B preflight current-state wording below.** The production `teamsheet-data-platform` Worker, `teamsheet-data` D1, `TEAMSHEET_DATA_DB` binding and `data.fpltsheet.co.uk` Custom Domain already exist; Access remains the proven live outer protection. DATA-S1C is repository implementation only: named read and ingest RPC entrypoints share the existing DATA-S1 operation layer, and a private read-only acceptance caller is configured with a Service Binding and no D1. After Cloudflare rejected the first caller upload with error `10068`, the caller gained a registered, inert HTTP handler that always returns an empty 404 while leaving useful access RPC-only. The candidate baseline is **994 tests passed, 0 failed, 0 skipped, 0 cancelled** plus deterministic build/provenance/identity gates. Nothing in this tree proves caller deployment or live RPC acceptance, and no Cloudflare mutation is authorised. See [DATA-S1C private Service Binding and RPC architecture](docs/DATA-S1C-PRIVATE-SERVICE-BINDING-RPC.md).
