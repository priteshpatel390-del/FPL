...
Live isolation proved `pragma_table_xinfo` unsupported (`33604986736`), `PRAGMA table_info` supported but top-level arrays unsupported (`33606192013`), and corrected `{batch:[...]}` groups fully compatible (`33606874736`). Bounded object probe `33607321730` proved the untouched D1 contains exactly Cloudflare's `_cf_KV`, now narrowly allowlisted. Contract run `33607679663` then passed INITIAL, setup, schema reconciliation, affinity, all statement/body profiles and A01; A02 returned Cloudflare's HTTP 400 rejection envelope and reconciled FULL_ROLLBACK. The decoder candidate recognizes that exact nonempty-error envelope as known SQL failure only for intentional A02/A03; every other mutation HTTP 400 remains ambiguous. The disposable database is now setup-mutated and requires separate exact reset/replacement before another from-INITIAL contract run. See [E2C-B preparation](workers/data-platform/DATA-S2B-E2C-B-DISPOSABLE-LIVE-EXPERIMENT-PREPARATION.md).
