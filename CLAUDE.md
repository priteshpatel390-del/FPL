## Current checkpoint — inactive collector staging repository candidate

Pritesh approved repository implementation only of the dormant first-collector infrastructure staging path. This candidate adds a manual, first-attempt-only exact-main workflow that requires a fresh `READY_FOR_REPOSITORY_INFRASTRUCTURE_STAGING` read-only admission before a dedicated protected upload environment can ever execute. Its future mutation surface is structurally limited to one Beta Worker-shell create and one stable inactive Version upload; it cannot deploy, schedule Cron, mutate secrets/routes/D1, call API-Football or enable runtime collection. The first Version explicitly binds the pinned production `teamsheet-data` D1 identity, keeps `REPOSITORY_ONLY_BLOCKED`, carries exactly three plain-text variables and zero secrets, and preserves the exact closed 19-module repository graph rooted at `collector.mjs`. Independent postflight returns to `data-steward-readonly`. Repository implementation does not authorize workflow dispatch or any live Cloudflare mutation; those remain a separate explicit owner gate. See [inactive collector staging](docs/API-FOOTBALL-COLLECTOR-INACTIVE-VERSION-STAGING.md).
