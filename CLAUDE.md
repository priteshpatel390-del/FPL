...
Owner approval covers repository implementation only of a dormant, concrete D1-backed zero-retry collector execution composition. Candidate gives generation orchestration sole ownership, separates transport from semantic success, validates exact discovery/fixture response identity, stages five-request generations, applies a 2,500-fixture ceiling, advances head only after five durable successes, consumes stale reservations without retry, and adds a separate post-0006 activation preflight contract. Shipped Wrangler remains `REPOSITORY_ONLY_BLOCKED` with empty Cron and placeholder D1 identity. No provider request, secret access, production D1 access, Worker deployment/binding, Cron, runtime activation, mapping change, workload ingestion, or model/product/UI influence is authorized. See [collector activation foundation](docs/API-FOOTBALL-COLLECTOR-ACTIVATION-FOUNDATION.md).
