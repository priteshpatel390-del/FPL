...
## Current checkpoint
Teamsheet 2.0.6 — Research, Evidence and Diagnostics Organisation is implemented for review in draft PR #65 on `agent/teamsheet-2.0.6-settings-organisation`; it is not merged. The owner approved the exact implementation scope on 4 August 2026. It adds nested identifier-free Settings routes, explicit evidence/recovery/export/storage mount ownership, consequence-led core-data warnings, complete Help & About guidance, actual build identity presentation, parent-aware history/focus and a mobile Player Explorer card layout. It changes no provider, endpoint, transport, model, expected-minutes, fixture, scoring, squad, captaincy, simulation, transfer optimiser, rank or Mini-League calculation. The review baseline is **520/520 tests** with deterministic exact-identity builds and root `index.html` equal to `dist/index.html`; model remains `2.4.0` and rules remain `2026-27.3`. Physical iPhone testing, VoiceOver acceptance and live populated-data acceptance were not performed. The current gate is owner review and explicit merge approval for PR #65. Do not begin Teamsheet 2.0.7 before 2.0.6 is merged.
