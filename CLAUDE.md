...
### Current Data-Ops checkpoint — A1.4 Persistent Incident Lifecycle + Independent Watchdog, repository candidate (draft PR, unmerged)

**This block adds a new layer alongside A1.3; it does not supersede or reopen any A1.3 fact below.** A1.3's live acceptance, its dormant-scheduling status and `DATA_STEWARD_SCHEDULED_ENABLED` evidence are unchanged and remain exactly as recorded in the block immediately below. Owner instruction is explicit: run #9 skipping *before* the owner enabled `DATA_STEWARD_SCHEDULED_ENABLED=true` is not proof of an A1.3 scheduling bug, and this checkpoint does not treat it as one or modify A1.3's schedule gate.

A1.4 turns the proven read-only A1.3 sensor into an operational monitor **from outside it**: a separate, isolated Cloudflare Worker, `workers/data-steward-watchdog/`, with its own dedicated identity (`teamsheet-data-steward-watchdog`), its own D1 database, its own GitHub read credential (`DATA_STEWARD_WATCHDOG_GITHUB_TOKEN`, distinct from every A1.3 environment name) and its own narrow `send_email` binding. It reads A1.3's own GitHub Actions run/job history (bounded, `GET`-only, plus at most one job-log read per cycle for A1.3's own sanitized summary line), classifies scheduled-heartbeat freshness as HEALTHY (success within 12h) / STALE (not within 12h) / MISSING (not within 24h) — thresholds chosen deliberately loose because this repository has already measured GitHub schedule delivery arriving 3h21m–4h44m late — reduces that plus GitHub-read availability through a pure, replay-safe incident lifecycle reducer (NEW/ONGOING/CHANGED/RECOVERED/REOPENED/NONE) keyed on a stable fingerprint that depends only on a closed (problem class, component) pair, persists state in three fixed, allowlisted-SQL-only D1 tables, and sends at most one email per material transition plus at most one reminder every 24 hours for an unresolved incident, to exactly one Cloudflare-configured fixed recipient it can never override.

**Nothing about A1.1–A1.3's safety boundary changed.** The watchdog imports nothing from `workers/data-steward/` and nothing there imports it (pinned bidirectionally by permanent test); A1.3's workflow, its sentinels and its existing no-mutation regressions are byte-unchanged. The watchdog itself has no ability to dispatch a GitHub workflow, write to GitHub, mutate any Cloudflare resource, invoke or influence production collection, read or write production Official FPL D1 or any database but its own, call a provider, send email to any recipient but the one bound address, or run arbitrary SQL/HTTP/shell — closed by a permanent whole-package scan. Repository suite: **1,915 tests, 1,902 passed, 13 failed** — the 13 failures are pre-existing, environment-specific `node:sqlite` failures in `tests/data-s2-production-query-plan.test.mjs`, reproduced identically against unmodified `origin/main` before this branch's changes, and are unrelated to and unchanged by this PR. Every one of the 93 new A1.4 tests passes.

**Nothing was executed for this checkpoint.** No Cloudflare deployment, D1 database, Cron Trigger, secret, credential, `send_email` binding or GitHub token was created, live-provisioned or activated. `wrangler.jsonc` declares its `send_email` destination as the literal placeholder `REPLACE_LOCALLY_BEFORE_DEPLOY@example.invalid`; the owner's real address is never committed. Declaring one Cloudflare Cron Trigger (`"17 5,11,17,23 * * *"`, four fires a day, one array entry) in the repository arms nothing until an attended deployment of this exact configuration occurs, which this checkpoint does not perform.

**Next gates, each separate:** (1) owner review and merge; (2) exact-`main` Verify; (3) the consolidated live-provisioning package in [A1.4 §14](docs/DATA-OPS-A1-4-WATCHDOG-LIFECYCLE.md#14-live-closeout--one-consolidated-package-for-a-later-separate-owner-gate) — isolated D1 creation/migration, scoped GitHub credential, verified email destination, attended deployment, Cron activation, safe functional acceptance, natural-fire observation — all one later, explicit, combined owner decision. A1.3's own separate scheduled-activation gate is unchanged by any of this. See [A1.4](docs/DATA-OPS-A1-4-WATCHDOG-LIFECYCLE.md).
