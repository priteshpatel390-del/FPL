...
## Current checkpoint
Stage 10 design is owner-approved. Stage 10.1 — Deadline-safe snapshot foundation is implemented and locally verified on draft PR #27 from branch `agent/stage10-1-deadline-snapshot`; it is not merged. The implementation freezes deadline-safe all-player projections, expected minutes, uncertainty summaries, provider provenance and available squad/decision/optimiser context into immutable anonymised JSON evidence. Verified source `09dc35a5aa71e493233e7b6d8cacca92b60abb48` passes **337/337 tests** with a successful production build, byte-identical two-build output and exact build identity. Physical iPhone capture/export/import remains the final review gate before Stage 10.1 can be described as complete. No model formula, provider blend, calibration, captaincy, squad or optimiser objective changed.
