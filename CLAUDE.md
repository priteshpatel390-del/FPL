...
<!-- DATA-S2B-PRODUCTION-CRON-ABSENT-2026-09-02 -->
### Current DATA-S2B checkpoint — production Cron is absent; scheduled collection has stopped

The consolidated read ran. Protected diagnostics run `33644480107`, dispatched from exact `main` `c5db7629d0f7bb7e5d88b8e4b5a4d5fba495370e` after exact-head Verify `33643906698`, completed with no fatal stop; all twenty-two reads passed, including Worker health. One dispatch produced the whole remaining read-only bundle, so the one-failure-per-merge loop is genuinely closed. Bundle outcome `FAIL` names the production state, not a fault in the read.

**The single genuine production defect is that the live Worker carries no Cron triggers at all** (`schedule_count=0`, `classification=absent`). This is worse than the drift previously suspected: the collector requires exactly `*/30 * * * *` and skips any other string as `cron_unrecognised`, but an absent schedule means `scheduled()` is never invoked. The retained ledger dates the stop — three runs exactly 1,440 minutes apart on 30 August (failed, redirect), 31 August (failed, redirect) and 1 September (completed, 9,860 records), then nothing — so the triggers were removed between 01:00 UTC on 1 and 2 September. No repository automation accounts for it: the only Cron-mutating workflow ran once on 29 August, the Phase 4B upload and deployment both precede the 1 September collection the schedule still drove, and every E2C-B run in the window targeted the disposable database. **The cause is outside this repository's automation and is not established by available evidence.** Restoring the schedule is a production Cron mutation and remains an explicit owner gate; it has not been performed.

No stale repository acceptance constant was found. The active Version, rollback and retained Versions, database identity, compatibility date, season binding and re-verified Free-plan limits all matched, so there is nothing of that kind to reconcile. The genuine populated baseline is proven: 9,860 observations reconcile exactly against 9,860 heads and 9,860 distinct logical keys with zero orphans, quarantine and rejections; the redirect remediation is proven live; both failed runs wrote nothing. `unchanged_cycle_proof` and `changed_fact_proof` stay PENDING because each needs a second successful collection, which cannot happen while the schedule is absent. `cpu_resource_suitability` stays PENDING — though the baseline completing rather than being terminated is behavioural evidence it fits the 10 ms Workers Free CPU ceiling — and `d1_accounting` and `observability` have no reachable PASS branch while first-party CPU and row metrics stay unavailable, so **the bundle can never report PASS** without a separate owner decision on a bounded analytics surface. The candidate adds only a stdout echo of the already-sanitized report, so a future bundle stays legible when the artifact host is unreachable. See [read-only production diagnostics](workers/data-platform/DATA-S2B-PHASE-4B-READ-ONLY-DIAGNOSTICS.md).
