## Current checkpoint — attended Version URL remediation after run 36254448484

Owner-approved attended acceptance run `36254448484`, attempt 1, on exact main `5a6f7eaf214fa0b921b6311117b7ccdfa227fc19` passed repository gate, fresh read-only admission, Preview enablement and both bounded D1 control mutations. The executor made exactly one HTTP invocation attempt, but independent reconciliation proved the request never reached the Worker: request attempts / generations / fixture revisions remained `0 / 0 / 0`, API-Football requests remained `0`, collection returned disabled, credential remained `AVAILABLE`, no lease remained, Preview/workers.dev were off, and Deployment/Cron/routes/domains remained zero. Run `36254448484` is consumed and must not be rerun.

Investigation identified an unsupported repository assumption in Version URL construction: acceptance synthesized a hostname from `versionId.slice(0,8)`, while Cloudflare defines the routable Version URL separately and exposes exact routable URLs on the Worker Version object. The repository remediation keeps the existing reviewed Worker/suffix proof, enables Preview, performs exactly one read-only Get Worker Version using `CLOUDFLARE_ATTENDED_READ_TOKEN`, validates the returned single routable URL against the trusted Worker/account suffix, and only then permits D1 enablement and the one-shot invocation. Mutation permissions, D1 control ceilings, provider request plan, no-retry semantics and model/product isolation are unchanged. **No new live acceptance attempt is authorized by this repository remediation or its merge.**

## Current checkpoint — attended critical-preflight credential separation

Owner-approved attended acceptance run `35777295834`, attempt 1, on exact main `cdcd0ed1ad2a2906b2a3c20aecd125ae3d711e4b` passed repository gate and fresh read-only admission, then stopped safely in protected execution as `attended_critical_state_drift__data_platform_binding_mismatch`. Independent reconciliation proved collection disabled, credential `AVAILABLE`, no active lease, request attempts / generations / fixture revisions all `0`, Preview/workers.dev disabled, Deployment/Cron/routes/domains all `0`, exact attended Version/two-Version inventory, and `productionMutations=0`, `apiFootballRequests=0`, `secretValuesRead=0`. Run `35777295834` is consumed and must not be rerun.

The repository-only remediation separates protected execution credentials. A new `CLOUDFLARE_ATTENDED_READ_TOKEN` is required exclusively for the full critical read-only preflight; `CLOUDFLARE_ATTENDED_MUTATION_TOKEN` remains exclusively for the two allowlisted control mutation endpoints. Identical read/mutation token values fail before network. The new read credential is not provisioned by repository work; future protected-environment setup remains owner-controlled and a future live dispatch remains separately gated. **No attended retry, Preview/D1 mutation, provider request or merge is authorized by this remediation.**

## Current checkpoint — attended critical-recheck diagnostic remediation

Owner-approved attended acceptance run `35751021431`, attempt 1, on exact main `1981871d30315ad31cf5274f3f88529592b36856` passed repository gate and fresh read-only attended admission. The protected environment supplied the trigger secret, Cloudflare account identity/fingerprint and attended mutation token, but the executor's fresh pre-mutation critical recheck stopped as `attended_critical_state_drift`. Independent read-only reconciliation proved collection disabled, credential `AVAILABLE`, no active lease, 0 request attempts / generations / fixture revisions, Preview and workers.dev disabled, Deployment/Cron/routes/domains all 0, exact attended Version and two-Version inventory, and `productionMutations=0`, `apiFootballRequests=0`, `secretValuesRead=0`. Run `35751021431` is consumed and must not be rerun.

The repository-only remediation exposes a closed sanitized diagnostic suffix for the critical recheck while preserving the existing fail-closed `attended_critical_state_drift` prefix. Known preflight reasons map only to repository-owned enums such as `inventory_unreadable`, `d1_read_failed`, `version_inventory_mismatch` and `preview_identity_mismatch`; unknown text collapses to `preflight_failed_unknown`. No HTTP status/body, token, account ID, provider payload, secret or arbitrary remote text is surfaced. Owner approval covers tests/build/docs/draft PR only. **No attended retry, Preview/D1 mutation, provider request or merge is authorized by this remediation.**

## Current checkpoint — attended acceptance Preview identity false-negative remediation

Owner-approved attended acceptance run `35748566087`, attempt 1, on exact main `ea423c7089a41287683e8d34f6e65175980e8a4c` stopped safely during fresh read-only admission as `STOP_ATTENDED_ACCEPTANCE_REVIEW_REQUIRED` / `attended_stage_inventory_unexpected`. Protected attended execution was skipped. Independent closeout proved credential `AVAILABLE`, collection disabled, no active lease, request attempts / generations / fixture revisions all `0`, Preview disabled, Deployment/Cron/routes/domains `0`, exact attended Version and exact two-Version inventory, and `productionMutations=0`, `apiFootballRequests=0`, `secretValuesRead=0`. The run is consumed and must not be rerun.

Root cause is a repository false-negative: the legacy Cloudflare Script Subdomain GET used for `enabled` / `previews_enabled` does not expose `preview_url_suffix`, while the acceptance classifier required an exact Preview suffix before mutation. Current Cloudflare Get Worker metadata exposes that suffix. Owner-approved remediation is repository-only: add one bounded read-only Get Worker call during `ATTENDED_ACCEPTANCE`, prove exact worker ID/name and Preview suffix there, keep the legacy Script Subdomain read for disabled-state flags, raise only the attended GET ceiling from 13 to 14, and preserve every no-retry/provider/mutation guard. **Do not rerun attended acceptance or merge without a new explicit gate.**

## Current checkpoint — attended acceptance immutable-provenance remediation

Successful attended credential recovery run `35736888644` on exact main `5a1bd41185e94639a8fc9aec2488f7ee4caf6f8c` independently closed with credential `AVAILABLE`, collection disabled, exact original + attended Version inventory, zero Deployment/Cron/routes/domains and pristine request/generation/fixture-revision history. The attended Version is `04d79556-3070-429f-9944-b5b53d799842` with immutable creation/provenance SHA `69bb84fadbcce94e9fece3ff438d985866cce183`.

The next repository checkpoint is remediation of the separately gated attended acceptance. Current execution SHA and immutable attended-Version provenance must remain distinct through read-only admission, the final protected pre-mutation critical recheck and independent reconciliation. Validation uses a pinned historical 17-module SHA-256 identity rather than mutable current-tree reconstruction. Owner approval covers this repository remediation through tests/build/docs/draft PR only. **Do not dispatch the five-request acceptance, mutate Preview/D1, call API-Football, revoke credentials or merge without the separate explicit gates.**

## Current checkpoint — attended credential recovery provenance remediation

Recovery run `35735507066`, attempt 1, on exact main `34aa60616baac58a3edc30fc6b0a12c9dcac9621` stopped safely before mutation as `STOP_ATTENDED_PREPARATION_VERSION_READY_REVIEW_REQUIRED` / `preparation_version_inventory_unexpected`; the credential job and closeout were skipped and the report recorded 0 production mutations, 0 API-Football requests and 0 secret-value reads. Investigation found a repository false-negative: the recovery preflight validated the already-created attended Version's immutable annotations against the newer recovery workflow SHA instead of its creation/provenance SHA. The attended Version remains `04d79556-3070-429f-9944-b5b53d799842`, created and independently proved on `69bb84fadbcce94e9fece3ff438d985866cce183`.

Owner approval covers investigation and repository remediation through completion, not an automatic production retry. The remediation must keep current-main execution identity separate from immutable Version provenance, pin the known Version ID and creation SHA, preserve exact module/binding/two-Version checks, and retain the one-call/one-statement/one-row D1 mutation ceiling. No Version upload, provider request, Deployment, Preview, Cron, route/domain change, collection enablement or attended acceptance is authorized by the repository fix.
...
**No live staging execution is authorized or has occurred.** Do not dispatch the new workflow, create the protected upload environment, create the Worker, upload a Version, provision/read `API_FOOTBALL_API_KEY`, mutate D1, create Deployment/Cron/routes/domains, enable collection or call API-Football without a new explicit owner gate. Exact branch verification and draft-PR evidence remain required before merge approval.

## Current checkpoint — inactive collector Worker Version / production D1 binding design

Corrected owner-attended activation preflight run `35634186433`, attempt 1, on exact main `d88312eb6263a3ef0e5de1be3e79b647a797b830` passed as `READY_FOR_REPOSITORY_INFRASTRUCTURE_STAGING`. Artifact `10655278906` has SHA-256 `5da299cc5b0bec4064ad7d839e40e05e2b2ad9dde41f7eed785534dfa9bb501b`. Live evidence proved exact migrations 0001–0006, zero FK violations, fresh exact-20 Official FPL authority, committed current 20/20 mapping with exact current canonical coverage, disabled runtime, credential `UNPROVISIONED`, no active lease, zero attempts/generations/fixture revisions, absent collector Worker/Cron/API-key binding, zero model/UI imports, 38,703 D1 rows read, 0 production mutations, 0 API-Football requests and 0 secret-value reads. Owner approval now covers documentation/design only for the next infrastructure mutation gate: an inert Worker shell plus one inactive Version with the production D1 binding, blocked activation, no secret, no Cron and no Deployment. Repository implementation and live execution remain separately owner-gated. See [inactive collector staging proposal](docs/API-FOOTBALL-COLLECTOR-INACTIVE-VERSION-STAGING.md).

## Current checkpoint — activation mapping-provenance false-negative remediation

Owner-attended read-only activation preflight run `35631979158`, attempt 1, on exact main `c4b67cd1709f4c14d53c79896468fa07c632e70a` stopped safely as `STOP_REPOSITORY_INFRASTRUCTURE_STAGING_REVIEW_REQUIRED` / `qualified_mapping_unavailable`. Sanitized evidence proved migrations 0001–0006, zero FK violations, fresh exact-20 Official FPL authority, committed 20/20 mapping cardinality, disabled runtime, zero attempts/generations/fixture revisions, absent collector/Cron/API-key binding, zero model/UI imports, 38,703 D1 rows read, 0 production mutations, 0 API-Football requests and 0 secret-value reads. Investigation proved the classifier compared Migration 0006's immutable historical authority-provenance hash directly with the latest authority digest; that equality is invalid. Owner approval covers repository-only correction: preserve valid historical provenance, independently require fresh Official FPL authority, and require the committed mapping's exact 20 canonical FPL team IDs to equal the current authority team set without exposing provider IDs or mapping pairs. Run `35631979158` is consumed and must not be rerun. No live dispatch, production mutation, provider request, secret action, deployment, binding, runtime or Cron change is authorized by this remediation.

## Current checkpoint — collector activation read-only preflight candidate

Owner approval covers the next gate only: a manual exact-main read-only production admission for API-Football collector repository/infrastructure staging. The candidate adds a new activation-specific workflow and live reader under `data-steward-readonly`; it does not repurpose the historical storage preflight. It may read only bounded Cloudflare Worker/D1 metadata and ten fixed read-only D1 statements, requires zero rows written, and targets only `READY_FOR_REPOSITORY_INFRASTRUCTURE_STAGING`. No Worker Version upload/deployment, production collector binding, API-Football key read/provisioning, runtime mutation, Cron, provider request, mapping mutation, model/product/UI influence or merge is authorized by this candidate. See [activation read-only preflight](docs/API-FOOTBALL-COLLECTOR-ACTIVATION-READONLY-PREFLIGHT.md).

## Current checkpoint — dormant collector activation foundation candidate

Owner approval covers repository implementation only of a dormant, concrete D1-backed zero-retry collector execution composition. Candidate gives generation orchestration sole ownership, gates runtime enablement before any collector mutation, separates transport from semantic success, validates exact discovery/fixture response identity with empty provider errors, stages five-request generations, applies a 2,500-fixture ceiling, advances head only after five durable successes, consumes stale reservations without retry, pins a 50-statement worst-case invocation envelope, and adds a stage-aware post-0006 activation preflight with pristine-history admission for the first acceptance. Shipped Wrangler remains `REPOSITORY_ONLY_BLOCKED` with empty Cron and placeholder D1 identity. No provider request, secret access, production D1 access, Worker deployment/binding, Cron, runtime activation, mapping change, workload ingestion, or model/product/UI influence is authorized. See [collector activation foundation](docs/API-FOOTBALL-COLLECTOR-ACTIVATION-FOUNDATION.md).

## Current checkpoint — Migration 0006 private mapping persistence CLOSED; collector activation investigation next

**Supersedes the Migration 0006 remediation/candidate blocks below; those remain historical evidence.** Migration 0006 schema and the private qualified 20/20 API-Football ↔ Official FPL mapping are now durably persisted and independently reconciled. Owner-attended persistence run `35607354636`, attempt 1, executed on exact SHA `051a68841fd04dde8f5f239dadbfb285231646c3` and returned `DEFINITELY_APPLIED_SUCCESSFULLY`: one D1 batch, HTTP 200, 43/43 statements successful, durable state 20 entity mappings / 1 qualification / 20 members / 1 head, complete 20-club coverage, durable reread/reconstruction success, and 0 API-Football requests. The earlier mutation-submitting runs `35575463178` and `35587588095` are consumed and must never be rerun.

Initial read-only reconciliation run `35607762989` returned `PARTIAL_OR_UNEXPECTED_MAPPING_STATE_REQUIRES_OWNER_ATTENTION` with 24,536 rows read, 0 production mutations and 0 API-Football requests. Investigation proved a reconciliation-code false negative: it incorrectly required the fresh `current_qualification_integrity_hash` to equal the historical owner-approved qualification hash. PR #276 changed only that invariant: the historical approval hash stays pinned while the current qualification hash is independently required to be valid lowercase 64-hex. PR #276 merged as `8c5c7fbf80b70961f2e39d8acc73c806d8c95659`; exact-main Verify Teamsheet run `35608962183` and Pages succeeded.

Final independent read-only reconciliation run `35609344995`, attempt 1, on exact main `8c5c7fbf80b70961f2e39d8acc73c806d8c95659` succeeded with `COMPLETE_QUALIFIED_MAPPING_VISIBLE`, 24,536 rows read, 0 production mutations and 0 API-Football requests. Artifact `10643366243` has SHA-256 `15cead166619fbf07a99c6f7aa5a09b5416080f1c0d122e56b33771fa7b4c2c8`. No retry, repair, restore or additional mapping write is required.

**Next checkpoint:** API-Football collector activation investigation/design only. No production collector deployment, production binding, `API_FOOTBALL_API_KEY` provisioning/read, Cron activation, provider request, production D1 write, `PRELIVE_PLANNER_ONLY` activation, workload ingestion or model/product influence is authorized without a new explicit owner gate.

## Current checkpoint — migration 0006 private-mapping persistence remediation candidate

Owner-approved repository-only remediation follows the second ambiguous private-mapping persistence run `35587588095` on exact main `1e4e82e009543a81631e29f2ebda03ab6040ae26`. Investigation and full migrated-SQLite execution proved the deterministic writer defect: the 19-column qualification insert declared 18 parameter placeholders while binding 17 values, so D1 could not execute the reviewed 43-statement batch. The candidate corrects that single placeholder mismatch, adds closed non-sensitive D1 outcome diagnostics, includes aggregate source-specific `entity_mappings` in comprehensive rereads, permits `DEFINITELY_NOT_APPLIED` only after an explicit rejection plus exact empty state, and keeps transport/malformed/partial/reread uncertainty ambiguous. It also removes the stale single-attempt persistence-hash assumption while strengthening dynamic-hash provenance/cardinality checks. No workflow gate, retry, restore, production query/mutation, API-Football request, deployment, Cron, collector or secret changed. Both consumed production runs remain forbidden from rerun; any future production attempt requires a new separate owner decision after merge and exact-main verification.

## Current checkpoint — attended private-mapping persistence gate candidate

PR #272 merged as `93cfbe47f8c4c17783dc7126026d1a9d9b9da645`; exact-main Verify Teamsheet run `35581847822` succeeded. Owner-attended read-only reconciliation run `35584694866`, attempt 1, then succeeded with sanitized classification `NO_SUBMITTED_MAPPING_STATE_VISIBLE`, 0 production mutations and 0 API-Football requests. Artifact `10631743543` has SHA-256 `d0bd4c795a561f5dc3fe8c68c3ed28e256ec31cbff598bb0ea42130e6f02b014`. This aggregate-only result establishes that the diagnostic saw no submitted mapping state; it does not prove facts outside its fixed queries. The earlier mutation-submitting recovery run `35575463178` remains consumed and must never be rerun. This repository candidate adds a new manual mapping-persistence-only gate that validates the approved reconciliation artifact before fresh `mapping_pre` admission and before the protected writer environment. It reuses the unchanged Migration 0006 mapping runner, keeps the private crosswalk confined to the persistence step, and retains independent sanitized `mapping_post` evidence. Repository implementation does not authorize production dispatch.
...
## Current checkpoint — migration 0006 independent read-only admission candidate

Owner approval covers repository implementation only of a dedicated manual read-only migration-0006 `schema_pre` workflow. It reuses the existing production preflight contract and `data-steward-readonly` credentials, requires exact current main plus exact-head Verify Teamsheet success, and has no D1 write token, private crosswalk, API-Football key, mutation runner, deployment or provider egress. It may establish fresh `READY_FOR_MIGRATION_0006` evidence and the 50,000-row D1 admission gate without starting the mutation-capable workflow. D1 Time Travel availability remains a separate attended execution precondition. No production access or mutation occurred while creating this repository candidate.

## Current checkpoint — migration 0006/private mapping production foundation candidate

Owner approval covers repository implementation only of the dormant, manual, two-phase migration-0006/private-20/20-mapping production foundation. It pins exact 0006 identity and 12-statement shape, preserves the 50,000 account-wide daily D1 write gate, separates schema application from private persistence with independent read-only admission, uses parameterized D1 bindings for private values, reconciles uncertain transport without retry, and emits only sanitized evidence. Migration 0006 has not been applied; no private mapping has been persisted; no API-Football request, collector deployment/binding, API key, Cron, model/calculation or product change occurred. Future production dispatch needs separate explicit owner approval after merge, exact-main Verify and fresh production evidence. See [migration 0006 production foundation](docs/API-FOOTBALL-MIGRATION-0006-PRODUCTION-FOUNDATION.md).

## Current checkpoint — migration 0005 live production closeout complete

**Supersedes the migration-0005 candidate/remediation blocks immediately below; those remain historical.** PR #266 merged as `f07c8acee8aecacfd55ae0e7f015e7799d190294`; exact-main Verify Teamsheet run `35497426515` passed **2,225/2,225** tests with two byte-identical exact-identity production builds and exact manifest identity, and Pages run `35497425916` succeeded. Owner-attended production run `35497565060`, `run_attempt=1`, then applied exactly migration 0005. Read-only preflight returned `READY_FOR_MIGRATION_0005` with current-UTC-day account D1 rows written **1,439** against the **50,000** admission ceiling. Application returned `DEFINITELY_APPLIED_SUCCESSFULLY`, `mutationIssued=true`, `recoveryIssued=false`, 6 D1 API calls, 417,635 rows read, 41 rows written and 24,014 request bytes. Official FPL authority digest was unchanged. Independent postflight returned `READY_FOR_MIGRATION_0006`, ledger exactly 0001–0005, zero FK violations, fresh 20-team Official FPL authority, zero legacy data-platform Cron, disabled API-Football runtime, zero request attempts/fixture revisions/mappings/mapping qualifications, no collector deployment/Cron/API-key binding, and 0 API-Football requests. Migration 0005 is therefore live and accepted. Migration 0006, private mapping persistence, collector deployment/binding, `API_FOOTBALL_API_KEY`, Cron activation, provider egress, workload ingestion and all model/UI influence remain separate explicit owner gates.

## Current checkpoint — migration 0005 first dispatch failed safely; checkout-isolation remediation candidate

Owner-approved remediation follows attended production run `35496920942` on exact main `985cd1427dfa358c45be0e0dc54f1c746f16648d`. The run passed dispatch identity, exact-main, exact-head Verify and all 27 focused migration tests, then failed the repository-gate cleanliness assertion because the checkout was no longer clean. Preflight, migration0005 and postflight were all skipped, so migration 0005 remains unapplied and no production mutation or API-Football request occurred. The failed log does not expose the dirty path, so the exact generated artefact remains unproven. This narrow candidate isolates the local Wrangler/D1 migration harness in its OS temp directory, resolves migration file paths to absolute repository paths, and adds a regression that requires the harness to leave `git status --porcelain --untracked-files=all` unchanged. Candidate Verify Teamsheet run `35497159414` on head `158da28c0884c48f72931d798da389dc924611c7` passed **2,225/2,225** tests, two byte-identical exact-identity production builds and exact manifest identity. This verifies the checkout-isolation regression on that candidate head. Any new production dispatch remains a separate explicit owner approval and must be a new workflow run, not a GitHub rerun.

## Current checkpoint — migration 0005 production application foundation candidate

Owner approval on 20 September 2026 now covers **repository implementation only** of the migration-0005 production application foundation. Baseline protected `main` is `094409e5dd31ca71bef8ff4905d7dbd8ba9fba07`; exact-main Verify Teamsheet run `35468057507` passed **2,214/2,214** tests with byte-identical exact-identity production rebuilds and exact manifest identity, and Pages run `35468056858` succeeded. The 20 September 01:17 UTC external Official FPL collection run `35481020246` also succeeded. The candidate adds a dormant manual-only exact-0005 workflow/runner, exact pre/post reconciliation, a 50,000-account-row daily D1 write-admission ceiling, a pre-mutation Time Travel bookmark and **no automatic restore**. It does not dispatch the workflow, apply 0005/0006, persist the private mapping, deploy/bind/credential/schedule the collector, make an API-Football request, or change model/product behaviour. Candidate-head verification is still required before review. The D1 write-budget gate uses a distinct future `DATA_STEWARD_CLOUDFLARE_ANALYTICS_TOKEN` scoped to Account Analytics Read; this PR does not provision it or widen the existing Workers/D1 read credential. After merge and exact-main verification, production execution remains a new explicit owner gate with that credential prerequisite, a fresh read-only preflight and current D1 write-budget evidence. See [migration 0005 production application foundation](docs/API-FOOTBALL-MIGRATION-0005-PRODUCTION-APPLICATION.md).

## Current checkpoint — migration 0004 live closeout complete

PR #263 merged to protected `main` as `bc87334dbdebce4e19d92374e6640a9fed0f44fa`; post-merge Verify Teamsheet run `35442144198` passed **2,214/2,214** tests with byte-identical exact-identity production rebuilds and exact manifest identity, and Pages run `35442143907` succeeded. Owner-attended production workflow run `35467787882`, `run_attempt=1`, then applied exactly migration 0004. Preflight returned `STOP_0004_NOT_APPLIED`; the mutation stage returned `DEFINITELY_APPLIED_SUCCESSFULLY` with `recoveryIssued=false`; independent postflight returned `READY_FOR_MIGRATION_0005`. The run made 0 API-Football requests and 0 Worker Cron/secret changes. Migration 0004 is therefore live and accepted; migrations 0005/0006, private 20/20 mapping persistence, collector deployment, Worker secret provisioning, Cron activation, provider egress and all model/UI use remain separately owner-gated. See [migration 0004 production application foundation](docs/API-FOOTBALL-MIGRATION-0004-PRODUCTION-APPLICATION.md).
...
PR #260 merged to protected `main` as `d71be7f95b31093d167ce8e33ad3e3fb41785650`; post-merge Verify Teamsheet run `35407292664` passed 2,201/2,201 tests with byte-identical exact-identity production rebuilds. Owner-attended read-only preflight run `35426768910` then completed successfully on that exact main with zero production mutations and zero API-Football requests. Live evidence proved the reviewed production D1 identity and data-platform binding match, the migration ledger is exactly 0001–0003, migrations 0004/0005/0006 are absent, foreign-key violations are zero, and the API-Football collector Worker/deployments/Cron/API-key secret binding are absent. The run returned `STOP_REVIEW_REQUIRED` only because its Official FPL authority read selected every current team metric while the validator required exactly 20 rows; DATA-S2A correctly stores ten team metrics per club. Owner approval now covers a repository-only remediation: select only current `present=true` team heads, require the exact canonical `official-fpl|2026-27|team|<id>|present` key set, and add multi-metric regression coverage. Until that correction is merged and the same read-only preflight is rerun, no migration or production write is authorized. See [live storage foundation read-only preflight](docs/API-FOOTBALL-LIVE-STORAGE-FOUNDATION.md).
...
**This block supersedes earlier API-Football mapping-status blocks below; those remain historical.** PR #257 merged to protected `main` as `aa694bc1d57faa6f197d4118ea0563808fa4ac0b`; post-merge Verify Teamsheet run `35332702502` passed 2,180/2,180 tests with byte-identical exact-identity production rebuilds. The owner then dispatched `API-Football Owner Mapping Qualification` once as run `35333898797`, `run_attempt=1`, on that exact protected main. Both `repository-gate` and `qualify` completed successfully. The sanitized result is **GO 20/20**: 20 verified Premier League teams, complete coverage true, 18 new owner-reviewed receipts validated, 2 legacy receipts reused, 0 unresolved mappings and 0 receipt failures. Current Official FPL authority was fetched at `2026-09-18T10:17:32.351Z` using exactly 2 Official FPL requests; the closeout made 0 API-Football requests. Qualification integrity hash is `670ccccbf62764bc076293962ae4aa85085e06db48b503f597397d882f771367`; approved crosswalk hash remains `d48c7980d1c39e7d4a6f82cb56d25a750286c925fd4996d0f8c728a4fb45c7f5`; provider-universe revision remains `api-football-team-universe:5b404fb0a736a72973b6a1772d984992acea1dff6e2d2d7336c8253a078ae5f1`. Raw provider/Official-FPL bodies, the crosswalk and mapping receipts were not persisted. This closes only team-identity qualification: migration 0005, live D1 mapping persistence, collector, Cron, expected-minutes/model logic and all product/UI influence remain inactive and require separate owner approval. The delivered GW1 readiness checkpoint remains PR #121 and is unchanged by this external-intelligence documentation closeout.

<!-- API-FOOTBALL-TEAM-UNIVERSE-ATTENDED-WORKFLOW-2026-09-18 -->
### Earlier API-Football checkpoint — dedicated team-universe attended workflow preparation

Owner-approved remediation adds a separate dormant manual-only GitHub Actions path for the already-merged qualification-only `GET /teams?league=39&season=2026` contract. It executes exact current `main`, rechecks remote `main` before credential-bearing egress, reuses protected environment `eia-api-football-qualification`, exposes `API_FOOTBALL_API_KEY` to one step only, invokes only `runAttendedApiFootballTeamUniverseQualification()`, and independently refuses a third HTTP egress. Normal success remains one request; the hard maximum remains two attempts under the existing transient-only retry contract. Sanitized integrity-bound evidence is retained outside the repository tree; provider names/codes remain review assistance only and cannot admit mappings. This repository change makes zero provider requests and does not access the credential. Before any later dispatch, an active protection ruleset for `refs/heads/main` and the protected-environment settings must be independently verified. At that preparation stage, mapping remained NO-GO 2/20; collector, migration 0005, Cron, live D1 and every model/UI path were inactive. No new EIA checkpoint identifier is introduced.

<!-- API-FOOTBALL-MAPPING-QUALIFICATION-MERGED-2026-09-18 -->
### Earlier API-Football checkpoint — 20-club mapping qualification contract merge

PR #254 merged to `main` as `83c7d5ce86077502ca691f7ff4e1941568dc083d`. Post-merge Verify Teamsheet run `35321967633` passed 2,162/2,162 tests, byte-identical production rebuilds and exact manifest identity. The qualified API-Football response ceiling remains 720,896 bytes. The repository now contains a dormant qualification-only `/teams?league=39&season=2026` evidence path plus explicit owner-reviewed mapping receipts; `teams` is not in the collector allowlist. At that contract-merge stage, admitted team mapping remained NO-GO 2/20 (Chelsea 49→Official FPL 6; Leeds 63→13). No API-Football provider request or credential access occurred in PR #254; migration 0005 remains unapplied live, collection/Cron remain disabled, and no model/UI path is active. The next separate owner gate is an attended provider-universe observation and explicit review/admission of remaining mappings. See [20-club mapping qualification](docs/API-FOOTBALL-20-CLUB-MAPPING-QUALIFICATION.md).
...
EIA-2I5B adds a repository-only, dormant, hardened API-Football fixture-discovery layer on top of EIA-2I5A identity contracts. The known-ID request API remains closed. A separate bounded discovery surface admits exactly five repository-controlled queries for FPL 2026-27 / provider season 2026 (league IDs 2, 3, 848, 45, 48). One bounded five-competition generation is output-atomic: only a complete successful scan (`scanState === 'completed'`) may return reconciled fixture evidence; incomplete/failed generation exposes `fixtures: []` and does not treat provisional rows as admitted. A valid HTTP 200 zero-row competition is still a successful logical query. Credential-bearing requests stay pinned to `https://v3.football.api-sports.io` with `x-apisports-key`, GET, redirect rejection and injected transport. Every HTTP attempt is bounded by `API_FOOTBALL_REQUEST_TIMEOUT_MS` (15000) via `AbortSignal.timeout`; timeout is sanitized `provider_timeout`, retryable once as transient transport, and still subordinate to HTTP 429. Serial scans use an injected 1s gap, at most one retry per logical request, a ten-attempt ceiling, and stop on HTTP 429 even when quota headers are absent or malformed. Independent cross-source evidence is fixture-scoped. 20-club completeness is a 1:1 bijection against a team universe issued only after DATA-S2A `normaliseOfficialFplHistory()` succeeds on full Official FPL bootstrap+fixtures evidence; a labeled 20-team snapshot cannot self-certify, and malformed optional team representations fail closed as `official_fpl_team_representation_conflicted` instead of throwing. Duplicate provider fixture IDs with incompatible core identity fail closed. Canonical Chelsea–Leeds evidence is API-Football 49→Official FPL 6 and 63→13 on 9 September 2026; 19:00 vs 19:15 remains an explicit kickoff conflict. No real credential, Worker secret, route, Cron, GitHub schedule, D1 writer, migration 0005 or model/UI path was added. Draft PR #249 received an automatic Cloudflare/GitHub **branch preview** deployment; that is not API-Football runtime activation. No authenticated provider call was made. Timeout/abort handling does not make the layer live-ready. See [EIA-2I5B](docs/EIA-2I5B-API-FOOTBALL-PROVIDER-DISCOVERY.md).
...
EIA-2I1 adds a disabled, server-side-capable API-Football adapter for normalized non-Premier-League workload research and the narrow `owner_risk_accepted_private_use` classification. R1 pins credential-bearing requests to one HTTPS origin and four closed endpoints, source-binds owner-risk workload retention, keeps FPL identity only for verified players/PL teams, and uses external identities for non-PL fixtures and competitions. Extra time is duration-backed; `PEN` alone proves nothing. Pritesh accepts residual underlying-rights uncertainty only for private, non-commercial, one-user research; this is not full legal clearance, permits no redistribution/raw-payload warehouse/public/commercial use, and must stop on objection. Direct minutes only; missing evidence remains unknown. The 100-request guard is per-instance/pre-live, not durable daily accounting. No account, credential, live request, D1 mutation/migration, schedule, deployment, model, recommendation or UI change occurred. Real 2026/27 coverage, duration-field semantics and free-tier sufficiency remain pending EIA-2I2. See [EIA-2I1](docs/EIA-2I1-API-FOOTBALL-SHADOW-FOUNDATION.md).
...
**Supersedes the Cron semantic-normalisation block below as the current status; that remediation merged as PR #238, post-merge `main` is `174a7ece2f6c52257902c79ac9a46de846edeb91`, and exact-`main` Verify succeeded (run `34345865860`, run number 662, event `push`).** After that merge and Verify, the owner performed the attended `Data Steward Read-Only Observer` dispatch recorded here: workflow run `34346126189`, run number 8, event `workflow_dispatch`, branch `main`, head SHA `174a7ece2f6c52257902c79ac9a46de846edeb91`, job `observe-production-chain` (job id `102448024505`), job result **SUCCESS**.
...
**Its checkpoint status is superseded by the live-acceptance block above; its design and evidence below remain accurate as a record and are retained.** This remediation merged as **PR #238**. The owner performed an attended `Data Steward Read-Only Observer` dispatch: workflow run `34342701912`, run number 7, event `workflow_dispatch`, branch `main`, head SHA `dfc78882a507e90662f2937582ab0b35af34bdec`. **This was not, by itself, a live acceptance.** No collection, repair, D1 write, schedule activation or Cloudflare mutation occurred.
...
11. Before any new external-data, provider-evaluation, shadow-evidence or ablation proposal: [External Intelligence Foundation](docs/EXTERNAL-INTELLIGENCE-FOUNDATION.md), then the current API-Football records [EIA-2I5B](docs/EIA-2I5B-API-FOOTBALL-PROVIDER-DISCOVERY.md) and [EIA-2I5A](docs/EIA-2I5A-API-FOOTBALL-SHADOW-IDENTITY-SCHEMA.md)
