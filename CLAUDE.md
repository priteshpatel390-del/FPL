...
Live isolation run `33604986736` proved that each of the five `pragma_table_xinfo(...)` statements alone receives HTTP 400 while every other individual inspection succeeds. Follow-up run `33606192013` proved `PRAGMA table_info(...)` succeeds individually but top-level query arrays fail, so the candidate restores D1's `{batch:[...]}` envelope. Corrected batch isolation run `33606874736` passed all 28 probes. Contract run `33606953403` then reached semantic INITIAL rejection; bounded follow-up `33607321730` proved the untouched D1 contains exactly Cloudflare's `_cf_KV` system table. The closed system-object allowlist now admits only `_cf_KV` and SQLite's `sqlite_sequence`; production and unknown objects remain rejected. Generated/hidden columns remain protected through exact `table_info` fields plus DDL declaration-count reconciliation. See [E2C-B preparation](workers/data-platform/DATA-S2B-E2C-B-DISPOSABLE-LIVE-EXPERIMENT-PREPARATION.md).
