...
## Current checkpoint — inactive API-Football collector infrastructure staging candidate

Owner approval now covers **repository implementation only** of the dormant inactive collector infrastructure-staging foundation. Draft PR #283 implements a manual-only, first-attempt exact-main gate, a fresh `data-steward-readonly` admission, a separately protected inactive-staging job, and an independent read-only closeout. The candidate permits only two future mutation primitives: creation of one inert `teamsheet-api-football-shadow-collector` Worker shell and upload of one inactive Version. It does not authorize dispatch, Worker creation, Version upload, Deployment, Cron, routes/domains, secrets, D1 mutation, runtime enablement or API-Football requests.

The upload graph is closed and deterministic from `collector.mjs`. Runtime mapping reads are isolated in `mapping-runtime.mjs`, preserving durable qualification/provenance/integrity checks while excluding owner crosswalk construction and qualification machinery from the future Worker bundle. Cloudflare's current first-party contract still requires the Beta Worker-object create API to establish pre-Version `workers.dev=false` and Preview URLs disabled; the stable Versions API remains the inactive upload primitive. Live execution remains a separate owner gate after merge and exact-main verification.
