...
Read this first. GitHub `main` is the permanent source of truth; repository evidence overrides conversations and old uploads. Last updated: 2026-08-03.
...
## Current checkpoint
Teamsheet 2.0.5 — Mini-League Intelligence is complete and merged through PR #63 at `0b04dd68194207d301667a7100c3ed804ec1e056`. It adds an ID-free on-demand exposure route, up to five explicitly selected rivals, bounded two-request concurrency, exact fresh/complete selected-rival player/captain/vice/chip counts, version-2 minimal local choice state and stricter validation of the Official FPL fields used by the comparison. Projected rank, rival-score prediction, remaining-player simulation, effective-ownership strategy, protect/chase logic and every Team/Transfers/model calculation remain unchanged and unapproved. The verified merged baseline is **510/510 tests** with deterministic exact-identity builds and root `index.html` equal to `dist/index.html`; model version remains `2.4.0` and rules version remains `2026-27.3`. Physical testing of the actual repository build on an iPhone, VoiceOver acceptance and live populated-data acceptance were not performed; automated tests do not substitute for those checks. The next formal checkpoint is **Teamsheet 2.0.6 — Research, Evidence and Diagnostics Organisation**, beginning with investigation and design only. Implementation requires explicit owner approval.
