...
test('API-Football discovery stays isolated from production, live config and migration 0005',()=>{
  ...
  for(const file of workflowFiles){
    ...
    if(file==='api-football-collector-inactive-version-staging.yml'){
      ...
      assert.doesNotMatch(source,/secrets\.API_FOOTBALL_API_KEY|API_FOOTBALL_OWNER_CROSSWALK_JSON|v3\.football\.api-sports\.io|wrangler\s+(?:deploy|secret)|collection_enabled\s*=\s*1|schedule:/i);
      continue;
    }
