...
const OPPORTUNITY='2026-09-10T04:17:00.000Z';
const RUN_ID=34450000001;
const row=(overrides={})=>({opportunity_at:OPPORTUNITY,cron:'17 4 * * *',dispatch_state:'DISPATCHED',
...
test('reads exactly one receipt by exact opportunity through SELECT only',async()=>{
  const fake=db(row());
  const result=await readObserverClockReceipt(fake,OPPORTUNITY);
  assert.deepEqual(result,{opportunityAt:OPPORTUNITY,state:'DISPATCHED',workflowRunId:RUN_ID,
  ...
  assert.deepEqual(fake.state.args,[OPPORTUNITY]);
  assert.equal(fake.state.runs,0);
});

test('missing receipt is absence, never guessed evidence',async()=>{
  assert.equal(await readObserverClockReceipt(db(null),OPPORTUNITY),null);
});
...
test('CLAIMED is valid only while unfinalized and has no trusted run id',async()=>{
  const result=await readObserverClockReceipt(db(row({dispatch_state:'CLAIMED',github_run_id:null,
    reason_code:'observer_dispatch_claimed',finalized_at:null})),OPPORTUNITY);
  ...
  await assert.rejects(readObserverClockReceipt(db(row({dispatch_state:'CLAIMED',github_run_id:null,
    reason_code:'observer_dispatch_claimed'})),OPPORTUNITY),ObserverClockReadError);
});

test('FAILED and AMBIGUOUS require finalization and can never carry a run id',async()=>{
  for(const state of ['FAILED','AMBIGUOUS']){
    const valid=await readObserverClockReceipt(db(row({dispatch_state:state,github_run_id:null})),OPPORTUNITY);
    assert.equal(valid.state,state);assert.equal(valid.workflowRunId,null);
    await assert.rejects(readObserverClockReceipt(db(row({dispatch_state:state})),OPPORTUNITY),ObserverClockReadError);
    await assert.rejects(readObserverClockReceipt(db(row({dispatch_state:state,github_run_id:null,
      finalized_at:null})),OPPORTUNITY),ObserverClockReadError);
  }
});

test('DISPATCHED requires a positive run id, exact cron and valid timestamps',async()=>{
  for(const overrides of [{github_run_id:null},{github_run_id:0},{cron:'17 8 * * *'},
    {claimed_at:'bad'},{finalized_at:null},{opportunity_at:'2026-09-10T08:17:00.000Z'}]){
    await assert.rejects(readObserverClockReceipt(db(row(overrides)),OPPORTUNITY),ObserverClockReadError);
  }
});

test('D1 failures and malformed input fail closed with named clock errors',async()=>{
  await assert.rejects(readObserverClockReceipt(db(null,{throwRead:true}),OPPORTUNITY),error=>
    error instanceof ObserverClockReadError&&error.code==='WATCHDOG_CLOCK_DB_UNAVAILABLE');
  await assert.rejects(readObserverClockReceipt(null,OPPORTUNITY),error=>
