...
/* GW1-P2C2 — behavioural coverage for the reconciled PR #119 browser delivery
   layer. src/ui/evidence-delivery.mjs, src/evidence/outbox.mjs and
   src/ui/evidence.mjs are byte-identical to PR #119, so the delivery semantics
   asserted here are the approved ones; only the transport target moved.

   Exactly two assumptions were adapted from the PR #119 original, both because
   Option A was exhausted and the approved archive is now the sibling subdomain:

     - the ingestion endpoint and Access sign-in origin are
       https://archive.fpltsheet.co.uk, not the workers.dev host;
     - the emitted-endpoint assertion reads the generated deployable
       (index.html) rather than the app.html template. build.mjs deliberately
       still tolerates the legacy value in the template during migration and
       rewrites it, so the template is not the place that proves the exact
       approved endpoint — the build output is.

   Every other behaviour below is unchanged from the approved PR #119 contract.
   'workers.dev' is deliberately retained in the forbidden-persistence list: the
   exhausted origin must never reappear in stored delivery state either.
...
const ENDPOINT = `${ARCHIVE_ORIGIN}/v1/evidence/predeadline`;
...
test('the archive endpoint contract accepts only the exact approved ingestion URL',()=>{
  ...
  for(const bad of ['',`http://archive.fpltsheet.co.uk/v1/evidence/predeadline`,
    `${ARCHIVE_ORIGIN}/v1/health`,
    `${ARCHIVE_ORIGIN}/v1/evidence/predeadline?x=1`,
    `${ARCHIVE_ORIGIN}/v1/evidence/predeadline#f`,
    `${ARCHIVE_ORIGIN}/v1/admin/reconcile`,
    `${ARCHIVE_ORIGIN}/v1/evidence/predeadline/`,
    'https://user:pass@archive.fpltsheet.co.uk/v1/evidence/predeadline','not a url'])
    assert.equal(delivery.normaliseArchiveEndpoint(bad),null,bad);
  assert.equal(delivery.archiveSignInUrl(ENDPOINT),`${ARCHIVE_ORIGIN}/v1/health`);
  /* Sign-in is the archive's own health route and never the ingestion path, so
     an Access challenge can never be answered by an evidence-bearing request. */
  assert.equal(delivery.archiveSignInUrl(ENDPOINT).includes('/v1/evidence/'),false);
  assert.equal(delivery.archiveSignInUrl(null),null);
});
...
test('the browser is permitted to reach the evidence archive and nothing wider',()=>{
  ...
  assert.ok(connect.includes(ARCHIVE_ORIGIN),'the approved archive origin is reachable');
  assert.equal(connect.includes(LEGACY_ARCHIVE_ORIGIN),false,'the exhausted workers.dev archive origin is gone');
  ...
  assert.equal(connect.includes('*.fpltsheet.co.uk'),false,'no wildcard sibling-subdomain trust');

  /* The generated deployable, not the template, is what the browser loads, and
     it must carry exactly the approved ingestion endpoint. app.html is
     deliberately excluded here: build.mjs still tolerates the legacy value in
     the template during migration and rewrites it on the way out. */
  const meta = /name="teamsheet-evidence-archive" content="([^"]+)"/.exec(html)?.[1] || '';
  assert.equal(meta,ENDPOINT);
  assert.equal(delivery.normaliseArchiveEndpoint(meta),ENDPOINT,
    'the emitted endpoint satisfies the client transport contract exactly');
  assert.equal(new URL(meta).pathname,'/v1/evidence/predeadline');
